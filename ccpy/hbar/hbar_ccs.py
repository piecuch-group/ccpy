...
def get_ccs_intermediates_opt(T, H0):
    ...
    # 1-body components
    # -------------------#
    H.a.ov += np.einsum("mnef,fn->me", H0.aa.oovv, T.a, optimize=True) + np.einsum(
        "mnef,fn->me", H0.ab.oovv, T.b, optimize=True
    )
    ...
    H.aa.oooo = 0.5 * H0.aa.oooo + np.einsum(
        "nmje,ei->mnij", I2A_ooov, T.a, optimize=True
    )
    ...
    H.aa.vvvv = 0.5 * H0.aa.vvvv - np.einsum(
        "amef,bm->abef", I2A_vovv, T.a, optimize=True
    )
    H.aa.vvvv -= np.transpose(H.aa.vvvv, (1, 0, 2, 3))

    H.aa.voov += np.einsum("amfe,fi->amie", I2A_vovv, T.a, optimize=True) - np.einsum(
        "nmie,an->amie", I2A_ooov, T.a, optimize=True
    )

    H.aa.vooo = (
        0.5 * H0.aa.vooo
        - 0.25 * np.einsum("mnij,am->amij", H.aa.oooo, T.a, optimize=True)
    ...
    H.aa.vvov = H0.aa.vvov + 0.5 * np.einsum(
        "abfe,fi->abie", H0.aa.vvvv, T.a, optimize=True
    )
    # -------------------#
    # AB parts
    # -------------------#
    ...
    H.ab.oooo += np.einsum("mnej,ei->mnij", I2B_oovo, T.a, optimize=True) + np.einsum(
        "mnie,ej->mnij", I2B_ooov, T.b, optimize=True
    )
    ...
    H.ab.vvvv -= np.einsum("mbef,am->abef", I2B_ovvv, T.a, optimize=True) + np.einsum(
        "amef,bm->abef", I2B_vovv, T.b, optimize=True
    )

    H.ab.voov += np.einsum("amfe,fi->amie", I2B_vovv, T.a, optimize=True) - np.einsum(
        "nmie,an->amie", I2B_ooov, T.a, optimize=True
    )

    H.ab.ovvo += np.einsum("maef,fi->maei", I2B_ovvv, T.b, optimize=True) - np.einsum(
        "mnei,an->maei", I2B_oovo, T.b, optimize=True
    )

    H.ab.ovov += np.einsum("mafe,fi->maie", I2B_ovvv, T.a, optimize=True) - np.einsum(
        "mnie,an->maie", I2B_ooov, T.b, optimize=True
    )

    H.ab.vovo -= np.einsum("nmei,an->amei", I2B_oovo, T.a, optimize=True) - np.einsum(
        "amef,fi->amei", I2B_vovv, T.b, optimize=True
    )

    H.ab.ovoo = (
        H0.ab.ovoo
        - np.einsum("mnij,bn->mbij", H.ab.oooo, T.b, optimize=True)
    ...
    H.bb.oooo = 0.5 * H0.bb.oooo + np.einsum(
        "nmje,ei->mnij", I2C_ooov, T.b, optimize=True
    )
    ...
    H.bb.vvvv = 0.5 * H0.bb.vvvv - np.einsum(
        "amef,bm->abef", I2C_vovv, T.b, optimize=True
    )
    H.bb.vvvv -= np.transpose(H.bb.vvvv, (1, 0, 2, 3))

    H.bb.voov += np.einsum("amfe,fi->amie", I2C_vovv, T.b, optimize=True) - np.einsum(
        "nmie,an->amie", I2C_ooov, T.b, optimize=True
    )

    H.bb.vooo = (
        0.5 * H0.bb.vooo
        - 0.25 * np.einsum("mnij,am->amij", H.bb.oooo, T.b, optimize=True)
    ...
    H.bb.vvov = H0.bb.vvov + 0.5 * np.einsum(
        "abef,fj->baje", H0.bb.vvvv, T.b, optimize=True
    )
...
def get_ccs_intermediates(T, H0):
    ...
    H.aa.vooo = (
        0.5 * H0.aa.vooo
        - 0.25 * np.einsum("nmij,an->amij", H0.aa.oooo, T.a, optimize=True)
        + 0.5 * np.einsum("amef,ei,fj->amij", H0.aa.vovv, T.a, T.a, optimize=True)
        + np.einsum("amie,ej->amij", H0.aa.voov, T.a, optimize=True)
        - 0.25
        * np.einsum("nmef,fj,an,ei->amij", H0.aa.oovv, T.a, T.a, T.a, optimize=True)
    ...
    H.bb.vooo = (
        0.5 * H0.bb.vooo
        - 0.25 * np.einsum("mnij,bn->bmji", H0.bb.oooo, T.b, optimize=True)
        + 0.5 * np.einsum("mbef,ei,fj->bmji", H0.bb.ovvv, T.b, T.b, optimize=True)
        - 0.25
        * np.einsum("mnef,fj,ei,bn->bmji", H0.bb.oovv, T.b, T.b, T.b, optimize=True)
