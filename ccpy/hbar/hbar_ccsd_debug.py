...
def build_hbar_ccsd(T, H0, RHF_symmetry, *args):
    ...
    Q2 = -H0.aa.voov - 0.5 * np.einsum("mnie,bn->bmie", H0.aa.ooov, T.a, optimize=True)
    Q2 = -np.einsum("bmie,am->abie", Q2, T.a, optimize=True)
    Q1 += Q2
    Q1 -= np.transpose(Q1, (1, 0, 2, 3))
    H.aa.vvov += Q1 + (
    ...
    Q2 = -H0.bb.voov - 0.5 * np.einsum("mnie,bn->bmie", H0.bb.ooov, T.b, optimize=True)
    Q2 = -np.einsum("bmie,am->abie", Q2, T.b, optimize=True)
    Q1 += Q2
    Q1 -= np.transpose(Q1, (1, 0, 2, 3))
    H.bb.vvov += Q1 + (
