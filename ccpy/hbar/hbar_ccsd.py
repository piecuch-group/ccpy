...
def get_ccsd_intermediates(cc_t, ints, sys):
    ...
    h1A_oo += 0.5 * np.einsum(
        "jnef,efin->ji", ints["vA"]["oovv"], cc_t["t2a"], optimize=True
    )
    ...
    h1A_vv -= 0.5 * np.einsum(
        "mnbf,afmn->ab", ints["vA"]["oovv"], cc_t["t2a"], optimize=True
    )
    ...
    h1B_oo += 0.5 * np.einsum(
        "jnef,efin->ji", ints["vC"]["oovv"], cc_t["t2c"], optimize=True
    )
    ...
    h1B_vv -= 0.5 * np.einsum(
        "mnbf,afmn->ab", ints["vC"]["oovv"], cc_t["t2c"], optimize=True
    )
    ...
    h2A_vvvv += 0.5 * np.einsum(
        "mnef,abmn->abef", ints["vA"]["oovv"], cc_t["t2a"], optimize=True
    )
    h2A_vvvv += Q1
    ...
    h2B_vvvv += np.einsum(
        "mnef,abmn->abef", ints["vB"]["oovv"], cc_t["t2b"], optimize=True
    )
    ...
    h2C_vvvv += 0.5 * np.einsum(
        "mnef,abmn->abef", ints["vC"]["oovv"], cc_t["t2c"], optimize=True
    )
    h2C_vvvv += Q1
    ...
    h2A_oooo += 0.5 * np.einsum(
        "mnef,efij->mnij", ints["vA"]["oovv"], cc_t["t2a"], optimize=True
    )
    h2A_oooo += Q1
    ...
    h2B_oooo += np.einsum(
        "mnef,efij->mnij", ints["vB"]["oovv"], cc_t["t2b"], optimize=True
    )
    ...
    h2C_oooo += 0.5 * np.einsum(
        "mnef,efij->mnij", ints["vC"]["oovv"], cc_t["t2c"], optimize=True
    )
    h2C_oooo += Q1
    ...
    h2A_voov += np.einsum(
        "nmfe,afin->amie", ints["vA"]["oovv"], cc_t["t2a"], optimize=True
    )
    h2A_voov += np.einsum(
        "mnef,afin->amie", ints["vB"]["oovv"], cc_t["t2b"], optimize=True
    )
    ...
    h2B_voov += np.einsum(
        "nmfe,afin->amie", ints["vB"]["oovv"], cc_t["t2a"], optimize=True
    )
    h2B_voov += np.einsum(
        "nmfe,afin->amie", ints["vC"]["oovv"], cc_t["t2b"], optimize=True
    )
    ...
    h2B_ovvo += np.einsum(
        "mnef,afin->maei", ints["vB"]["oovv"], cc_t["t2c"], optimize=True
    )
    h2B_ovvo += np.einsum(
        "mnef,fani->maei", ints["vA"]["oovv"], cc_t["t2b"], optimize=True
    )
    ...
    h2B_ovov -= np.einsum(
        "mnfe,fain->maie", ints["vB"]["oovv"], cc_t["t2b"], optimize=True
    )
    ...
    h2B_vovo -= np.einsum(
        "nmef,afni->amei", ints["vB"]["oovv"], cc_t["t2b"], optimize=True
    )
    ...
    h2C_voov += np.einsum(
        "nmfe,afin->amie", ints["vC"]["oovv"], cc_t["t2c"], optimize=True
    )
    h2C_voov += np.einsum(
        "nmfe,fani->amie", ints["vB"]["oovv"], cc_t["t2b"], optimize=True
    )

    Q1 = +np.einsum(
        "mnjf,afin->amij", h2A_ooov, cc_t["t2a"], optimize=True
    ) + np.einsum("mnjf,afin->amij", h2B_ooov, cc_t["t2b"], optimize=True)
    Q2 = ints["vA"]["voov"] + 0.5 * np.einsum(
        "amef,ei->amif", ints["vA"]["vovv"], cc_t["t1a"], optimize=True
    )
    ...
    h2A_vooo += 0.5 * np.einsum(
        "amef,efij->amij", ints["vA"]["vovv"], cc_t["t2a"], optimize=True
    )
    h2A_vooo += Q1

    Q1 = ints["vB"]["voov"] + np.einsum(
        "amfe,fi->amie", ints["vB"]["vovv"], cc_t["t1a"], optimize=True
    )
    h2B_vooo = 0.0
    ...
    h2B_vooo += np.einsum(
        "amej,ei->amij", ints["vB"]["vovo"], cc_t["t1a"], optimize=True
    )
    h2B_vooo += np.einsum("amie,ej->amij", Q1, cc_t["t1b"], optimize=True)
    h2B_vooo += np.einsum(
        "amef,efij->amij", ints["vB"]["vovv"], cc_t["t2b"], optimize=True
    )

    Q1 = ints["vB"]["ovov"] + np.einsum(
        "mafe,fj->maje", ints["vB"]["ovvv"], cc_t["t1a"], optimize=True
    )
    h2B_ovoo = 0.0
    ...
    h2B_ovoo += np.einsum(
        "maei,ej->maji", ints["vB"]["ovvo"], cc_t["t1a"], optimize=True
    )
    h2B_ovoo += np.einsum(
        "mafe,feji->maji", ints["vB"]["ovvv"], cc_t["t2b"], optimize=True
    )

    Q1 = np.einsum("mnjf,afin->amij", h2C_ooov, cc_t["t2c"], optimize=True) + np.einsum(
        "nmfj,fani->amij", h2B_oovo, cc_t["t2b"], optimize=True
    )
    Q2 = ints["vC"]["voov"] + 0.5 * np.einsum(
        "amef,ei->amif", ints["vC"]["vovv"], cc_t["t1b"], optimize=True
    )
    ...
    h2C_vooo += 0.5 * np.einsum(
        "amef,efij->amij", ints["vC"]["vovv"], cc_t["t2c"], optimize=True
    )
    h2C_vooo += Q1

    Q1 = np.einsum("bnef,afin->abie", h2A_vovv, cc_t["t2a"], optimize=True) + np.einsum(
        "bnef,afin->abie", h2B_vovv, cc_t["t2b"], optimize=True
    )
    Q2 = ints["vA"]["ovov"] - 0.5 * np.einsum(
        "mnie,bn->mbie", ints["vA"]["ooov"], cc_t["t1a"], optimize=True
    )
    ...
    h2A_vvov += 0.5 * np.einsum(
        "mnie,abmn->abie", ints["vA"]["ooov"], cc_t["t2a"], optimize=True
    )
    h2A_vvov += Q1

    Q1 = ints["vB"]["ovov"] - np.einsum(
        "mnie,bn->mbie", ints["vB"]["ooov"], cc_t["t1b"], optimize=True
    )
    ...
    h2B_vvov += Q1
    h2B_vvov -= np.einsum(
        "amie,bm->abie", ints["vB"]["voov"], cc_t["t1b"], optimize=True
    )
    h2B_vvov += np.einsum(
        "nmie,abnm->abie", ints["vB"]["ooov"], cc_t["t2b"], optimize=True
    )

    Q1 = ints["vB"]["vovo"] - np.einsum(
        "nmei,bn->bmei", ints["vB"]["oovo"], cc_t["t1a"], optimize=True
    )
    ...
    h2B_vvvo += Q1
    h2B_vvvo -= np.einsum(
        "naei,bn->baei", ints["vB"]["ovvo"], cc_t["t1a"], optimize=True
    )
    h2B_vvvo += np.einsum(
        "nmei,banm->baei", ints["vB"]["oovo"], cc_t["t2b"], optimize=True
    )

    Q1 = +np.einsum(
        "bnef,afin->abie", h2C_vovv, cc_t["t2c"], optimize=True
    ) + np.einsum("nbfe,fani->abie", h2B_ovvv, cc_t["t2b"], optimize=True)
    Q2 = ints["vC"]["ovov"] - 0.5 * np.einsum(
        "mnie,bn->mbie", ints["vC"]["ooov"], cc_t["t1b"], optimize=True
    )
    ...
    h2C_vvov += 0.5 * np.einsum(
        "mnie,abmn->abie", ints["vC"]["ooov"], cc_t["t2c"], optimize=True
    )
    h2C_vvov += Q1
