...
def crcc24(cc_t, H1A, H1B, H2A, H2B, H2C, ints, sys, flag_RHF=True, iroot=0):
    ...
    print(
        "CR-CC(2,4)_A = {} Eh     Ecorr_A = {} Eh     Delta_A = {} Eh".format(
            E24A, EcorrA, deltaA
        )
    )
    print(
        "CR-CC(2,4)_B = {} Eh     Ecorr_B = {} Eh     Delta_B = {} Eh".format(
            E24B, EcorrB, deltaB
        )
    )
    print(
        "CR-CC(2,4)_C = {} Eh     Ecorr_C = {} Eh     Delta_C = {} Eh".format(
            E24C, EcorrC, deltaC
        )
    )
    print(
        "CR-CC(2,4)_D = {} Eh     Ecorr_D = {} Eh     Delta_D = {} Eh".format(
            E24D, EcorrD, deltaD
        )
    )
...
def build_L4D(cc_t, ints, sys, iroot=0):
    ...
    L4D += -permute(L4D, [1, 2, 3, 4, 5, 7, 6, 8]) - permute(
        L4D, [1, 2, 3, 4, 5, 8, 7, 6]
    )  # A(j/kl)
    L4D += -permute(L4D, [1, 3, 2, 4, 5, 6, 7, 8]) - permute(
        L4D, [1, 4, 3, 2, 5, 6, 7, 8]
    )  # A(b/cd)
...
def build_L4E(cc_t, ints, sys, iroot=0):
    ...
    L4E = (
        L4E
        - permute(L4E, [1, 2, 3, 4, 7, 6, 5, 8])
        - permute(L4E, [1, 2, 3, 4, 8, 6, 7, 5])
        - permute(L4E, [1, 2, 3, 4, 5, 7, 6, 8])
        - permute(L4E, [1, 2, 3, 4, 5, 8, 7, 6])
        + permute(L4E, [1, 2, 3, 4, 7, 8, 5, 6])
    )  # A(ij/kl)
    L4E = (
        L4E
        - permute(L4E, [3, 2, 1, 4, 5, 6, 7, 8])
        - permute(L4E, [4, 2, 3, 1, 5, 6, 7, 8])
        - permute(L4E, [1, 3, 2, 4, 5, 6, 7, 8])
        - permute(L4E, [1, 4, 3, 2, 5, 6, 7, 8])
