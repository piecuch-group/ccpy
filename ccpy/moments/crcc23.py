...
def crcc23(cc_t,H1A,H1B,H2A,H2B,H2C,ints,sys,flag_RHF=False,nroot=0,omega=0.0):
    ...
    dA_AAA, dB_AAA, dC_AAA, dD_AAA = crcc_loops.crcc23a_opt(cc_t['t2a'],cc_t['l1a'][0],cc_t['l2a'][0],\
                    H2A['vooo'],I2A_vvov,ints['vA']['oovv'],H1A['ov'],H2A['vovv'],H2A['ooov'],ints['fA']['oo'],ints['fA']['vv'],\
    ...
    dA_AAB, dB_AAB, dC_AAB, dD_AAB = crcc_loops.crcc23b_opt(cc_t['t2a'],cc_t['t2b'],cc_t['l1a'][0],cc_t['l1b'][0],\
                    ...
                    ints['fA']['oo'],ints['fA']['vv'],ints['fB']['oo'],ints['fB']['vv'],\
    ...
    if flag_RHF:
        ...
        dA_BBB, dB_BBB, dC_BBB, dD_BBB = crcc_loops.crcc23d_opt(cc_t['t2c'],cc_t['l1b'][0],cc_t['l2c'][0],\
                    H2C['vooo'],I2C_vvov,ints['vC']['oovv'],H1B['ov'],H2C['vovv'],H2C['ooov'],ints['fB']['oo'],ints['fB']['vv'],\
    ...
    for iroot in range(nroot):
        ...
        dA_AAA, dB_AAA, dC_AAA, dD_AAA = crcc_loops.creomcc23a_opt(omega[iroot],cc_t['r0'][iroot],\
                        ...
                        H2A['vovv'],H2A['ooov'],ints['fA']['oo'],ints['fA']['vv'],\
                        H1A['oo'],H1A['vv'],H2A['voov'],H2A['oooo'],H2A['vvvv'],D3A['O'],D3A['V'],\
                        sys['Nocc_a'],sys['Nunocc_a'])
        ...
        if flag_RHF:
            ...
            I2C_vooo = H2C['vooo'] - np.einsum('me,cekj->cmkj',H1B['ov'],cc_t['t2c'],optimize=True)
            I2B_ovoo = H2B['ovoo'] - np.einsum('me,ebij->mbij',H1A['ov'],cc_t['t2b'],optimize=True)
            chi2B_vvov,chi2B_vooo,chi2C_vvvo,chi2C_vooo,chi2B_vvvo,chi2B_ovoo = calc_eomcc23c_intermediates(cc_t,H2A,H2B,H2C,iroot)
            dA_ABB, dB_ABB, dC_ABB, dD_ABB = crcc_loops.creomcc23c_opt(omega[iroot],cc_t['r0'][iroot],\
                    cc_t['t2b'],cc_t['t2c'],cc_t['r2b'][iroot],cc_t['r2c'][iroot],\
                    cc_t['l1a'][iroot+1],cc_t['l1b'][iroot+1],cc_t['l2b'][iroot+1],cc_t['l2c'][iroot+1],\
                    I2B_vooo,I2C_vooo,I2B_ovoo,H2B['vvov'],H2C['vvov'],H2B['vvvo'],\
                    H2B['ovvv'],H2B['vovv'],H2C['vovv'],H2B['oovo'],H2B['ooov'],H2C['ooov'],\
                    chi2B_vvov,chi2B_vooo,chi2C_vvvo,chi2C_vooo,chi2B_vvvo,chi2B_ovoo,\
                    H2B['vooo'],H2C['vooo'],H2B['ovoo'],\
                    H1A['ov'],H1B['ov'],ints['vB']['oovv'],ints['vC']['oovv'],\
                    ints['fA']['oo'],ints['fA']['vv'],ints['fB']['oo'],ints['fB']['vv'],\
                    H1A['oo'],H1A['vv'],H1B['oo'],H1B['vv'],H2A['voov'],\
                    H2B['ovov'],H2B['vovo'],H2B['oooo'],H2B['vvvv'],H2C['voov'],H2C['oooo'],H2C['vvvv'],\
            ...
            dA_BBB, dB_BBB, dC_BBB, dD_BBB = crcc_loops.creomcc23d_opt(omega[iroot],cc_t['r0'][iroot],\
                    cc_t['t2c'],cc_t['r2c'][iroot],cc_t['l1b'][iroot+1],cc_t['l2c'][iroot+1],\
                    H2C['vooo'],I2C_vvov,H2C['vvov'],chi2C_vvvo,chi2C_ovoo,\
                    ints['vC']['oovv'],H1B['ov'],H2C['vovv'],H2C['ooov'],ints['fB']['oo'],ints['fB']['vv'],\
                    H1B['oo'],H1B['vv'],H2C['voov'],H2C['oooo'],H2C['vvvv'],D3D['O'],D3D['V'],\
                    sys['Nocc_b'],sys['Nunocc_b'])
...
def calc_eomcc23c_intermediates(cc_t,H2A,H2B,H2C,iroot):
    ...
    Q2 = np.einsum('mnef,fn->me',H2B['oovv'],cc_t['r1b'][iroot],optimize=True)\
                        +np.einsum('nmfe,fn->me',H2A['oovv'],cc_t['r1a'][iroot],optimize=True)
    # Intermediate 1: X2B(cbke)*Y2C(aeij) -> Z3C(cbakji)
    Int1 = -1.0*np.einsum('cmke,bm->cbke',H2B['voov'],cc_t['r1b'][iroot],optimize=True)
    ...
    Int2 = -1.0*np.einsum('mnkj,cm->cnkj',H2B['oooo'],cc_t['r1a'][iroot],optimize=True)
    ...
    Int3 = np.einsum('amje,bm->abej',H2C['voov'],cc_t['r1b'][iroot],optimize=True) #(*) flipped sign to use H2A(voov) instead of H2A(vovo)
    Int3 += 0.5*np.einsum('abfe,ej->abfj',H2C['vvvv'],cc_t['r1b'][iroot],optimize=True) #(*) added factor 1/2 to compensate A(ab)
    Int3 += 0.25*np.einsum('nmje,abmn->abej',H2C['ooov'],cc_t['r2c'][iroot],optimize=True) #(*) added factor 1/2 to compensate A(ab)
    ...
    Int4 = -0.5*np.einsum('nmij,bm->bnji',H2C['oooo'],cc_t['r1b'][iroot],optimize=True) #(*) added factor 1/2 to compenate A(ij)
    Int4 -= np.einsum('bmie,ej->bmji',H2C['voov'],cc_t['r1b'][iroot],optimize=True) #(*) flipped sign to use H2A(voov) instead of H2A(vovo)
    Int4 += 0.25*np.einsum('bmfe,efij->bmji',H2C['vovv'],cc_t['r2c'][iroot],optimize=True) #(*) added factor 1/2 to compensate A(ij)
    ...
    Int5 = -1.0*np.einsum('cmej,bm->cbej',H2B['vovo'],cc_t['r1b'][iroot],optimize=True)
    ...
    Int6 = -1.0*np.einsum('nmkj,bm->nbkj',H2B['oooo'],cc_t['r1b'][iroot],optimize=True)
...
def calc_eomm23d_intermediates(cc_t,H2B,H2C,iroot):
    ...
    I2 += 0.5*np.einsum('nmje,abmn->abej',H2C['ooov'],cc_t['r2c'][iroot],optimize=True)
    ...
    I1 -= np.transpose(I1,(0,1,3,2))
    I2 = -1.0*np.einsum('nmij,bm->nbij',H2C['oooo'],cc_t['r1b'][iroot],optimize=True)
    I2 += 0.5*np.einsum('bmfe,efij->mbij',H2C['vovv'],cc_t['r2c'][iroot],optimize=True)
    chi2C_ovoo = I1 + I2
