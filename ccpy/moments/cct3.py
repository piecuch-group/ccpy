...
def calc_eomm23a_intermediates(T, R, H):
    Q1 = ccpy_einsum("mnef,fn->me", H.aa.oovv, R.a)
    Q1 += ccpy_einsum("mnef,fn->me", H.ab.oovv, R.b)

    I1 = ccpy_einsum("amje,bm->abej", H.aa.voov, R.a)
    I1 += ccpy_einsum("amfe,bejm->abfj", H.aa.vovv, R.aa)
    I1 += ccpy_einsum("amfe,bejm->abfj", H.ab.vovv, R.ab)
    I1 -= np.transpose(I1, (1, 0, 2, 3))
    I2 = ccpy_einsum("abfe,ej->abfj", H.aa.vvvv, R.a)
    I2 += 0.5 * ccpy_einsum("nmje,abmn->abej", H.aa.ooov, R.aa)
    I2 -= ccpy_einsum("me,abmj->abej", Q1, T.aa)
    chi2A_vvvo = I1 + I2

    I1 = -ccpy_einsum("bmie,ej->mbij", H.aa.voov, R.a)
    I1 += ccpy_einsum("nmie,bejm->nbij", H.aa.ooov, R.aa)
    I1 += ccpy_einsum("nmie,bejm->nbij", H.ab.ooov, R.ab)
    I1 -= np.transpose(I1, (0, 1, 3, 2))
    I2 = -1.0 * ccpy_einsum("nmij,bm->nbij", H.aa.oooo, R.a)
    I2 += 0.5 * ccpy_einsum("bmfe,efij->mbij", H.aa.vovv, R.aa)
    chi2A_ovoo = I1 + I2
...
def calc_eomcc23b_intermediates(T, R, H):
    ...
    Int1 = -1.0 * ccpy_einsum("mcek,bm->bcek", H.ab.ovvo, R.a)
    Int1 -= ccpy_einsum("bmek,cm->bcek", H.ab.vovo, R.b)
    Int1 += ccpy_einsum("bcfe,ek->bcfk", H.ab.vvvv, R.b)
    Int1 += ccpy_einsum("mnek,bcmn->bcek", H.ab.oovo, R.ab)
    Int1 += ccpy_einsum("bmfe,ecmk->bcfk", H.aa.vovv, R.ab)
    Int1 += ccpy_einsum("bmfe,ecmk->bcfk", H.ab.vovv, R.bb)
    Int1 -= ccpy_einsum("mcfe,bemk->bcfk", H.ab.ovvv, R.ab)
    # Intermediate 2: X2B(ncjk)*Y2A(abin) -> Z3B(abcijk)
    Int2 = -1.0 * ccpy_einsum("nmjk,cm->ncjk", H.ab.oooo, R.b)
    Int2 += ccpy_einsum("mcje,ek->mcjk", H.ab.ovov, R.b)
    Int2 += ccpy_einsum("mcek,ej->mcjk", H.ab.ovvo, R.a)
    Int2 += ccpy_einsum("mcef,efjk->mcjk", H.ab.ovvv, R.ab)
    Int2 += ccpy_einsum("nmje,ecmk->ncjk", H.aa.ooov, R.ab)
    Int2 += ccpy_einsum("nmje,ecmk->ncjk", H.ab.ooov, R.bb)
    Int2 -= ccpy_einsum("nmek,ecjm->ncjk", H.ab.oovo, R.ab)
    ...
    Int3 += 0.5 * ccpy_einsum("abfe,ej->abfj", H.aa.vvvv, R.a)  # (*) added factor 1/2 to compensate A(ab)
    Int3 += 0.25 * np.einsum("nmje,abmn->abej", H.aa.ooov, R.aa,
                             optimize=True)  # (*) added factor 1/2 to compensate A(ab)
    Int3 += ccpy_einsum("amfe,bejm->abfj", H.aa.vovv, R.aa)
    Int3 += ccpy_einsum("amfe,bejm->abfj", H.ab.vovv, R.ab)
    Int3 -= 0.5 * ccpy_einsum("me,abmj->abej", Q1, T.aa)  # (*) added factor 1/2 to compensate A(ab)
    Int3 -= np.transpose(Int3, (1, 0, 2, 3))
    # Intermediate 4: X2A(bnji)*Y2B(acnk) -> Z3B(abcijk)
    Int4 = -0.5 * ccpy_einsum("nmij,bm->bnji", H.aa.oooo, R.a)  # (*) added factor 1/2 to compenate A(ij)
    ...
    Int4 += ccpy_einsum("nmie,bejm->bnji", H.aa.ooov, R.aa)
    Int4 += ccpy_einsum("nmie,bejm->bnji", H.ab.ooov, R.ab)
    Int4 += 0.5 * ccpy_einsum("me,ebij->bmji", Q1, T.aa)  # (*) added factor 1/2 to compensate A(ij)
    Int4 -= np.transpose(Int4, (0, 1, 3, 2))
    # Intermediate 5: X2B(bcje)*Y2B(aeik) -> Z3B(abcijk)
    Int5 = -1.0 * ccpy_einsum("mcje,bm->bcje", H.ab.ovov, R.a)
    Int5 -= ccpy_einsum("bmje,cm->bcje", H.ab.voov, R.b)
    Int5 += ccpy_einsum("bcef,ej->bcjf", H.ab.vvvv, R.a)
    Int5 += ccpy_einsum("mnjf,bcmn->bcjf", H.ab.ooov, R.ab)
    Int5 += ccpy_einsum("mcef,bejm->bcjf", H.ab.ovvv, R.aa)
    Int5 += ccpy_einsum("cmfe,bejm->bcjf", H.bb.vovv, R.ab)
    Int5 -= ccpy_einsum("bmef,ecjm->bcjf", H.ab.vovv, R.ab)
    # Intermediate 6: X2B(bnjk)*Y2B(acin) -> Z3B(abcijk)
    Int6 = -1.0 * ccpy_einsum("mnjk,bm->bnjk", H.ab.oooo, R.a)
    Int6 += ccpy_einsum("bmje,ek->bmjk", H.ab.voov, R.b)
    Int6 += ccpy_einsum("bmek,ej->bmjk", H.ab.vovo, R.a)
    Int6 += ccpy_einsum("bnef,efjk->bnjk", H.ab.vovv, R.ab)
    Int6 += ccpy_einsum("mnek,bejm->bnjk", H.ab.oovo, R.aa)
    Int6 += ccpy_einsum("nmke,bejm->bnjk", H.bb.ooov, R.ab)
    Int6 -= ccpy_einsum("nmje,benk->bmjk", H.ab.ooov, R.ab)
    Int6 += ccpy_einsum("me,bejk->bmjk", Q2, T.ab)
...
def calc_eomcc23c_intermediates(T, R, H):
    Q1 = (
            ccpy_einsum("mnef,fn->me", H.bb.oovv, R.b)
            + ccpy_einsum("nmfe,fn->me", H.ab.oovv, R.a)
    )
    Q2 = (
            ccpy_einsum("mnef,fn->me", H.ab.oovv, R.b)
            + ccpy_einsum("nmfe,fn->me", H.aa.oovv, R.a)
    )
    # Intermediate 1: X2B(cbke)*Y2C(aeij) -> Z3C(cbakji)
    Int1 = -1.0 * ccpy_einsum("cmke,bm->cbke", H.ab.voov, R.b)
    Int1 -= ccpy_einsum("mbke,cm->cbke", H.ab.ovov, R.a)
    Int1 += ccpy_einsum("cbef,ek->cbkf", H.ab.vvvv, R.a)
    Int1 += ccpy_einsum("nmke,cbnm->cbke", H.ab.ooov, R.ab)
    Int1 += ccpy_einsum("bmfe,cekm->cbkf", H.bb.vovv, R.ab)
    Int1 += ccpy_einsum("mbef,ecmk->cbkf", H.ab.ovvv, R.aa)
    Int1 -= ccpy_einsum("cmef,ebkm->cbkf", H.ab.vovv, R.ab)
    # Intermediate 2: X2B(cnkj)*Y2C(abin) -> Z3C(cbakji)
    Int2 = -1.0 * ccpy_einsum("mnkj,cm->cnkj", H.ab.oooo, R.a)
    Int2 += ccpy_einsum("cmej,ek->cmkj", H.ab.vovo, R.a)
    Int2 += ccpy_einsum("cmke,ej->cmkj", H.ab.voov, R.b)
    Int2 += ccpy_einsum("cmfe,fekj->cmkj", H.ab.vovv, R.ab)
    Int2 += ccpy_einsum("nmje,cekm->cnkj", H.bb.ooov, R.ab)
    Int2 += ccpy_einsum("mnej,ecmk->cnkj", H.ab.oovo, R.aa)
    Int2 -= ccpy_einsum("mnke,cemj->cnkj", H.ab.ooov, R.ab)
    # Intermediate 3: X2C(abej)*Y2B(ceki) -> Z3C(cbakji)
    Int3 = np.einsum("amje,bm->abej", H.bb.voov, R.b,
                     optimize=True)  # (*) flipped sign to use H2A(voov) instead of H2A(vovo)
    Int3 += 0.5 * ccpy_einsum("abfe,ej->abfj", H.bb.vvvv, R.b)  # (*) added factor 1/2 to compensate A(ab)
    Int3 += 0.25 * np.einsum("nmje,abmn->abej", H.bb.ooov, R.bb,
                             optimize=True)  # (*) added factor 1/2 to compensate A(ab)
    Int3 += ccpy_einsum("amfe,bejm->abfj", H.bb.vovv, R.bb)
    Int3 += ccpy_einsum("maef,ebmj->abfj", H.ab.ovvv, R.ab)
    Int3 -= 0.5 * ccpy_einsum("me,abmj->abej", Q1, T.bb)  # (*) added factor 1/2 to compensate A(ab)
    Int3 -= np.transpose(Int3, (1, 0, 2, 3))
    # Intermediate 4: X2C(bnji)*Y2B(cakn) -> Z3C(cbakji)
    Int4 = -0.5 * ccpy_einsum("nmij,bm->bnji", H.bb.oooo, R.b)  # (*) added factor 1/2 to compenate A(ij)
    Int4 -= np.einsum("bmie,ej->bmji", H.bb.voov, R.b,
                      optimize=True)  # (*) flipped sign to use H2A(voov) instead of H2A(vovo)
    Int4 += 0.25 * np.einsum("bmfe,efij->bmji", H.bb.vovv, R.bb,
                             optimize=True)  # (*) added factor 1/2 to compensate A(ij)
    Int4 += ccpy_einsum("nmie,bejm->bnji", H.bb.ooov, R.bb)
    Int4 += ccpy_einsum("mnei,ebmj->bnji", H.ab.oovo, R.ab)
    Int4 += 0.5 * ccpy_einsum("me,ebij->bmji", Q1, T.bb)  # (*) added factor 1/2 to compensate A(ij)
    Int4 -= np.transpose(Int4, (0, 1, 3, 2))
    # Intermediate 5: X2B(cbej)*Y2B(eaki) -> Z3C(cbakji)
    Int5 = -1.0 * ccpy_einsum("cmej,bm->cbej", H.ab.vovo, R.b)
    Int5 -= ccpy_einsum("mbej,cm->cbej", H.ab.ovvo, R.a)
    Int5 += ccpy_einsum("cbfe,ej->cbfj", H.ab.vvvv, R.b)
    Int5 += ccpy_einsum("nmfj,cbnm->cbfj", H.ab.oovo, R.ab)
    Int5 += ccpy_einsum("cmfe,bejm->cbfj", H.ab.vovv, R.bb)
    Int5 += ccpy_einsum("cmfe,ebmj->cbfj", H.aa.vovv, R.ab)
    Int5 -= ccpy_einsum("mbfe,cemj->cbfj", H.ab.ovvv, R.ab)
    # Intermediate 6: X2B(nbkj)*Y2B(cani) -> Z3C(cbakji)
    Int6 = -1.0 * ccpy_einsum("nmkj,bm->nbkj", H.ab.oooo, R.b)
    Int6 += ccpy_einsum("mbej,ek->mbkj", H.ab.ovvo, R.a)
    Int6 += ccpy_einsum("mbke,ej->mbkj", H.ab.ovov, R.b)
    Int6 += ccpy_einsum("nbfe,fekj->nbkj", H.ab.ovvv, R.ab)
    Int6 += ccpy_einsum("nmke,bejm->nbkj", H.ab.ooov, R.bb)
    Int6 += ccpy_einsum("nmke,ebmj->nbkj", H.aa.ooov, R.ab)
    Int6 -= ccpy_einsum("mnej,ebkn->mbkj", H.ab.oovo, R.ab)
    Int6 += ccpy_einsum("me,ebkj->mbkj", Q2, T.ab)

    return Int1, Int2, Int3, Int4, Int5, Int6


def calc_eomm23d_intermediates(T, R, H):
    Q1 = ccpy_einsum("mnef,fn->me", H.bb.oovv, R.b)
    Q1 += ccpy_einsum("nmfe,fn->me", H.ab.oovv, R.a)

    I1 = ccpy_einsum("amje,bm->abej", H.bb.voov, R.b)
    I1 += ccpy_einsum("amfe,bejm->abfj", H.bb.vovv, R.bb)
    I1 += ccpy_einsum("maef,ebmj->abfj", H.ab.ovvv, R.ab)
    I1 -= np.transpose(I1, (1, 0, 2, 3))
    I2 = ccpy_einsum("abfe,ej->abfj", H.bb.vvvv, R.b)
    I2 += 0.5 * ccpy_einsum("nmje,abmn->abej", H.bb.ooov, R.bb)
    I2 -= ccpy_einsum("me,abmj->abej", Q1, T.bb)
    chi2C_vvvo = I1 + I2

    I1 = -ccpy_einsum("bmie,ej->mbij", H.bb.voov, R.b)
    I1 += ccpy_einsum("nmie,bejm->nbij", H.bb.ooov, R.bb)
    I1 += ccpy_einsum("mnei,ebmj->nbij", H.ab.oovo, R.ab)
    I1 -= np.transpose(I1, (0, 1, 3, 2))
    I2 = -1.0 * ccpy_einsum("nmij,bm->nbij", H.bb.oooo, R.b)
    I2 += 0.5 * ccpy_einsum("bmfe,efij->mbij", H.bb.vovv, R.bb)
    chi2C_ovoo = I1 + I2
