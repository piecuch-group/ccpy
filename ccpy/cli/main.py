...
if __name__ == "__main__":
    ...
    print("  CC shift = {}".format(inputs["ccshift"]))
    ...
    if "EOM" in inputs["calc_type"]:
        ...
        print(
            "  EOMCC Initial Guess: no = {}  nu = {}".format(
                inputs["eom_guess_noact"], inputs["eom_guess_nuact"]
            )
        )
    ...
    for i in range(sys["Norb"]):
        print(
            "     {}       {:>6f}          {}         {}".format(
                i + 1, sys["mo_energy"][i], sys["sym"][i], sys["mo_occ"][i]
            )
        )
