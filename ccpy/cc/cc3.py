...
### CODE FOR DOING BATCHED (abc) VECTORIZATION IN PYTHON ###
    ...
    #             t3a_abc = -0.5 * ccpy_einsum("am,bcm->abc", X["aa"]["vooo"][:, :, i, j], T.aa[:, :, :, k])
    #             t3a_abc += 0.5 * ccpy_einsum("am,bcm->abc", X["aa"]["vooo"][:, :, k, j], T.aa[:, :, :, i])
    #             t3a_abc += 0.5 * ccpy_einsum("am,bcm->abc", X["aa"]["vooo"][:, :, i, k], T.aa[:, :, :, j])
    #             # 1/2 A(i/jk)A(abc) I(abie) * t(ecjk)
    #             t3a_abc += 0.5 * ccpy_einsum("abe,ec->abc", X["aa"]["vvov"][:, :, i, :], T.aa[:, :, j, k])
    #             t3a_abc -= 0.5 * ccpy_einsum("abe,ec->abc", X["aa"]["vvov"][:, :, j, :], T.aa[:, :, i, k])
    #             t3a_abc -= 0.5 * ccpy_einsum("abe,ec->abc", X["aa"]["vvov"][:, :, k, :], T.aa[:, :, j, i])
    ...
    #             dT.a[:, i] += 0.5 * ccpy_einsum("bc,abc->a", H.aa.oovv[j, k, :, :], t3a_abc)
    #             dT.a[:, j] -= 0.5 * ccpy_einsum("bc,abc->a", H.aa.oovv[i, k, :, :], t3a_abc)
    #             dT.a[:, k] -= 0.5 * ccpy_einsum("bc,abc->a", H.aa.oovv[j, i, :, :], t3a_abc)
    #             # Compute diagram: A(ij) [A(k/ij) h(ke) * t3(abeijk)]
    #             dT.aa[:, :, i, j] += 0.5 * ccpy_einsum("e,abe->ab", hbar.a.ov[k, :], t3a_abc) # (1)
    #             dT.aa[:, :, j, k] += 0.5 * ccpy_einsum("e,abe->ab", hbar.a.ov[i, :], t3a_abc) # (ik)
    #             dT.aa[:, :, i, k] -= 0.5 * ccpy_einsum("e,abe->ab", hbar.a.ov[j, :], t3a_abc) # (jk)
    #             # Compute diagram: -A(j/ik) h(ik:f) * t3(abfijk)
    #             dT.aa[:, :, :, j] -= 0.5 * ccpy_einsum("mf,abf->abm", hbar.aa.ooov[i, k, :, :], t3a_abc)
    #             dT.aa[:, :, :, i] += 0.5 * ccpy_einsum("mf,abf->abm", hbar.aa.ooov[j, k, :, :], t3a_abc)
    #             dT.aa[:, :, :, k] += 0.5 * ccpy_einsum("mf,abf->abm", hbar.aa.ooov[i, j, :, :], t3a_abc)
    #             # Compute diagram: 1/2 A(k/ij) h(akef) * t3(ebfijk)
    #             dT.aa[:, :, i, j] += 0.5 * ccpy_einsum("aef,ebf->ab", hbar.aa.vovv[:, k, :, :], t3a_abc)
    #             dT.aa[:, :, j, k] += 0.5 * ccpy_einsum("aef,ebf->ab", hbar.aa.vovv[:, i, :, :], t3a_abc)
    #             dT.aa[:, :, i, k] -= 0.5 * ccpy_einsum("aef,ebf->ab", hbar.aa.vovv[:, j, :, :], t3a_abc)
    # ### t3aab ###
    ...
    #             t3b_abc = ccpy_einsum("bce,ae->abe", X["ab"]["vvvo"][:, :, :, k], T.aa[:, :, i, j,])
    #             # Diagram 2: -A(ij) I2B(mcjk) * t2a(abim)
    #             t3b_abc -= 0.5 * ccpy_einsum("mc,abm->abc", X["ab"]["ovoo"][:, :, j, k], T.aa[:, :, i, :])
    #             t3b_abc += 0.5 * ccpy_einsum("mc,abm->abc", X["ab"]["ovoo"][:, :, i, k], T.aa[:, :, j, :])
    #             # Diagram 3: A(ab)A(ij) H2B(acie)*t2b(bejk)
    #             t3b_abc += ccpy_einsum("ace,be->abc", X["ab"]["vvov"][:, :, i, :], T.ab[:, :, j, k])
    #             t3b_abc -= ccpy_einsum("ace,be->abc", X["ab"]["vvov"][:, :, j, :], T.ab[:, :, i, k])
    #             # Diagram 4: -A(ab)A(ij) I2B(amik)*t2b(bcjm)
    #             t3b_abc -= ccpy_einsum("am,bcm->abc", X["ab"]["vooo"][:, :, i, k], T.ab[:, :, j, :])
    #             t3b_abc += ccpy_einsum("am,bcm->abc", X["ab"]["vooo"][:, :, j, k], T.ab[:, :, i, :])
    #             # Diagram 5: A(ij) H2A(abie)*t2b(ecjk)
    #             t3b_abc += 0.5 * ccpy_einsum("abe,ec->abc", X["ab"]["vvov"][:, :, i, :], T.ab[:, :, j, k])
    #             t3b_abc -= 0.5 * ccpy_einsum("abe,ec->abc", X["ab"]["vvov"][:, :, j, :], T.ab[:, :, i, k])
    #             # Diagram 6: -A(ab) I2A(amij)*t2b(bcmk)
    #             t3b_abc -= ccpy_einsum("am,bcm->abc", X["aa"]["vooo"][:, :, i, j], T.ab[:, :, :, k])
    #             # Antisymmetrize A(ab)
    #             t3b_abc -= np.transpose(t3b_abc, (1, 0, 2)) # A(ab)
    #             # Divide t_abc by the denominator
    #             t3b_abc /= (e_ijk + e_abc)
    #             # A(ij)A(ab) vB(jkbc) * t3b(abcijk)
    #             dT.a[:, i] += ccpy_einsum("bc,abc->a", H.ab.oovv[j, k, :, :], t3b_abc)
    #             dT.a[:, j] -= ccpy_einsum("bc,abc->a", H.ab.oovv[i, k, :, :], t3b_abc)
    #             # vA(ijab) * t3b(abcijk)
    #             dT.b[:, k] += ccpy_einsum("ab,abc->c", H.aa.oovv[i, j, :, :], t3b_abc)
    #             # A(ij)A(ab) [h1b(me) * t3b(abeijm)]
    #             dT.aa[:, :, i, j] += ccpy_einsum("c,abc->ab", hbar.b.ov[k, :], t3b_abc)
    #             # A(ij)A(ab) [A(jm) -h2b(mnif) * t3b(abfmjn)]
    #             dT.aa[:, :, :, j] -= ccpy_einsum("mc,abc->abm", hbar.ab.ooov[i, k, :, :], t3b_abc)
    #             dT.aa[:, :, :, i] += ccpy_einsum("mc,abc->abm", hbar.ab.ooov[j, k, :, :], t3b_abc)
    #             # A(ij)A(ab) [A(be) h2b(anef) * t3b(ebfijn)]
    #             dT.aa[:, :, i, j] += ccpy_einsum("eac,abc->eb", hbar.ab.vovv[:, k, :, :], t3b_abc)
    #             # A(af) -h2a(mnif) * t3b(afbmnj)
    #             dT.ab[:, :, :, k] -= ccpy_einsum("mb,abc->acm", hbar.aa.ooov[i, j, :, :], t3b_abc)
    #             # A(af)A(in) -h2b(nmfj) * t3b(afbinm)
    #             dT.ab[:, :, i, :] -= ccpy_einsum("bm,abc->acm", hbar.ab.oovo[j, k, :, :], t3b_abc)
    #             dT.ab[:, :, j, :] += ccpy_einsum("bm,abc->acm", hbar.ab.oovo[i, k, :, :], t3b_abc)
    #             # A(in) h2a(anef) * t3b(efbinj)
    #             dT.ab[:, :, i, k] += ccpy_einsum("eab,abc->ec", hbar.aa.vovv[:, j, :, :], t3b_abc)
    #             dT.ab[:, :, j, k] -= ccpy_einsum("eab,abc->ec", hbar.aa.vovv[:, i, :, :], t3b_abc)
    #             # A(af)A(in) h2b(nbfe) * t3b(afeinj)
    #             dT.ab[:, :, i, k] += ccpy_einsum("ebc,abc->ae", hbar.ab.ovvv[j, :, :, :], t3b_abc)
    #             dT.ab[:, :, j, k] -= ccpy_einsum("ebc,abc->ae", hbar.ab.ovvv[i, :, :, :], t3b_abc)
    #             # A(ae)A(im) h1a(me) * t3b(aebimj)
    #             dT.ab[:, :, i, k] += ccpy_einsum("b,abc->ab", hbar.a.ov[j, :], t3b_abc)
    #             dT.ab[:, :, j, k] -= ccpy_einsum("b,abc->ab", hbar.a.ov[i, :], t3b_abc)
