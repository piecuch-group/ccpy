...
def update_t2a(T, dT, H, H0, VT_ext, shift, T_ext):
    ...
    # T3 parts
    dT.aa += 0.25 * ccpy_einsum("me,abeijm->abij", H.a.ov, T_ext.aaa)
    dT.aa += 0.25 * ccpy_einsum("me,abeijm->abij", H.b.ov, T_ext.aab)
    dT.aa -= 0.5 * ccpy_einsum("mnif,abfmjn->abij", H0.ab.ooov + H.ab.ooov, T_ext.aab)
    dT.aa -= 0.25 * ccpy_einsum("mnif,abfmjn->abij", H0.aa.ooov + H.aa.ooov, T_ext.aaa)
    dT.aa += 0.25 * ccpy_einsum("anef,ebfijn->abij", H0.aa.vovv + H.aa.vovv, T_ext.aaa)
    dT.aa += 0.5 * ccpy_einsum("anef,ebfijn->abij", H0.ab.vovv + H.ab.vovv, T_ext.aab)
...
def update_t2b(T, dT, H, H0, VT_ext, shift, T_ext):
    ...
    # T3 parts
    dT.ab -= 0.5 * ccpy_einsum("mnif,afbmnj->abij", H0.aa.ooov + H.aa.ooov, T_ext.aab)
    dT.ab -= ccpy_einsum("nmfj,afbinm->abij", H0.ab.oovo + H.ab.oovo, T_ext.aab)
    dT.ab -= 0.5 * ccpy_einsum("mnjf,afbinm->abij", H0.bb.ooov + H.bb.ooov, T_ext.abb)
    dT.ab -= ccpy_einsum("mnif,afbmnj->abij", H0.ab.ooov + H.ab.ooov, T_ext.abb)
    dT.ab += 0.5 * ccpy_einsum("anef,efbinj->abij", H0.aa.vovv + H.aa.vovv, T_ext.aab)
    dT.ab += ccpy_einsum("anef,efbinj->abij", H0.ab.vovv + H.ab.vovv, T_ext.abb)
    dT.ab += ccpy_einsum("nbfe,afeinj->abij", H0.ab.ovvv + H.ab.ovvv, T_ext.aab)
    dT.ab += 0.5 * ccpy_einsum("bnef,afeinj->abij", H0.bb.vovv + H.bb.vovv, T_ext.abb)
    dT.ab += ccpy_einsum("me,aebimj->abij", H.a.ov, T_ext.aab)
    dT.ab += ccpy_einsum("me,aebimj->abij", H.b.ov, T_ext.abb)
...
def update_t2c(T, dT, H, H0, VT_ext, shift, T_ext):
    ...
    # T3 parts
    dT.bb += 0.25 * ccpy_einsum("me,eabmij->abij", H.a.ov, T_ext.abb)
    dT.bb += 0.25 * ccpy_einsum("me,abeijm->abij", H.b.ov, T_ext.bbb)
    dT.bb += 0.25 * ccpy_einsum("anef,ebfijn->abij", H0.bb.vovv + H.bb.vovv, T_ext.bbb)
    dT.bb += 0.5 * ccpy_einsum("nafe,febnij->abij", H0.ab.ovvv + H.ab.ovvv, T_ext.abb)
    dT.bb -= 0.25 * ccpy_einsum("mnif,abfmjn->abij", H0.bb.ooov + H.bb.ooov, T_ext.bbb)
    dT.bb -= 0.5 * ccpy_einsum("nmfi,fabnmj->abij", H0.ab.oovo + H.ab.oovo, T_ext.abb)
