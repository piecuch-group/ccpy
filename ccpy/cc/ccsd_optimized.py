...
import cc_loops2
import numpy as np
from HBar_module import get_ccs_intermediates
from solvers import solve_cc_jacobi
...
def update_t1a(cc_t, ints, sys, shift):
    ...
    h1A_ov = (
        ints["fA"]["ov"]
        + np.einsum("mnef,fn->me", ints["vA"]["oovv"], cc_t["t1a"], optimize=True)
        + np.einsum("mnef,fn->me", ints["vB"]["oovv"], cc_t["t1b"], optimize=True)
    )
    h1B_ov = (
        ints["fB"]["ov"]
        + np.einsum("mnef,fn->me", ints["vC"]["oovv"], cc_t["t1b"], optimize=True)
        + np.einsum("nmfe,fn->me", ints["vB"]["oovv"], cc_t["t1a"], optimize=True)
    )
    h1A_oo = (
        ints["fA"]["oo"]
        + np.einsum("mnif,fn->mi", ints["vA"]["ooov"], cc_t["t1a"], optimize=True)
        + np.einsum("mnif,fn->mi", ints["vB"]["ooov"], cc_t["t1b"], optimize=True)
        + 0.5
        * np.einsum("mnef,efin->mi", ints["vA"]["oovv"], cc_t["t2a"], optimize=True)
        + np.einsum("mnef,efin->mi", ints["vB"]["oovv"], cc_t["t2b"], optimize=True)
    )
    h1A_vv = (
        ints["fA"]["vv"]
        + np.einsum("anef,fn->ae", ints["vA"]["vovv"], cc_t["t1a"], optimize=True)
        + np.einsum("anef,fn->ae", ints["vB"]["vovv"], cc_t["t1b"], optimize=True)
        - 0.5
        * np.einsum("mnef,afmn->ae", ints["vA"]["oovv"], cc_t["t2a"], optimize=True)
    ...
    X1A = (
        ints["fA"]["vo"]
        + np.einsum("me,aeim->ai", h1A_ov, cc_t["t2a"], optimize=True)
        ...
        - 0.5
        * np.einsum("mnif,afmn->ai", ints["vA"]["ooov"], cc_t["t2a"], optimize=True)
        - np.einsum("mnif,afmn->ai", ints["vB"]["ooov"], cc_t["t2b"], optimize=True)
        + 0.5
        * np.einsum("anef,efin->ai", ints["vA"]["vovv"], cc_t["t2a"], optimize=True)
...
def update_t1b(cc_t, ints, sys, shift):
    ...
    h1B_oo = (
        ints["fB"]["oo"]
        + np.einsum("mnif,fn->mi", ints["vC"]["ooov"], cc_t["t1b"], optimize=True)
        + np.einsum("nmfi,fn->mi", ints["vB"]["oovo"], cc_t["t1a"], optimize=True)
        + 0.5
        * np.einsum("mnef,efin->mi", ints["vC"]["oovv"], cc_t["t2c"], optimize=True)
        + np.einsum("nmfe,feni->mi", ints["vB"]["oovv"], cc_t["t2b"], optimize=True)
    )
    h1B_vv = (
        ints["fB"]["vv"]
        + np.einsum("anef,fn->ae", ints["vC"]["vovv"], cc_t["t1b"], optimize=True)
        + np.einsum("nafe,fn->ae", ints["vB"]["ovvv"], cc_t["t1a"], optimize=True)
        - 0.5
        * np.einsum("mnef,afmn->ae", ints["vC"]["oovv"], cc_t["t2c"], optimize=True)
    ...
    X1B = (
        ints["fB"]["vo"]
        + np.einsum("me,aeim->ai", h1B_ov, cc_t["t2c"], optimize=True)
        ...
        - 0.5
        * np.einsum("mnif,afmn->ai", ints["vC"]["ooov"], cc_t["t2c"], optimize=True)
        - np.einsum("nmfi,fanm->ai", ints["vB"]["oovo"], cc_t["t2b"], optimize=True)
        + 0.5
        * np.einsum("anef,efin->ai", ints["vC"]["vovv"], cc_t["t2c"], optimize=True)
