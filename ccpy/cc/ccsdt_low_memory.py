...
def ccsdt(
    sys, ints, work_dir, maxit=100, tol=1e-08, diis_size=6, shift=0.0, flag_RHF=False
):
    ...
    while it_micro < maxit:
        ...
        cc_t["t2a"] = np.reshape(
            T[idx_2a], (sys["Nunocc_a"], sys["Nunocc_a"], sys["Nocc_a"], sys["Nocc_a"])
        )
        cc_t["t2b"] = np.reshape(
            T[idx_2b], (sys["Nunocc_a"], sys["Nunocc_b"], sys["Nocc_a"], sys["Nocc_b"])
        )
        cc_t["t2c"] = np.reshape(
            T[idx_2c], (sys["Nunocc_b"], sys["Nunocc_b"], sys["Nocc_b"], sys["Nocc_b"])
        )
        cc_t["t3a"] = np.reshape(
            T[idx_3a],
            (
        ...
        # update T2
        cc_t, T_resid[idx_2a] = update_t2a(
            cc_t, ints, H1A, H1B, H2A, H2B, H2C, sys, shift
        )
        cc_t, T_resid[idx_2b] = update_t2b(
            cc_t, ints, H1A, H1B, H2A, H2B, H2C, sys, shift
        )
        if flag_RHF:
            cc_t["t2c"] = cc_t["t2a"]
            ...
            cc_t, T_resid[idx_2c] = update_t2c(
                cc_t, ints, H1A, H1B, H2A, H2B, H2C, sys, shift
            )
        # CCSD intermediates
        H1A, H1B, H2A, H2B, H2C = get_ccsd_intermediates(cc_t, ints, sys)
        # update T3
        cc_t, T_resid[idx_3a] = update_t3a(
            cc_t, ints, H1A, H1B, H2A, H2B, H2C, sys, shift
        )
        cc_t, T_resid[idx_3b] = update_t3b(
            cc_t, ints, H1A, H1B, H2A, H2B, H2C, sys, shift
        )
        if flag_RHF:
            ...
            cc_t, T_resid[idx_3c] = update_t3c(
                cc_t, ints, H1A, H1B, H2A, H2B, H2C, sys, shift
            )
            cc_t, T_resid[idx_3d] = update_t3d(
                cc_t, ints, H1A, H1B, H2A, H2B, H2C, sys, shift
            )
    ...
    if flag_conv:
        print(
            "CCSDT successfully converged! ({:0.2f}m  {:0.2f}s)".format(
                minutes, seconds
            )
        )
        print("")
...
def update_t2b(cc_t, ints, H1A, H1B, H2A, H2B, H2C, sys, shift):
    ...
    t2b, flatten = cc_loops2.cc_loops2.update_t2b(
    ...
    for i in range(sys["Nocc_a"]):
        for j in range(i + 1, sys["Nocc_a"]):
            for k in range(j + 1, sys["Nocc_a"]):
                ...
                X3A = -0.5 * np.einsum(
                    "am,bcm->abc", I2A_vooo[:, :, i, j], t2a[:, :, :, k], optimize=True
                )
                X3A += 0.5 * np.einsum(
                    "am,bcm->abc", I2A_vooo[:, :, k, j], t2a[:, :, :, i], optimize=True
                )
                X3A += 0.5 * np.einsum(
                    "am,bcm->abc", I2A_vooo[:, :, i, k], t2a[:, :, :, j], optimize=True
                )

                X3A += 0.5 * np.einsum(
                    "abe,ec->abc", I2A_vvov[:, :, i, :], t2a[:, :, j, k], optimize=True
                )
                X3A -= 0.5 * np.einsum(
                    "abe,ec->abc", I2A_vvov[:, :, j, :], t2a[:, :, i, k], optimize=True
                )
                X3A -= 0.5 * np.einsum(
                    "abe,ec->abc", I2A_vvov[:, :, k, :], t2a[:, :, j, i], optimize=True
                )

                X3A -= (1.0 / 6.0) * np.einsum(
                    "m,abcm->abc", H1A["oo"][:, k], t3a[:, :, :, i, j, :], optimize=True
                )
                X3A += (1.0 / 6.0) * np.einsum(
                    "m,abcm->abc", H1A["oo"][:, i], t3a[:, :, :, k, j, :], optimize=True
                )
                X3A += (1.0 / 6.0) * np.einsum(
                    "m,abcm->abc", H1A["oo"][:, j], t3a[:, :, :, i, k, :], optimize=True
                )

                X3A += 0.5 * np.einsum(
                    "ce,abe->abc", H1A["vv"], t3a[:, :, :, i, j, k], optimize=True
                )
                ...
                X3A += 0.25 * np.einsum(
                    "abef,efc->abc", H2A["vvvv"], t3a[:, :, :, i, j, k], optimize=True
                )
    ...
    for i in range(sys["Nocc_a"]):
        for j in range(i + 1, sys["Nocc_a"]):
            for k in range(sys["Nocc_b"]):
                X3B = np.einsum(
                    "bce,ae->abc", I2B_vvvo[:, :, :, k], t2a[:, :, i, j], optimize=True
                )

                X3B -= 0.5 * np.einsum(
                    "mc,abm->abc", I2B_ovoo[:, :, j, k], t2a[:, :, i, :], optimize=True
                )
                X3B += 0.5 * np.einsum(
                    "mc,abm->abc", I2B_ovoo[:, :, i, k], t2a[:, :, j, :], optimize=True
                )

                X3B += np.einsum(
                    "ace,be->abc", I2B_vvov[:, :, i, :], t2b[:, :, j, k], optimize=True
                )
                X3B -= np.einsum(
                    "ace,be->abc", I2B_vvov[:, :, j, :], t2b[:, :, i, k], optimize=True
                )

                X3B -= np.einsum(
                    "am,bcm->abc", I2B_vooo[:, :, i, k], t2b[:, :, j, :], optimize=True
                )
                X3B += np.einsum(
                    "am,bcm->abc", I2B_vooo[:, :, j, k], t2b[:, :, i, :], optimize=True
                )

                X3B += 0.5 * np.einsum(
                    "abe,ec->abc", I2A_vvov[:, :, i, :], t2b[:, :, j, k], optimize=True
                )
                X3B -= 0.5 * np.einsum(
                    "abe,ec->abc", I2A_vvov[:, :, j, :], t2b[:, :, i, k], optimize=True
                )

                X3B -= np.einsum(
                    "am,bcm->abc", I2A_vooo[:, :, i, j], t2b[:, :, :, k], optimize=True
                )

                # (HBar*T3)_C
                X3B -= 0.5 * np.einsum(
                    "m,abcm->abc", H1A["oo"][:, i], t3b[:, :, :, :, j, k], optimize=True
                )
                X3B += 0.5 * np.einsum(
                    "m,abcm->abc", H1A["oo"][:, j], t3b[:, :, :, :, i, k], optimize=True
                )

                X3B -= 0.5 * np.einsum(
                    "m,abcm->abc", H1B["oo"][:, k], t3b[:, :, :, i, j, :], optimize=True
                )

                X3B += np.einsum(
                    "ae,ebc->abc", H1A["vv"], t3b[:, :, :, i, j, k], optimize=True
                )

                X3B += 0.5 * np.einsum(
                    "ce,abe->abc", H1B["vv"], t3b[:, :, :, i, j, k], optimize=True
                )
                ...
                X3B += 0.25 * np.einsum(
                    "abef,efc->abc", H2A["vvvv"], t3b[:, :, :, i, j, k], optimize=True
                )

                X3B += np.einsum(
                    "bcef,aef->abc", H2B["vvvv"], t3b[:, :, :, i, j, k], optimize=True
                )
    ...
    for i in range(sys["Nocc_a"]):
        for j in range(sys["Nocc_b"]):
            for k in range(j + 1, sys["Nocc_b"]):
                # MM(2,3)C
                X3C = np.einsum(
                    "abe,ec->abc", I2B_vvov[:, :, i, :], t2c[:, :, j, k], optimize=True
                )

                X3C -= 0.5 * np.einsum(
                    "am,bcm->abc", I2B_vooo[:, :, i, j], t2c[:, :, :, k], optimize=True
                )
                X3C += 0.5 * np.einsum(
                    "am,bcm->abc", I2B_vooo[:, :, i, k], t2c[:, :, :, j], optimize=True
                )

                X3C += 0.5 * np.einsum(
                    "cbe,ae->abc", I2C_vvov[:, :, k, :], t2b[:, :, i, j], optimize=True
                )
                X3C -= 0.5 * np.einsum(
                    "cbe,ae->abc", I2C_vvov[:, :, j, :], t2b[:, :, i, k], optimize=True
                )

                X3C -= np.einsum(
                    "cm,abm->abc", I2C_vooo[:, :, k, j], t2b[:, :, i, :], optimize=True
                )

                X3C += np.einsum(
                    "abe,ec->abc", I2B_vvvo[:, :, :, j], t2b[:, :, i, k], optimize=True
                )
                X3C -= np.einsum(
                    "abe,ec->abc", I2B_vvvo[:, :, :, k], t2b[:, :, i, j], optimize=True
                )

                X3C -= np.einsum(
                    "mb,acm->abc", I2B_ovoo[:, :, i, j], t2b[:, :, :, k], optimize=True
                )
                X3C += np.einsum(
                    "mb,acm->abc", I2B_ovoo[:, :, i, k], t2b[:, :, :, j], optimize=True
                )

                # (HBar*T3)_C
                X3C -= 0.5 * np.einsum(
                    "m,abcm->abc", H1A["oo"][:, i], t3c[:, :, :, :, j, k], optimize=True
                )

                X3C -= 0.5 * np.einsum(
                    "m,abcm->abc", H1B["oo"][:, j], t3c[:, :, :, i, :, k], optimize=True
                )
                X3C += 0.5 * np.einsum(
                    "m,abcm->abc", H1B["oo"][:, k], t3c[:, :, :, i, :, j], optimize=True
                )

                X3C += 0.5 * np.einsum(
                    "ae,ebc->abc", H1A["vv"], t3c[:, :, :, i, j, k], optimize=True
                )

                X3C += np.einsum(
                    "be,aec->abc", H1B["vv"], t3c[:, :, :, i, j, k], optimize=True
                )
                ...
                X3C += 0.25 * np.einsum(
                    "bcef,aef->abc", H2C["vvvv"], t3c[:, :, :, i, j, k], optimize=True
                )

                X3C += np.einsum(
                    "abef,efc->abc", H2B["vvvv"], t3c[:, :, :, i, j, k], optimize=True
                )
    ...
    for i in range(sys["Nocc_b"]):
        for j in range(i + 1, sys["Nocc_b"]):
            for k in range(j + 1, sys["Nocc_b"]):
                # MM(2,3)D
                X3D = -0.5 * np.einsum(
                    "am,bcm->abc", I2C_vooo[:, :, i, j], t2c[:, :, :, k], optimize=True
                )
                X3D += 0.5 * np.einsum(
                    "am,bcm->abc", I2C_vooo[:, :, k, j], t2c[:, :, :, i], optimize=True
                )
                X3D += 0.5 * np.einsum(
                    "am,bcm->abc", I2C_vooo[:, :, i, k], t2c[:, :, :, j], optimize=True
                )

                X3D += 0.5 * np.einsum(
                    "abe,ec->abc", I2C_vvov[:, :, i, :], t2c[:, :, j, k], optimize=True
                )
                X3D -= 0.5 * np.einsum(
                    "abe,ec->abc", I2C_vvov[:, :, j, :], t2c[:, :, i, k], optimize=True
                )
                X3D -= 0.5 * np.einsum(
                    "abe,ec->abc", I2C_vvov[:, :, k, :], t2c[:, :, j, i], optimize=True
                )

                # (HBar*T3)_C
                X3D -= (1.0 / 6.0) * np.einsum(
                    "m,abcm->abc", H1B["oo"][:, k], t3d[:, :, :, i, j, :], optimize=True
                )
                X3D += (1.0 / 6.0) * np.einsum(
                    "m,abcm->abc", H1B["oo"][:, i], t3d[:, :, :, k, j, :], optimize=True
                )
                X3D += (1.0 / 6.0) * np.einsum(
                    "m,abcm->abc", H1B["oo"][:, j], t3d[:, :, :, i, k, :], optimize=True
                )

                X3D += 0.5 * np.einsum(
                    "ce,abe->abc", H1B["vv"], t3d[:, :, :, i, j, k], optimize=True
                )
                ...
                X3D += 0.25 * np.einsum(
                    "abef,efc->abc", H2C["vvvv"], t3d[:, :, :, i, j, k], optimize=True
                )
...
def get_ccs_intermediates(cc_t, ints, sys):
    ...
    h2A_vooo -= 0.5 * np.einsum(
        "nmef,fj,an,ei->amij", vA["oovv"], t1a, t1a, t1a, optimize=True
    )
    ...
    h2B_vooo -= np.einsum(
        "nmef,an,ei,fj->amij", vB["oovv"], t1a, t1a, t1b, optimize=True
    )
    ...
    h2C_ovoo -= 0.5 * np.einsum(
        "mnef,fj,ei,bn->mbij", vC["oovv"], t1b, t1b, t1b, optimize=True
    )
...
def get_ccsd_intermediates(cc_t, ints, sys):
    ...
    Q1 = +np.einsum("mnjf,afin->amij", h2A_ooov, t2a, optimize=True) + np.einsum(
        "mnjf,afin->amij", h2B_ooov, t2b, optimize=True
    )
    ...
    Q1 = +np.einsum("mnjf,afin->amij", h2C_ooov, t2c, optimize=True) + np.einsum(
        "nmfj,fani->amij", h2B_oovo, t2b, optimize=True
    )
    ...
    Q1 = +np.einsum("bnef,afin->abie", h2A_vovv, t2a, optimize=True) + np.einsum(
        "bnef,afin->abie", h2B_vovv, t2b, optimize=True
    )
    ...
    Q1 = +np.einsum("bnef,afin->abie", h2C_vovv, t2c, optimize=True) + np.einsum(
        "nbfe,fani->abie", h2B_ovvv, t2b, optimize=True
    )
