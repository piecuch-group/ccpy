...
def t4_aaaa_ijkl(i, j, k, l, HT1, T):
    ...
    # (1)
    x4a = -0.5 * ccpy_einsum("ame,bcm,ed->abcd", HT1["aa"]["voov"][:, :, i, :], T.aa[:, :, :, k], T.aa[:, :, j, l]) # (1)
    x4a += 0.5 * ccpy_einsum("ame,bcm,ed->abcd", HT1["aa"]["voov"][:, :, j, :], T.aa[:, :, :, k], T.aa[:, :, i, l]) # (ij)
    x4a += 0.5 * ccpy_einsum("ame,bcm,ed->abcd", HT1["aa"]["voov"][:, :, l, :], T.aa[:, :, :, k], T.aa[:, :, j, i]) # (il)
    # (ik)
    x4a += 0.5 * ccpy_einsum("ame,bcm,ed->abcd", HT1["aa"]["voov"][:, :, k, :], T.aa[:, :, :, i], T.aa[:, :, j, l]) # (ik)
    x4a -= 0.5 * ccpy_einsum("ame,bcm,ed->abcd", HT1["aa"]["voov"][:, :, j, :], T.aa[:, :, :, i], T.aa[:, :, k, l]) # (ij)(ik)
    x4a -= 0.5 * ccpy_einsum("ame,bcm,ed->abcd", HT1["aa"]["voov"][:, :, l, :], T.aa[:, :, :, i], T.aa[:, :, j, k]) # (il)(ik)
    # (jk)
    x4a += 0.5 * ccpy_einsum("ame,bcm,ed->abcd", HT1["aa"]["voov"][:, :, i, :], T.aa[:, :, :, j], T.aa[:, :, k, l]) # (jk)
    x4a -= 0.5 * ccpy_einsum("ame,bcm,ed->abcd", HT1["aa"]["voov"][:, :, k, :], T.aa[:, :, :, j], T.aa[:, :, i, l]) # (ij)(jk)
    x4a -= 0.5 * ccpy_einsum("ame,bcm,ed->abcd", HT1["aa"]["voov"][:, :, l, :], T.aa[:, :, :, j], T.aa[:, :, k, i]) # (il)(jk)
    # (kl)
    x4a += 0.5 * ccpy_einsum("ame,bcm,ed->abcd", HT1["aa"]["voov"][:, :, i, :], T.aa[:, :, :, l], T.aa[:, :, j, k]) # (kl)
    x4a -= 0.5 * ccpy_einsum("ame,bcm,ed->abcd", HT1["aa"]["voov"][:, :, j, :], T.aa[:, :, :, l], T.aa[:, :, i, k]) # (ij)(kl)
    x4a -= 0.5 * ccpy_einsum("ame,bcm,ed->abcd", HT1["aa"]["voov"][:, :, k, :], T.aa[:, :, :, l], T.aa[:, :, j, i]) # (il)(kl)
    # Diagram 2: A(ij/kl)A(bc/ad) h2a(mnij) * t2a(adml) * t2a(bcnk) [checked]
    x4a += 0.25 * ccpy_einsum("mn,adm,bcn->abcd", HT1["aa"]["oooo"][:, :, i, j], T.aa[:, :, :, l], T.aa[:, :, :, k])  # (1)
    x4a -= 0.25 * ccpy_einsum("mn,adm,bcn->abcd", HT1["aa"]["oooo"][:, :, k, j], T.aa[:, :, :, l], T.aa[:, :, :, i])  # (ik)
    x4a -= 0.25 * ccpy_einsum("mn,adm,bcn->abcd", HT1["aa"]["oooo"][:, :, l, j], T.aa[:, :, :, i], T.aa[:, :, :, k])  # (il)
    x4a -= 0.25 * ccpy_einsum("mn,adm,bcn->abcd", HT1["aa"]["oooo"][:, :, i, k], T.aa[:, :, :, l], T.aa[:, :, :, j])  # (jk)
    x4a -= 0.25 * ccpy_einsum("mn,adm,bcn->abcd", HT1["aa"]["oooo"][:, :, i, l], T.aa[:, :, :, j], T.aa[:, :, :, k])  # (jl)
    x4a += 0.25 * ccpy_einsum("mn,adm,bcn->abcd", HT1["aa"]["oooo"][:, :, k, l], T.aa[:, :, :, j], T.aa[:, :, :, i])  # (ik)(jl)
    # Diagram 3: A(jk/il)A(ab/cd) h2a(abef) * t2a(fcjk) * t2a(edil) [checked]
    x4a += 0.25 * ccpy_einsum("abef,fc,ed->abcd", HT1["aa"]["vvvv"], T.aa[:, :, j, k], T.aa[:, :, i, l])  # (1)
    x4a -= 0.25 * ccpy_einsum("abef,fc,ed->abcd", HT1["aa"]["vvvv"], T.aa[:, :, i, k], T.aa[:, :, j, l])  # (ij)
    x4a -= 0.25 * ccpy_einsum("abef,fc,ed->abcd", HT1["aa"]["vvvv"], T.aa[:, :, l, k], T.aa[:, :, i, j])  # (jl)
    x4a -= 0.25 * ccpy_einsum("abef,fc,ed->abcd", HT1["aa"]["vvvv"], T.aa[:, :, j, i], T.aa[:, :, k, l])  # (ik)
    x4a -= 0.25 * ccpy_einsum("abef,fc,ed->abcd", HT1["aa"]["vvvv"], T.aa[:, :, j, l], T.aa[:, :, i, k])  # (kl)
    x4a += 0.25 * ccpy_einsum("abef,fc,ed->abcd", HT1["aa"]["vvvv"], T.aa[:, :, i, l], T.aa[:, :, j, k])  # (ij)(kl)
    # Diagram 4: A(ab/cd)A(k/ijl) h2a(cdke) * t3a(abeijl), weight = 6/24 = (1.0 / 4.0) [checked]
    x4a += (1.0 / 4.0) * ccpy_einsum("cde,abe->abcd", HT1["aa"]["vvov"][:, :, k, :], T.aaa[:, :, :, i, j, l]) # (1)
    x4a -= (1.0 / 4.0) * ccpy_einsum("cde,abe->abcd", HT1["aa"]["vvov"][:, :, i, :], T.aaa[:, :, :, k, j, l]) # (ik)
    x4a -= (1.0 / 4.0) * ccpy_einsum("cde,abe->abcd", HT1["aa"]["vvov"][:, :, j, :], T.aaa[:, :, :, i, k, l]) # (jk)
    x4a -= (1.0 / 4.0) * ccpy_einsum("cde,abe->abcd", HT1["aa"]["vvov"][:, :, l, :], T.aaa[:, :, :, i, j, k]) # (kl)
    # Diagram 5: -A(c/abd)A(kl/ij) h2a(cmkl) * t3a(abdijm), weight = 4/24 = (1.0 / 6.0) [checked]
    x4a -= (1.0 / 6.0) * ccpy_einsum("cm,abdm->abcd", HT1["aa"]["vooo"][:, :, k, l], T.aaa[:, :, :, i, j, :]) # (1)
    x4a += (1.0 / 6.0) * ccpy_einsum("cm,abdm->abcd", HT1["aa"]["vooo"][:, :, i, l], T.aaa[:, :, :, k, j, :]) # (ik)
    x4a += (1.0 / 6.0) * ccpy_einsum("cm,abdm->abcd", HT1["aa"]["vooo"][:, :, j, l], T.aaa[:, :, :, i, k, :]) # (jk)
    x4a += (1.0 / 6.0) * ccpy_einsum("cm,abdm->abcd", HT1["aa"]["vooo"][:, :, k, i], T.aaa[:, :, :, l, j, :]) # (il)
    x4a += (1.0 / 6.0) * ccpy_einsum("cm,abdm->abcd", HT1["aa"]["vooo"][:, :, k, j], T.aaa[:, :, :, i, l, :]) # (jl)
    x4a -= (1.0 / 6.0) * ccpy_einsum("cm,abdm->abcd", HT1["aa"]["vooo"][:, :, i, j], T.aaa[:, :, :, k, l, :]) # (ik)(jl)
...
def t4_aaab_ijkl(i, j, k, l, HT1, T):
    ...
    x4b = -0.5 * ccpy_einsum("mde,abm,ec->abcd", HT1["ab"]["ovvo"][:, :, :, l], T.aa[:, :, i, :], T.aa[:, :, j, k]) # (1)
    x4b += 0.5 * ccpy_einsum("mde,abm,ec->abcd", HT1["ab"]["ovvo"][:, :, :, l], T.aa[:, :, j, :], T.aa[:, :, i, k]) # (ij)
    x4b += 0.5 * ccpy_einsum("mde,abm,ec->abcd", HT1["ab"]["ovvo"][:, :, :, l], T.aa[:, :, k, :], T.aa[:, :, j, i]) # (ik)
    # Diagram 2:   A(k/ij)A(a/bc) h2a(mnij) * t2a(bcnk) * t2b(adml)
    x4b += 0.5 * ccpy_einsum("mn,bcn,adm->abcd", HT1["aa"]["oooo"][:, :, i, j], T.aa[:, :, :, k], T.ab[:, :, :, l]) # (1)
    x4b -= 0.5 * ccpy_einsum("mn,bcn,adm->abcd", HT1["aa"]["oooo"][:, :, k, j], T.aa[:, :, :, i], T.ab[:, :, :, l]) # (ik)
    x4b -= 0.5 * ccpy_einsum("mn,bcn,adm->abcd", HT1["aa"]["oooo"][:, :, i, k], T.aa[:, :, :, j], T.ab[:, :, :, l]) # (jk)
    # Diagram 3:  -A(ijk)A(c/ab) h2b(mdjf) * t2a(abim) * t2b(cfkl)
    x4b -= 0.5 * ccpy_einsum("mdf,abm,cf->abcd", HT1["ab"]["ovov"][:, :, j, :], T.aa[:, :, i, :], T.ab[:, :, k, l]) # (1)
    x4b += 0.5 * ccpy_einsum("mdf,abm,cf->abcd", HT1["ab"]["ovov"][:, :, i, :], T.aa[:, :, j, :], T.ab[:, :, k, l]) # (ij)
    x4b += 0.5 * ccpy_einsum("mdf,abm,cf->abcd", HT1["ab"]["ovov"][:, :, j, :], T.aa[:, :, k, :], T.ab[:, :, i, l]) # (ik)
    x4b += 0.5 * ccpy_einsum("mdf,abm,cf->abcd", HT1["ab"]["ovov"][:, :, k, :], T.aa[:, :, i, :], T.ab[:, :, j, l]) # (jk)
    x4b -= 0.5 * ccpy_einsum("mdf,abm,cf->abcd", HT1["ab"]["ovov"][:, :, i, :], T.aa[:, :, k, :], T.ab[:, :, j, l]) # (ij)(jk)
    x4b -= 0.5 * ccpy_einsum("mdf,abm,cf->abcd", HT1["ab"]["ovov"][:, :, k, :], T.aa[:, :, j, :], T.ab[:, :, i, l]) # (ik)(jk)
    # Diagram 4:  -A(ijk)A(abc) h2b(amie) * t2b(bejl) * t2b(cdkm)
    x4b -= ccpy_einsum("ame,be,cdm->abcd", HT1["ab"]["voov"][:, :, i, :], T.ab[:, :, j, l], T.ab[:, :, k, :]) # (1)
    x4b += ccpy_einsum("ame,be,cdm->abcd", HT1["ab"]["voov"][:, :, j, :], T.ab[:, :, i, l], T.ab[:, :, k, :]) # (ij)
    x4b += ccpy_einsum("ame,be,cdm->abcd", HT1["ab"]["voov"][:, :, k, :], T.ab[:, :, j, l], T.ab[:, :, i, :]) # (ik)
    x4b += ccpy_einsum("ame,be,cdm->abcd", HT1["ab"]["voov"][:, :, i, :], T.ab[:, :, k, l], T.ab[:, :, j, :]) # (jk)
    x4b -= ccpy_einsum("ame,be,cdm->abcd", HT1["ab"]["voov"][:, :, k, :], T.ab[:, :, i, l], T.ab[:, :, j, :]) # (ij)(jk)
    x4b -= ccpy_einsum("ame,be,cdm->abcd", HT1["ab"]["voov"][:, :, j, :], T.ab[:, :, k, l], T.ab[:, :, i, :]) # (ik)(jk)
    # Diagram 5:   A(ijk)A(a/bc) h2b(mnjl) * t2a(bcmk) * t2b(adin)
    x4b += 0.5 * ccpy_einsum("mn,bcm,adn->abcd", HT1["ab"]["oooo"][:, :, j, l], T.aa[:, :, :, k], T.ab[:, :, i, :]) # (1)
    x4b -= 0.5 * ccpy_einsum("mn,bcm,adn->abcd", HT1["ab"]["oooo"][:, :, i, l], T.aa[:, :, :, k], T.ab[:, :, j, :]) # (ij)
    x4b -= 0.5 * ccpy_einsum("mn,bcm,adn->abcd", HT1["ab"]["oooo"][:, :, j, l], T.aa[:, :, :, i], T.ab[:, :, k, :]) # (ik)
    x4b -= 0.5 * ccpy_einsum("mn,bcm,adn->abcd", HT1["ab"]["oooo"][:, :, k, l], T.aa[:, :, :, j], T.ab[:, :, i, :]) # (jk)
    x4b += 0.5 * ccpy_einsum("mn,bcm,adn->abcd", HT1["ab"]["oooo"][:, :, i, l], T.aa[:, :, :, j], T.ab[:, :, k, :]) # (ij)(jk)
    x4b += 0.5 * ccpy_einsum("mn,bcm,adn->abcd", HT1["ab"]["oooo"][:, :, k, l], T.aa[:, :, :, i], T.ab[:, :, j, :]) # (ik)(jk)
    # Diagram 6:  -A(i/jk)A(abc) h2b(bmel) * t2a(ecjk) * t2b(adim)
    x4b -= ccpy_einsum("bme,ec,adm->abcd", HT1["ab"]["vovo"][:, :, :, l], T.aa[:, :, j, k], T.ab[:, :, i, :]) # (1)
    x4b += ccpy_einsum("bme,ec,adm->abcd", HT1["ab"]["vovo"][:, :, :, l], T.aa[:, :, i, k], T.ab[:, :, j, :]) # (ij)
    x4b += ccpy_einsum("bme,ec,adm->abcd", HT1["ab"]["vovo"][:, :, :, l], T.aa[:, :, j, i], T.ab[:, :, k, :]) # (ik)
    # Diagram 7:  -A(i/jk)A(abc) h2a(amie) * t2a(ecjk) * t2b(bdml)
    x4b -= ccpy_einsum("ame,ec,bdm->abcd", HT1["aa"]["voov"][:, :, i, :], T.aa[:, :, j, k], T.ab[:, :, :, l]) # (1)
    x4b += ccpy_einsum("ame,ec,bdm->abcd", HT1["aa"]["voov"][:, :, j, :], T.aa[:, :, i, k], T.ab[:, :, :, l]) # (ij)
    x4b += ccpy_einsum("ame,ec,bdm->abcd", HT1["aa"]["voov"][:, :, k, :], T.aa[:, :, j, i], T.ab[:, :, :, l]) # (ik)
    # Diagram 8:   A(i/jk)A(c/ab) h2a(abef) * t2a(fcjk) * t2b(edil)
    x4b += 0.5 * ccpy_einsum("abef,fc,ed->abcd", HT1["aa"]["vvvv"], T.aa[:, :, j, k], T.ab[:, :, i, l]) # (1)
    x4b -= 0.5 * ccpy_einsum("abef,fc,ed->abcd", HT1["aa"]["vvvv"], T.aa[:, :, i, k], T.ab[:, :, j, l]) # (ij)
    x4b -= 0.5 * ccpy_einsum("abef,fc,ed->abcd", HT1["aa"]["vvvv"], T.aa[:, :, j, i], T.ab[:, :, k, l]) # (ik)
    # Diagram 9:  -A(ijk)A(a/bc) h2a(amie) * t2a(bcmk) * t2b(edjl)
    x4b -= 0.5 * ccpy_einsum("ame,bcm,ed->abcd", HT1["aa"]["voov"][:, :, i, :], T.aa[:, :, :, k], T.ab[:, :, j, l]) # (1)
    x4b += 0.5 * ccpy_einsum("ame,bcm,ed->abcd", HT1["aa"]["voov"][:, :, j, :], T.aa[:, :, :, k], T.ab[:, :, i, l]) # (ij)
    x4b += 0.5 * ccpy_einsum("ame,bcm,ed->abcd", HT1["aa"]["voov"][:, :, k, :], T.aa[:, :, :, i], T.ab[:, :, j, l]) # (ik)
    x4b += 0.5 * ccpy_einsum("ame,bcm,ed->abcd", HT1["aa"]["voov"][:, :, i, :], T.aa[:, :, :, j], T.ab[:, :, k, l]) # (jk)
    x4b -= 0.5 * ccpy_einsum("ame,bcm,ed->abcd", HT1["aa"]["voov"][:, :, k, :], T.aa[:, :, :, j], T.ab[:, :, i, l]) # (ij)(jk)
    x4b -= 0.5 * ccpy_einsum("ame,bcm,ed->abcd", HT1["aa"]["voov"][:, :, j, :], T.aa[:, :, :, i], T.ab[:, :, k, l]) # (ik)(jk)
    # Diagram 10:  A(k/ij)A(abc) h2b(adef) * t2a(ebij) * t2b(cfkl)
    x4b += ccpy_einsum("adef,eb,cf->abcd", HT1["ab"]["vvvv"], T.aa[:, :, i, j], T.ab[:, :, k, l]) # (1)
    x4b -= ccpy_einsum("adef,eb,cf->abcd", HT1["ab"]["vvvv"], T.aa[:, :, k, j], T.ab[:, :, i, l]) # (ik)
    x4b -= ccpy_einsum("adef,eb,cf->abcd", HT1["ab"]["vvvv"], T.aa[:, :, i, k], T.ab[:, :, j, l]) # (jk)
    # Diagram 11: -A(k/ij) h2b(mdkl) * t3a(abcijm)
    x4b -= (1.0 / 6.0) * ccpy_einsum("md,abcm->abcd", HT1["ab"]["ovoo"][:, :, k, l], T.aaa[:, :, :, i, j, :]) # (1)
    x4b += (1.0 / 6.0) * ccpy_einsum("md,abcm->abcd", HT1["ab"]["ovoo"][:, :, i, l], T.aaa[:, :, :, k, j, :]) # (ik)
    x4b += (1.0 / 6.0) * ccpy_einsum("md,abcm->abcd", HT1["ab"]["ovoo"][:, :, j, l], T.aaa[:, :, :, i, k, :]) # (jk)
    # Diagram 12: -A(j/ik)A(a/bc) h2a(amik) * t3b(bcdjml)
    x4b -= 0.5 * ccpy_einsum("am,bcdm->abcd", HT1["aa"]["vooo"][:, :, i, k], T.aab[:, :, :, j, :, l]) # (1)
    x4b += 0.5 * ccpy_einsum("am,bcdm->abcd", HT1["aa"]["vooo"][:, :, j, k], T.aab[:, :, :, i, :, l]) # (ij)
    x4b += 0.5 * ccpy_einsum("am,bcdm->abcd", HT1["aa"]["vooo"][:, :, i, j], T.aab[:, :, :, k, :, l]) # (jk)
    # Diagram 13: -A(a/bc)A(i/jk) h2b(amil) * t3b(bcdjkm)
    x4b -= 0.5 * ccpy_einsum("am,bcdm->abcd", HT1["ab"]["vooo"][:, :, i, l], T.aab[:, :, :, j, k, :]) # (1)
    x4b += 0.5 * ccpy_einsum("am,bcdm->abcd", HT1["ab"]["vooo"][:, :, j, l], T.aab[:, :, :, i, k, :]) # (ij)
    x4b += 0.5 * ccpy_einsum("am,bcdm->abcd", HT1["ab"]["vooo"][:, :, k, l], T.aab[:, :, :, j, i, :]) # (ik)
    # Diagram 14: A(c/ab) h2b(cdel) * t3a(abeijk)
    x4b += 0.5 * ccpy_einsum("cde,abe->abcd", HT1["ab"]["vvvo"][:, :, :, l], T.aaa[:, :, :, i, j, k]) # (1)
    # Diagram 15: A(b/ac)A(i/jk) h2a(acie) * t3b(bedjkl)
    x4b += 0.5 * ccpy_einsum("ace,bed->abcd", HT1["aa"]["vvov"][:, :, i, :], T.aab[:, :, :, j, k, l]) # (1)
    x4b -= 0.5 * ccpy_einsum("ace,bed->abcd", HT1["aa"]["vvov"][:, :, j, :], T.aab[:, :, :, i, k, l]) # (ij)
    x4b -= 0.5 * ccpy_einsum("ace,bed->abcd", HT1["aa"]["vvov"][:, :, k, :], T.aab[:, :, :, j, i, l]) # (ik)
    # Diagram 16: A(a/bc)A(i/jk) h2b(adie) * t3b(bcejkl)
    x4b += 0.5 * ccpy_einsum("ade,bce->abcd", HT1["ab"]["vvov"][:, :, i, :], T.aab[:, :, :, j, k, l]) # (1)
    x4b -= 0.5 * ccpy_einsum("ade,bce->abcd", HT1["ab"]["vvov"][:, :, j, :], T.aab[:, :, :, i, k, l]) # (ij)
    x4b -= 0.5 * ccpy_einsum("ade,bce->abcd", HT1["ab"]["vvov"][:, :, k, :], T.aab[:, :, :, j, i, l]) # (ik)
...
def t4_aabb_ijkl(i, j, k, l, HT1, T):
    ...
    x4c = -ccpy_einsum("cme,adm,be->abcd", HT1["bb"]["voov"][:, :, k, :], T.ab[:, :, i, :], T.ab[:, :, j, l]) # (1)
    x4c += ccpy_einsum("cme,adm,be->abcd", HT1["bb"]["voov"][:, :, k, :], T.ab[:, :, j, :], T.ab[:, :, i, l]) # (ij)
    x4c += ccpy_einsum("cme,adm,be->abcd", HT1["bb"]["voov"][:, :, l, :], T.ab[:, :, i, :], T.ab[:, :, j, k]) # (kl)
    x4c -= ccpy_einsum("cme,adm,be->abcd", HT1["bb"]["voov"][:, :, l, :], T.ab[:, :, j, :], T.ab[:, :, i, k]) # (ij)(kl)
    # Diagram 2:  -A(ij)A(kl)A(ab)A(cd) h2a(amie) * t2b(bcmk) * t2b(edjl)
    x4c -= ccpy_einsum("ame,bcm,ed->abcd", HT1["aa"]["voov"][:, :, i, :], T.ab[:, :, :, k], T.ab[:, :, j, l]) # (1)
    x4c += ccpy_einsum("ame,bcm,ed->abcd", HT1["aa"]["voov"][:, :, j, :], T.ab[:, :, :, k], T.ab[:, :, i, l]) # (ij)
    x4c += ccpy_einsum("ame,bcm,ed->abcd", HT1["aa"]["voov"][:, :, i, :], T.ab[:, :, :, l], T.ab[:, :, j, k]) # (kl)
    x4c -= ccpy_einsum("ame,bcm,ed->abcd", HT1["aa"]["voov"][:, :, j, :], T.ab[:, :, :, l], T.ab[:, :, i, k]) # (ij)(kl)
    # Diagram 3:  -A(kl)A(ab)A(cd) h2b(mcek) * t2a(aeij) * t2b(bdml)
    x4c -= ccpy_einsum("mce,ae,bdm->abcd", HT1["ab"]["ovvo"][:, :, :, k], T.aa[:, :, i, j], T.ab[:, :, :, l]) # (1)
    x4c += ccpy_einsum("mce,ae,bdm->abcd", HT1["ab"]["ovvo"][:, :, :, l], T.aa[:, :, i, j], T.ab[:, :, :, k]) # (2)
    # Diagram 4:  -A(ij)A(ab)A(cd) h2b(amie) * t2b(bdjm) * t2c(cekl)
    x4c -= ccpy_einsum("ame,bdm,ce->abcd", HT1["ab"]["voov"][:, :, i, :], T.ab[:, :, j, :], T.bb[:, :, k, l]) # (1)
    x4c += ccpy_einsum("ame,bdm,ce->abcd", HT1["ab"]["voov"][:, :, j, :], T.ab[:, :, i, :], T.bb[:, :, k, l]) # (ij)
    # Diagram 5:  -A(ij)A(kl)A(cd) h2b(mcek) * t2a(abim) * t2b(edjl)
    x4c -= 0.5 * ccpy_einsum("mce,abm,ed->abcd", HT1["ab"]["ovvo"][:, :, :, k], T.aa[:, :, i, :], T.ab[:, :, j, l]) # (1)
    x4c += 0.5 * ccpy_einsum("mce,abm,ed->abcd", HT1["ab"]["ovvo"][:, :, :, k], T.aa[:, :, j, :], T.ab[:, :, i, l]) # (ij)
    x4c += 0.5 * ccpy_einsum("mce,abm,ed->abcd", HT1["ab"]["ovvo"][:, :, :, l], T.aa[:, :, i, :], T.ab[:, :, j, k]) # (kl)
    x4c -= 0.5 * ccpy_einsum("mce,abm,ed->abcd", HT1["ab"]["ovvo"][:, :, :, l], T.aa[:, :, j, :], T.ab[:, :, i, k]) # (ij)(kl)
    # Diagram 6:  -A(ij)A(kl)A(ab) h2b(amie) * t2c(cdkm) * t2b(bejl)
    x4c -= 0.5 * ccpy_einsum("ame,cdm,be->abcd", HT1["ab"]["voov"][:, :, i, :], T.bb[:, :, k, :], T.ab[:, :, j, l]) # (1)
    x4c += 0.5 * ccpy_einsum("ame,cdm,be->abcd", HT1["ab"]["voov"][:, :, j, :], T.bb[:, :, k, :], T.ab[:, :, i, l]) # (ij)
    x4c += 0.5 * ccpy_einsum("ame,cdm,be->abcd", HT1["ab"]["voov"][:, :, i, :], T.bb[:, :, l, :], T.ab[:, :, j, k]) # (kl)
    x4c -= 0.5 * ccpy_einsum("ame,cdm,be->abcd", HT1["ab"]["voov"][:, :, j, :], T.bb[:, :, l, :], T.ab[:, :, i, k]) # (ij)(kl)
    # Diagram 7:  -A(ij)A(kl)A(ab)A(cd) h2b(bmel) * t2b(adim) * t2b(ecjk)
    x4c -= ccpy_einsum("bme,adm,ec->abcd", HT1["ab"]["vovo"][:, :, :, l], T.ab[:, :, i, :], T.ab[:, :, j, k]) # (1)
    x4c += ccpy_einsum("bme,adm,ec->abcd", HT1["ab"]["vovo"][:, :, :, l], T.ab[:, :, j, :], T.ab[:, :, i, k]) # (ij)
    x4c += ccpy_einsum("bme,adm,ec->abcd", HT1["ab"]["vovo"][:, :, :, k], T.ab[:, :, i, :], T.ab[:, :, j, l]) # (kl)
    x4c -= ccpy_einsum("bme,adm,ec->abcd", HT1["ab"]["vovo"][:, :, :, k], T.ab[:, :, j, :], T.ab[:, :, i, l]) # (ij)(kl)
    # Diagram 8:  -A(ij)A(kl)A(ab)A(cd) h2b(mdje) * t2b(bcmk) * t2b(aeil)
    x4c -= ccpy_einsum("mde,bcm,ae->abcd", HT1["ab"]["ovov"][:, :, j, :], T.ab[:, :, :, k], T.ab[:, :, i, l]) # (1)
    x4c += ccpy_einsum("mde,bcm,ae->abcd", HT1["ab"]["ovov"][:, :, i, :], T.ab[:, :, :, k], T.ab[:, :, j, l]) # (ij)
    x4c += ccpy_einsum("mde,bcm,ae->abcd", HT1["ab"]["ovov"][:, :, j, :], T.ab[:, :, :, l], T.ab[:, :, i, k]) # (kl)
    x4c -= ccpy_einsum("mde,bcm,ae->abcd", HT1["ab"]["ovov"][:, :, i, :], T.ab[:, :, :, l], T.ab[:, :, j, k]) # (ij)(kl)
    # Diagram 9:  -A(ij)A(cd) h2b(mdje) * t2a(abim) * t2c(cekl)
    x4c -= 0.5 * ccpy_einsum("mde,abm,ce->abcd", HT1["ab"]["ovov"][:, :, j, :], T.aa[:, :, i, :], T.bb[:, :, k, l]) # (1)
    x4c += 0.5 * ccpy_einsum("mde,abm,ce->abcd", HT1["ab"]["ovov"][:, :, i, :], T.aa[:, :, j, :], T.bb[:, :, k, l]) # (ij)
    # Diagram 10: -A(kl)A(ab) h2b(bmel) * t2a(aeij) * t2c(cdkm)
    x4c -= 0.5 * ccpy_einsum("bme,ae,cdm->abcd", HT1["ab"]["vovo"][:, :, :, l], T.aa[:, :, i, j], T.bb[:, :, k, :]) # (1)
    x4c += 0.5 * ccpy_einsum("bme,ae,cdm->abcd", HT1["ab"]["vovo"][:, :, :, k], T.aa[:, :, i, j], T.bb[:, :, l, :]) # (kl)
    # Diagram 11:  A(kl)A(ab) h2a(mnij) * t2b(acmk) * t2b(bdnl)
    x4c += 0.5 * ccpy_einsum("mn,acm,bdn->abcd", HT1["aa"]["oooo"][:, :, i, j], T.ab[:, :, :, k], T.ab[:, :, :, l]) # (1)
    x4c -= 0.5 * ccpy_einsum("mn,acm,bdn->abcd", HT1["aa"]["oooo"][:, :, i, j], T.ab[:, :, :, l], T.ab[:, :, :, k]) # (kl)
    # Diagram 12:  A(ij)A(kl) h2a(abef) * t2b(ecik) * t2b(fdjl)
    x4c += 0.25 * ccpy_einsum("abef,ec,fd->abcd", HT1["aa"]["vvvv"], T.ab[:, :, i, k], T.ab[:, :, j, l]) # (1)
    x4c -= 0.25 * ccpy_einsum("abef,ec,fd->abcd", HT1["aa"]["vvvv"], T.ab[:, :, j, k], T.ab[:, :, i, l]) # (ij)
    x4c -= 0.25 * ccpy_einsum("abef,ec,fd->abcd", HT1["aa"]["vvvv"], T.ab[:, :, i, l], T.ab[:, :, j, k]) # (kl)
    x4c += 0.25 * ccpy_einsum("abef,ec,fd->abcd", HT1["aa"]["vvvv"], T.ab[:, :, j, l], T.ab[:, :, i, k]) # (ij)(kl)
    # Diagram 13:  A(ij)A(kl) h2b(mnik) * t2a(abmj) * t2c(cdnl)
    x4c += 0.25 * ccpy_einsum("mn,abm,cdn->abcd", HT1["ab"]["oooo"][:, :, i, k], T.aa[:, :, :, j], T.bb[:, :, :, l]) # (1)
    x4c -= 0.25 * ccpy_einsum("mn,abm,cdn->abcd", HT1["ab"]["oooo"][:, :, j, k], T.aa[:, :, :, i], T.bb[:, :, :, l]) # (ij)
    x4c -= 0.25 * ccpy_einsum("mn,abm,cdn->abcd", HT1["ab"]["oooo"][:, :, i, l], T.aa[:, :, :, j], T.bb[:, :, :, k]) # (kl)
    x4c += 0.25 * ccpy_einsum("mn,abm,cdn->abcd", HT1["ab"]["oooo"][:, :, j, l], T.aa[:, :, :, i], T.bb[:, :, :, k]) # (ij)(kl)
    # Diagram 14:  A(ab)A(cd) h2b(acef) * t2a(ebij) * t2c(fdkl)
    x4c += ccpy_einsum("acef,eb,fd->abcd", HT1["ab"]["vvvv"], T.aa[:, :, i, j], T.bb[:, :, k, l]) # (1)
    # Diagram 15:  A(ij)A(kl)A(ab)A(cd) h2b(mnik) * t2b(adml) * t2b(bcjn)
    x4c += ccpy_einsum("mn,adm,bcn->abcd", HT1["ab"]["oooo"][:, :, i, k], T.ab[:, :, :, l], T.ab[:, :, j, :]) # (1)
    x4c -= ccpy_einsum("mn,adm,bcn->abcd", HT1["ab"]["oooo"][:, :, j, k], T.ab[:, :, :, l], T.ab[:, :, i, :]) # (ij)
    x4c -= ccpy_einsum("mn,adm,bcn->abcd", HT1["ab"]["oooo"][:, :, i, l], T.ab[:, :, :, k], T.ab[:, :, j, :]) # (kl)
    x4c += ccpy_einsum("mn,adm,bcn->abcd", HT1["ab"]["oooo"][:, :, j, l], T.ab[:, :, :, k], T.ab[:, :, i, :]) # (ij)(kl)
    # Diagram 16:  A(ij)A(kl)A(ab)A(cd) h2b(acef) * t2b(edil) * t2b(bfjk)
    x4c += ccpy_einsum("acef,ed,bf->abcd", HT1["ab"]["vvvv"], T.ab[:, :, i, l], T.ab[:, :, j, k]) # (1)
    x4c -= ccpy_einsum("acef,ed,bf->abcd", HT1["ab"]["vvvv"], T.ab[:, :, j, l], T.ab[:, :, i, k]) # (ij)
    x4c -= ccpy_einsum("acef,ed,bf->abcd", HT1["ab"]["vvvv"], T.ab[:, :, i, k], T.ab[:, :, j, l]) # (kl)
    x4c += ccpy_einsum("acef,ed,bf->abcd", HT1["ab"]["vvvv"], T.ab[:, :, j, k], T.ab[:, :, i, l]) # (ij)(kl)
    # Diagram 17:  A(ij)A(cd) h2c(mnkl) * t2b(adin) * t2b(bcjm)
    x4c += 0.5 * ccpy_einsum("mn,adn,bcm->abcd", HT1["bb"]["oooo"][:, :, k, l], T.ab[:, :, i, :], T.ab[:, :, j, :]) # (1)
    x4c -= 0.5 * ccpy_einsum("mn,adn,bcm->abcd", HT1["bb"]["oooo"][:, :, k, l], T.ab[:, :, j, :], T.ab[:, :, i, :]) # (ij)
    # Diagram 18:  A(ij)A(kl) h2c(cdef) * t2b(afil) * t2b(bejk)
    x4c += 0.25 * ccpy_einsum("cdef,af,be->abcd", HT1["bb"]["vvvv"], T.ab[:, :, i, l], T.ab[:, :, j, k]) # (1)
    x4c -= 0.25 * ccpy_einsum("cdef,af,be->abcd", HT1["bb"]["vvvv"], T.ab[:, :, j, l], T.ab[:, :, i, k]) # (ij)
    x4c -= 0.25 * ccpy_einsum("cdef,af,be->abcd", HT1["bb"]["vvvv"], T.ab[:, :, i, k], T.ab[:, :, j, l]) # (kl)
    x4c += 0.25 * ccpy_einsum("cdef,af,be->abcd", HT1["bb"]["vvvv"], T.ab[:, :, j, k], T.ab[:, :, i, l]) # (ij)(kl)
    # Diagram 19: -A(ij)A(kl)A(cd) h2b(mdil) * t3b(abcmjk)
    x4c -= 0.5 * ccpy_einsum("md,abcm->abcd", HT1["ab"]["ovoo"][:, :, i, l], T.aab[:, :, :, :, j, k]) # (1)
    x4c += 0.5 * ccpy_einsum("md,abcm->abcd", HT1["ab"]["ovoo"][:, :, j, l], T.aab[:, :, :, :, i, k]) # (ij)
    x4c += 0.5 * ccpy_einsum("md,abcm->abcd", HT1["ab"]["ovoo"][:, :, i, k], T.aab[:, :, :, :, j, l]) # (kl)
    x4c -= 0.5 * ccpy_einsum("md,abcm->abcd", HT1["ab"]["ovoo"][:, :, j, k], T.aab[:, :, :, :, i, l]) # (ij)(kl)
    # Diagram 20: -A(ab) h2a(bmji) * t3c(acdmkl)
    x4c -= 0.5 * ccpy_einsum("bm,acdm->abcd", HT1["aa"]["vooo"][:, :, j, i], T.abb[:, :, :, :, k, l]) # (1)
    # Diagram 21: -A(cd) h2c(cmkl) * t3b(abdijm)
    x4c -= 0.5 * ccpy_einsum("cm,abdm->abcd", HT1["bb"]["vooo"][:, :, k, l], T.aab[:, :, :, i, j, :]) # (1)
    # Diagram 22: -A(ij)A(ab)A(kl) h2b(amil) * t3c(bcdjkm)
    x4c -= 0.5 * ccpy_einsum("am,bcdm->abcd", HT1["ab"]["vooo"][:, :, i, l], T.abb[:, :, :, j, k, :]) # (1)
    x4c += 0.5 * ccpy_einsum("am,bcdm->abcd", HT1["ab"]["vooo"][:, :, j, l], T.abb[:, :, :, i, k, :]) # (ij)
    x4c += 0.5 * ccpy_einsum("am,bcdm->abcd", HT1["ab"]["vooo"][:, :, i, k], T.abb[:, :, :, j, l, :]) # (kl)
    x4c -= 0.5 * ccpy_einsum("am,bcdm->abcd", HT1["ab"]["vooo"][:, :, j, k], T.abb[:, :, :, i, l, :]) # (ij)(kl)
    # Diagram 23: A(ab)A(kl)A(cd) h2b(adel) * t3b(becjik)
    x4c += ccpy_einsum("ade,bec->abcd", HT1["ab"]["vvvo"][:, :, :, l], T.aab[:, :, :, j, i, k]) # (1)
    x4c -= ccpy_einsum("ade,bec->abcd", HT1["ab"]["vvvo"][:, :, :, k], T.aab[:, :, :, j, i, l]) # (kl)
    # Diagram 24: A(ij) h2a(baje) * t3c(ecdikl)
    x4c += 0.25 * ccpy_einsum("bae,ecd->abcd", HT1["aa"]["vvov"][:, :, j, :], T.abb[:, :, :, i, k, l]) # (1)
    x4c -= 0.25 * ccpy_einsum("bae,ecd->abcd", HT1["aa"]["vvov"][:, :, i, :], T.abb[:, :, :, j, k, l]) # (ij)
    # Diagram 25: A(ij)A(ab)A(cd) h2b(adie) * t3c(bcejkl)
    x4c += ccpy_einsum("ade,bce->abcd", HT1["ab"]["vvov"][:, :, i, :], T.abb[:, :, :, j, k, l]) # (1)
    x4c -= ccpy_einsum("ade,bce->abcd", HT1["ab"]["vvov"][:, :, j, :], T.abb[:, :, :, i, k, l]) # (ij)
    # Diagram 26: A(kl) h2c(cdke) * t3b(abeijl)
    x4c += 0.25 * ccpy_einsum("cde,abe->abcd", HT1["bb"]["vvov"][:, :, k, :], T.aab[:, :, :, i, j, l]) # (1)
    x4c -= 0.25 * ccpy_einsum("cde,abe->abcd", HT1["bb"]["vvov"][:, :, l, :], T.aab[:, :, :, i, j, k]) # (kl)
...
def add_t4_aaaa_contributions(T, dT, HT1, H0, X):
    ...
    # Exact expressions
    #
    # exact_aa = 0.25 * ccpy_einsum("mnef,abefijmn->abij", H0.aa.oovv, t4["aaaa"])
    # exact_aaa = (
    #                (1.0 / 24.0) * ccpy_einsum("cnef,abefijkn->abcijk", H0.aa.vovv, t4["aaaa"]) # (c/ab) = 3
    #                - (1.0 / 24.0) * ccpy_einsum("mnkf,abcfijmn->abcijk", H0.aa.ooov, t4["aaaa"]) # (k/ij) = 3
    # )
        ...
        for j in range(i + 1, noa):
            for k in range(j + 1, noa):
                for l in range(k + 1, noa):
                    ...
                    dT.aa[:, :, i, j] += 0.25 * ccpy_einsum("ef,abef->ab", H0.aa.oovv[k, l, :, :], t4a) # (1)
                    dT.aa[:, :, k, j] -= 0.25 * ccpy_einsum("ef,abef->ab", H0.aa.oovv[i, l, :, :], t4a) # (ik)
                    dT.aa[:, :, l, j] -= 0.25 * ccpy_einsum("ef,abef->ab", H0.aa.oovv[k, i, :, :], t4a) # (il)
                    dT.aa[:, :, i, k] -= 0.25 * ccpy_einsum("ef,abef->ab", H0.aa.oovv[j, l, :, :], t4a) # (jk)
                    dT.aa[:, :, i, l] -= 0.25 * ccpy_einsum("ef,abef->ab", H0.aa.oovv[k, j, :, :], t4a) # (jl)
                    dT.aa[:, :, k, l] += 0.25 * ccpy_einsum("ef,abef->ab", H0.aa.oovv[i, j, :, :], t4a) # (ik)(jl)
                    # A(l/ijk) x(abcijk) <- h2a(:lef)*t4a(abefijkl)
                    dT.aaa[:, :, :, i, j, k] += 0.25 * ccpy_einsum("cef,abef->abc", H0.aa.vovv[:, l, :, :] + X.aa.vovv[:, l, :, :], t4a) # (1)
                    dT.aaa[:, :, :, l, j, k] -= 0.25 * ccpy_einsum("cef,abef->abc", H0.aa.vovv[:, i, :, :] + X.aa.vovv[:, i, :, :], t4a) # (il)
                    dT.aaa[:, :, :, i, l, k] -= 0.25 * ccpy_einsum("cef,abef->abc", H0.aa.vovv[:, j, :, :] + X.aa.vovv[:, j, :, :], t4a) # (jl)
                    dT.aaa[:, :, :, i, j, l] -= 0.25 * ccpy_einsum("cef,abef->abc", H0.aa.vovv[:, k, :, :] + X.aa.vovv[:, k, :, :], t4a) # (kl)
                    # A(ij/kl) x(abcijk) <- -h2a(klmd)*t4a(abcdijkl)
                    dT.aaa[:, :, :, i, j, :] -= (1.0 / 6.0) * ccpy_einsum("md,abcd->abcm", H0.aa.ooov[k, l, :, :] + X.aa.ooov[k, l, :, :], t4a) # (1)
                    dT.aaa[:, :, :, k, j, :] += (1.0 / 6.0) * ccpy_einsum("md,abcd->abcm", H0.aa.ooov[i, l, :, :] + X.aa.ooov[i, l, :, :], t4a) # (ik)
                    dT.aaa[:, :, :, l, j, :] += (1.0 / 6.0) * ccpy_einsum("md,abcd->abcm", H0.aa.ooov[k, i, :, :] + X.aa.ooov[k, i, :, :], t4a) # (il)
                    dT.aaa[:, :, :, i, k, :] += (1.0 / 6.0) * ccpy_einsum("md,abcd->abcm", H0.aa.ooov[j, l, :, :] + X.aa.ooov[j, l, :, :], t4a) # (jk)
                    dT.aaa[:, :, :, i, l, :] += (1.0 / 6.0) * ccpy_einsum("md,abcd->abcm", H0.aa.ooov[k, j, :, :] + X.aa.ooov[k, j, :, :], t4a) # (jl)
                    dT.aaa[:, :, :, k, l, :] -= (1.0 / 6.0) * ccpy_einsum("md,abcd->abcm", H0.aa.ooov[i, j, :, :] + X.aa.ooov[i, j, :, :], t4a) # (ik)(jl)
    # antisymmetrize
...
def add_t4_aaab_contributions(T, dT, HT1, H0, X):
    ...
    # Exact expressions
    #
    # exact_aa = ccpy_einsum("mnef,abefijmn->abij", H0.ab.oovv, t4["aaab"])
    # exact_ab = (
    #             0.25 * ccpy_einsum("mnef,aefbimnj->abij", H0.aa.oovv, t4["aaab"])
    #             + 0.25 * ccpy_einsum("mnef,abefijmn->abij", H0.bb.oovv, t4["abbb"])
    # )
    # exact_aaa = (
    #             (1.0 / 12.0) * ccpy_einsum("cnef,abefijkn->abcijk", H0.ab.vovv, t4["aaab"])  # (c/ab) = 3
    #             - (1.0 / 12.0) * ccpy_einsum("mnkf,abcfijmn->abcijk", H0.ab.ooov, t4["aaab"])  # (k/ij) = 3
    # )
    # exact_aaa -= np.transpose(exact_aaa, (0, 1, 2, 3, 5, 4)) # (jk)
    # exact_aaa -= np.transpose(exact_aaa, (0, 1, 2, 4, 3, 5)) + np.transpose(exact_aaa, (0, 1, 2, 5, 4, 3)) # (i/jk)
    # exact_aaa -= np.transpose(exact_aaa, (0, 2, 1, 3, 4, 5)) # (bc)
    ...
    #
    # exact_aab = (
    #         - 0.25 * ccpy_einsum("mnjf,abfcimnk->abcijk", H0.aa.ooov, t4["aaab"])  # (ij) = 2
    #         - 0.25 * ccpy_einsum("nmfk,abfcijnm->abcijk", H0.ab.oovo, t4["aaab"])  # (1) = 1
    #         + 0.25 * ccpy_einsum("bnef,aefcijnk->abcijk", H0.aa.vovv, t4["aaab"])  # (ab) = 2
    #         + 0.25 * ccpy_einsum("ncfe,abfeijnk->abcijk", H0.ab.ovvv, t4["aaab"])  # (1) = 1
    # )
        ...
        for j in range(i + 1, noa):
            for k in range(j + 1, noa):
                for l in range(nob):
                    ...
                    dT.aa[:, :, i, j] += 0.5 * ccpy_einsum("ef,abef->ab", H0.ab.oovv[k, l, :, :], t4b) # (1)
                    dT.aa[:, :, k, j] -= 0.5 * ccpy_einsum("ef,abef->ab", H0.ab.oovv[i, l, :, :], t4b) # (ik)
                    dT.aa[:, :, i, k] -= 0.5 * ccpy_einsum("ef,abef->ab", H0.ab.oovv[j, l, :, :], t4b) # (jk)
                    # A(i/jk) h2a(jkef) * t4b(aefbijkl) -> x2b(abil)
                    dT.ab[:, :, i, l] += 0.5 * ccpy_einsum("ef,aefb->ab", H0.aa.oovv[j, k, :, :], t4b) # (1)
                    dT.ab[:, :, j, l] -= 0.5 * ccpy_einsum("ef,aefb->ab", H0.aa.oovv[i, k, :, :], t4b) # (1)
                    dT.ab[:, :, k, l] -= 0.5 * ccpy_einsum("ef,aefb->ab", H0.aa.oovv[j, i, :, :], t4b) # (1)
                    # h2b(clef) * t4b(abefijkl) -> x3a(abcijk)
                    dT.aaa[:, :, :, i, j, k] += 0.5 * ccpy_einsum("cef,abef->abc", H0.ab.vovv[:, l, :, :] + X.ab.vovv[:, l, :, :], t4b) # (1)
                    # A(k/ij) -h2b(klmf) * t4b(abcfijkl) -> x3a(abcijk)
                    dT.aaa[:, :, :, i, j, :] -= (1.0 / 6.0) * ccpy_einsum("mf,abcf->abcm", H0.ab.ooov[k, l, :, :] + X.ab.ooov[k, l, :, :], t4b) # (1)
                    dT.aaa[:, :, :, k, j, :] += (1.0 / 6.0) * ccpy_einsum("mf,abcf->abcm", H0.ab.ooov[i, l, :, :] + X.ab.ooov[i, l, :, :], t4b) # (ik)
                    dT.aaa[:, :, :, i, k, :] += (1.0 / 6.0) * ccpy_einsum("mf,abcf->abcm", H0.ab.ooov[j, l, :, :] + X.ab.ooov[j, l, :, :], t4b) # (jk)
                    # A(i/jk) -h2a(jkmf) * t4b(abfcijkl) -> x3b(abciml)
                    dT.aab[:, :, :, i, :, l] -= 0.5 * ccpy_einsum("mf,abfc->abcm", H0.aa.ooov[j, k, :, :] + X.aa.ooov[j, k, :, :], t4b) # (1)
                    dT.aab[:, :, :, j, :, l] += 0.5 * ccpy_einsum("mf,abfc->abcm", H0.aa.ooov[i, k, :, :] + X.aa.ooov[i, k, :, :], t4b) # (ij)
                    dT.aab[:, :, :, k, :, l] += 0.5 * ccpy_einsum("mf,abfc->abcm", H0.aa.ooov[j, i, :, :] + X.aa.ooov[j, i, :, :], t4b) # (ik)
                    # A(k/ij) -h2b(klfn) * t4b(abfcijkl) -> x3b(abcijn)
                    dT.aab[:, :, :, i, j, :] -= 0.5 * ccpy_einsum("fn,abfc->abcn", H0.ab.oovo[k, l, :, :] + X.ab.oovo[k, l, :, :], t4b) # (1)
                    dT.aab[:, :, :, k, j, :] += 0.5 * ccpy_einsum("fn,abfc->abcn", H0.ab.oovo[i, l, :, :] + X.ab.oovo[i, l, :, :], t4b) # (ik)
                    dT.aab[:, :, :, i, k, :] += 0.5 * ccpy_einsum("fn,abfc->abcn", H0.ab.oovo[j, l, :, :] + X.ab.oovo[j, l, :, :], t4b) # (jk)
                    # A(k/ij) h2a(bkef) * t4b(aefcijkl) -> x3b(abcijl)
                    dT.aab[:, :, :, i, j, l] += 0.5 * ccpy_einsum("bef,aefc->abc", H0.aa.vovv[:, k, :, :] + X.aa.vovv[:, k, :, :], t4b) # (1)
                    dT.aab[:, :, :, k, j, l] -= 0.5 * ccpy_einsum("bef,aefc->abc", H0.aa.vovv[:, i, :, :] + X.aa.vovv[:, i, :, :], t4b) # (ik)
                    dT.aab[:, :, :, i, k, l] -= 0.5 * ccpy_einsum("bef,aefc->abc", H0.aa.vovv[:, j, :, :] + X.aa.vovv[:, j, :, :], t4b) # (jk)
                    # A(k/ij) h2b(kcfe) * t4b(abfeijkl) -> x3b(abcijl)
                    dT.aab[:, :, :, i, j, l] += 0.5 * ccpy_einsum("cfe,abfe->abc", H0.ab.ovvv[k, :, :, :] + X.ab.ovvv[k, :, :, :], t4b) # (1)
                    dT.aab[:, :, :, k, j, l] -= 0.5 * ccpy_einsum("cfe,abfe->abc", H0.ab.ovvv[i, :, :, :] + X.ab.ovvv[i, :, :, :], t4b) # (ik)
                    dT.aab[:, :, :, i, k, l] -= 0.5 * ccpy_einsum("cfe,abfe->abc", H0.ab.ovvv[j, :, :, :] + X.ab.ovvv[j, :, :, :], t4b) # (jk)
...
def add_t4_aabb_contributions(T, dT, HT1, H0, X):
    ...
    # Exact expressions
    #
    # exact_aa = 0.25 * ccpy_einsum("mnef,abefijmn->abij", H0.bb.oovv, t4["aabb"])
    # exact_ab = (
    #     ccpy_einsum("mnef,aefbimnj->abij", H0.ab.oovv, t4["aabb"])
    # )
    #
    # exact_aab = (
    #         -0.5 * ccpy_einsum("mnjf,abfcimnk->abcijk", H0.ab.ooov, t4["aabb"]) # (ij) = 2
    #         -0.125 * ccpy_einsum("mnkf,abfcijnm->abcijk", H0.bb.ooov, t4["aabb"]) # (1) = 1
    #         +0.5 * ccpy_einsum("bnef,aefcijnk->abcijk", H0.ab.vovv, t4["aabb"]) # (ab) = 2
    #         +0.125 * ccpy_einsum("cnef,abfeijnk->abcijk", H0.bb.vovv, t4["aabb"]) # (1) = 1
    # )
    # exact_aab -= np.transpose(exact_aab, (1, 0, 2, 3, 4, 5)) # (ab)
    # exact_aab -= np.transpose(exact_aab, (0, 1, 2, 4, 3, 5)) # (ij)
    #
        ...
        for j in range(i + 1, noa):
            for k in range(nob):
                for l in range(k + 1, nob):
                    ...
                    dT.aa[:, :, i, j] += 0.25 * ccpy_einsum("ef,abef->ab", H0.bb.oovv[k, l, :, :], t4c) # (1)
                    # A(ij)A(kl) h2b(jkef) * t4c(aefbijkl) -> x2b(abil)
                    dT.ab[:, :, i, l] += ccpy_einsum("ef,aefb->ab", H0.ab.oovv[j, k, :, :], t4c) # (1)
                    dT.ab[:, :, j, l] -= ccpy_einsum("ef,aefb->ab", H0.ab.oovv[i, k, :, :], t4c) # (ij)
                    dT.ab[:, :, i, k] -= ccpy_einsum("ef,aefb->ab", H0.ab.oovv[j, l, :, :], t4c) # (kl)
                    dT.ab[:, :, j, k] += ccpy_einsum("ef,aefb->ab", H0.ab.oovv[i, l, :, :], t4c) # (ij)(kl)
                    # A(ij)A(kl)A(fc) -h2b(jkmf) * t4c(abfcijkl) -> x3b(abciml)
                    dT.aab[:, :, :, i, :, l] -= 0.5 * ccpy_einsum("mf,abfc->abcm", H0.ab.ooov[j, k, :, :] + X.ab.ooov[j, k, :, :], t4c) # (1)
                    dT.aab[:, :, :, j, :, l] += 0.5 * ccpy_einsum("mf,abfc->abcm", H0.ab.ooov[i, k, :, :] + X.ab.ooov[i, k, :, :], t4c) # (ij)
                    dT.aab[:, :, :, i, :, k] += 0.5 * ccpy_einsum("mf,abfc->abcm", H0.ab.ooov[j, l, :, :] + X.ab.ooov[j, l, :, :], t4c) # (kl)
                    dT.aab[:, :, :, j, :, k] -= 0.5 * ccpy_einsum("mf,abfc->abcm", H0.ab.ooov[i, l, :, :] + X.ab.ooov[i, l, :, :], t4c) # (ij)(kl)
                    # -h2c(lknf) * t4c(abfcijkl) -> x3b(abcijn)
                    dT.aab[:, :, :, i, j, :] -= 0.5 * ccpy_einsum("nf,abfc->abcn", H0.bb.ooov[l, k, :, :] + X.bb.ooov[l, k, :, :], t4c)
                    # A(ab)A(kl) h2b(bkef) * t4c(aefcijkl) -> x3b(abcijl)
                    dT.aab[:, :, :, i, j, l] += ccpy_einsum("bef,aefc->abc", H0.ab.vovv[:, k, :, :] + X.ab.vovv[:, k, :, :], t4c) # (1)
                    dT.aab[:, :, :, i, j, k] -= ccpy_einsum("bef,aefc->abc", H0.ab.vovv[:, l, :, :] + X.ab.vovv[:, l, :, :], t4c) # (kl)
                    # A(kl) h2c(ckef) * t4c(abfeijkl) -> x3b(abcijl)
                    dT.aab[:, :, :, i, j, l] += 0.25 * ccpy_einsum("cef,abfe->abc", H0.bb.vovv[:, k, :, :] + X.bb.vovv[:, k, :, :], t4c) # (1)
                    dT.aab[:, :, :, i, j, k] -= 0.25 * ccpy_einsum("cef,abfe->abc", H0.bb.vovv[:, l, :, :] + X.bb.vovv[:, l, :, :], t4c) # (1)
    # antisymmetrize
    # dT.aa -= np.transpose(dT.aa, (1, 0, 2, 3)) # (ab)
    # dT.aa -= np.transpose(dT.aa, (0, 1, 3, 2)) # (ij)
    # #
