...
def load_gamess_integrals(
    ...
    if fcidump_file:
        nmo_fcidump, nelectron_fcidump, ms2_fcidump = read_params_from_fcidump(fcidump_file)
...
def read_params_from_fcidump(fcidump):
    with open(fcidump, "r") as f:
        # Holds ['NORB=#', 'NELEC=#', 'MS2=#']
        firstline = f.readline().strip("\n").split()[1].split(",")
        # Get norbitals
        norbitals = int(firstline[0].split("=")[1])
        # Get nelectrons
        nelectrons = int(firstline[1].split("=")[1])
        # Get MS2 (note: this is not going to be correct for open shells in GAMESS)
        ms2 = int(firstline[2].split("=")[1])
