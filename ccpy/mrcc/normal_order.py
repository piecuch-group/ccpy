...
def shift_normal_order(H, occ_a, occ_b, occ_a_prev, occ_b_prev):
    ...
    if occ_a_prev: # if occ_a_prev /= []
        H.a -= ccpy_einsum("piqi->pq", H.aa[:, occ_a_prev, :, occ_a_prev])
        H.b -= ccpy_einsum("ipiq->pq", H.ab[occ_a_prev, :, occ_a_prev, :])
    if occ_b_prev: # if occ_b_prev /= []
        H.a -= ccpy_einsum("piqi->pq", H.ab[:, occ_b_prev, :, occ_b_prev])
        H.b -= ccpy_einsum("piqi->pq", H.bb[:, occ_b_prev, :, occ_b_prev])

    # compute reference energy with bare Hamiltonian
    reference_energy = calc_hf_energy_unsorted(H, occ_a, occ_b)

    # normal order F = Z + G
    H.a += ccpy_einsum("piqi->pq", H.aa[:, occ_a, :, occ_a])
    H.b += ccpy_einsum("ipiq->pq", H.ab[occ_a, :, occ_a, :])
    H.a += ccpy_einsum("piqi->pq", H.ab[:, occ_b, :, occ_b])
    H.b += ccpy_einsum("piqi->pq", H.bb[:, occ_b, :, occ_b])
