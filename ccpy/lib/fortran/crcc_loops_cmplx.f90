                        ...
                        do i = 1 , noa
                            do j = i+1, noa
                                do k = j+1, noa
                                   ...
                                   call zgemm('n','t',nua,nua2,noa,-0.5d0,H2A_vooo(:,:,i,j),nua,t2a(:,:,:,k),nua2,1.0d0,X3A,nua)
                                   call zgemm('n','t',nua,nua2,noa,0.5d0,H2A_vooo(:,:,k,j),nua,t2a(:,:,:,i),nua2,1.0d0,X3A,nua)
                                   call zgemm('n','t',nua,nua2,noa,0.5d0,H2A_vooo(:,:,i,k),nua,t2a(:,:,:,j),nua2,1.0d0,X3A,nua)
                                   ! Diagram 2: A(i/jk)A(c/ab) I2A_vvov(a,b,i,e)*t2a(e,c,j,k)
                                   call zgemm('n','n',nua2,nua,nua,0.5d0,I2A_vvov_1243(:,:,:,i),nua2,t2a(:,:,j,k),nua,1.0d0,X3A,nua2)
                                   call zgemm('n','n',nua2,nua,nua,-0.5d0,I2A_vvov_1243(:,:,:,j),nua2,t2a(:,:,i,k),nua,1.0d0,X3A,nua2)
                                   call zgemm('n','n',nua2,nua,nua,-0.5d0,I2A_vvov_1243(:,:,:,k),nua2,t2a(:,:,j,i),nua,1.0d0,X3A,nua2)
                                   !!!!! L3A !!!!!
                                   ! Diagram 1: A(i/jk)A(c/ab) H2A_vovv(e,i,b,a)*l2a(e,c,j,k)
                                   call zgemm('n','n',nua2,nua,nua,0.5d0,H2A_vovv_4312(:,:,:,i),nua2,l2a(:,:,j,k),nua,1.0d0,L3A,nua2)                         
                                   call zgemm('n','n',nua2,nua,nua,-0.5d0,H2A_vovv_4312(:,:,:,j),nua2,l2a(:,:,i,k),nua,1.0d0,L3A,nua2)                         
                                   call zgemm('n','n',nua2,nua,nua,-0.5d0,H2A_vovv_4312(:,:,:,k),nua2,l2a(:,:,j,i),nua,1.0d0,L3A,nua2)
                                   ! Diagram 2: -A(k/ij)A(a/bc) H2A_ooov(j,i,m,a)*l2a(b,c,m,k)-> a,m,j,i * (b,c,m,k)'
                                   call zgemm('n','t',nua,nua2,noa,-0.5d0,H2A_ooov_4312(:,:,j,i),nua,l2a(:,:,:,k),nua2,1.0d0,L3A,nua)
                                   call zgemm('n','t',nua,nua2,noa,0.5d0,H2A_ooov_4312(:,:,k,i),nua,l2a(:,:,:,j),nua2,1.0d0,L3A,nua)
                                   call zgemm('n','t',nua,nua2,noa,0.5d0,H2A_ooov_4312(:,:,j,k),nua,l2a(:,:,:,i),nua2,1.0d0,L3A,nua)
              ...
              subroutine crcc23B_opt(deltaA,deltaB,deltaC,deltaD,&
                        ...
                        do i = 1, noa
                            do j = i+1, noa
                                ...
                                call zgemm('n','t',nua,nuanub,nua,1.0d0,t2a(:,:,i,j),nua,H2B_vvvo(:,:,:,k),nuanub,1.0d0,X3B,nua)
                                ! Diagram 2: -A(ij) I2B(mcjk)*t2a(abim)
                                call zgemm('n','n',nua2,nub,noa,0.5d0,t2a(:,:,:,i),nua2,I2B_ovoo(:,:,j,k),noa,1.0d0,X3B,nua2)
                                call zgemm('n','n',nua2,nub,noa,-0.5d0,t2a(:,:,:,j),nua2,I2B_ovoo(:,:,i,k),noa,1.0d0,X3B,nua2)
                                ! Diagram 3: A(ab)A(ij) H2B(acie)*t2b(bejk) -> A(ab)A(ij) t2b(aeik)*H2B(bcje)
                                call zgemm('n','t',nua,nuanub,nub,1.0d0,t2b(:,:,i,k),nua,H2B_vvov_1243(:,:,:,j),nuanub,1.0d0,X3B,nua)
                                call zgemm('n','t',nua,nuanub,nub,-1.0d0,t2b(:,:,j,k),nua,H2B_vvov_1243(:,:,:,i),nuanub,1.0d0,X3B,nua)
                                ! Diagram 4: -A(ab)A(ij) I2B(amik)*t2b(bcjm)
                                call zgemm('n','t',nua,nuanub,nob,-1.0d0,I2B_vooo(:,:,i,k),nua,t2b_1243(:,:,:,j),nuanub,1.0d0,X3B,nua)
                                call zgemm('n','t',nua,nuanub,nob,1.0d0,I2B_vooo(:,:,j,k),nua,t2b_1243(:,:,:,i),nuanub,1.0d0,X3B,nua)
                                ! Diagram 5: A(ij) H2A(abie)*t2b(ecjk)
                                call zgemm('n','n',nua2,nub,nua,0.5d0,H2A_vvov_1243(:,:,:,i),nua2,t2b(:,:,j,k),nua,1.0d0,X3B,nua2)
                                call zgemm('n','n',nua2,nub,nua,-0.5d0,H2A_vvov_1243(:,:,:,j),nua2,t2b(:,:,i,k),nua,1.0d0,X3B,nua2)
                                ! Diagram 6: -A(ab) I2A(amij)*t2b(bcmk)
                                call zgemm('n','t',nua,nuanub,noa,-1.0d0,I2A_vooo(:,:,i,j),nua,t2b(:,:,:,k),nuanub,1.0d0,X3B,nua)
                                !!!!! L3B !!!!!
                                ! Diagram 1: A(ab) H2B(ekbc)*l2a(aeij)
                                call zgemm('n','n',nua,nuanub,nua,1.0d0,l2a(:,:,i,j),nua,H2B_vovv_1342(:,:,:,k),nua,1.0d0,L3B,nua)
                                ! Diagram 2: A(ij) H2A(eiba)*l2b(ecjk)
                                call zgemm('n','n',nua2,nub,nua,0.5d0,H2A_vovv_4312(:,:,:,i),nua2,l2b(:,:,j,k),nua,1.0d0,L3B,nua2)
                                call zgemm('n','n',nua2,nub,nua,-0.5d0,H2A_vovv_4312(:,:,:,j),nua2,l2b(:,:,i,k),nua,1.0d0,L3B,nua2)
                                ! Diagram 3: A(ij)A(ab) H2B(ieac)*l2b(bejk) -> l2b(aeik)*H2B(jebc)
                                call zgemm('n','n',nua,nuanub,nub,1.0d0,l2b(:,:,i,k),nua,H2B_ovvv_2341(:,:,:,j),nub,1.0d0,L3B,nua)
                                call zgemm('n','n',nua,nuanub,nub,-1.0d0,l2b(:,:,j,k),nua,H2B_ovvv_2341(:,:,:,i),nub,1.0d0,L3B,nua)
                                ! Diagram 4: -A(ij) H2B(jkmc)*l2a(abim) -> +A(ij) H2B(jkmc)*l2a(abmi)
                                call zgemm('n','n',nua2,nub,noa,0.5d0,l2a(:,:,:,i),nua2,H2B_ooov_3412(:,:,j,k),noa,1.0d0,L3B,nua2)
                                call zgemm('n','n',nua2,nub,noa,-0.5d0,l2a(:,:,:,j),nua2,H2B_ooov_3412(:,:,i,k),noa,1.0d0,L3B,nua2)
                                ! Diagram 5: -A(ab) H2A(jima)*l2b(bcmk)
                                call zgemm('n','t',nua,nuanub,noa,-1.0d0,H2A_ooov_4312(:,:,j,i),nua,l2b(:,:,:,k),nuanub,1.0d0,L3B,nua)
                                ! Diagram 6: -A(ij)A(ab) H2B(ikam)*l2b(bcjm)
                                call zgemm('n','t',nua,nuanub,nob,-1.0d0,H2B_oovo_3412(:,:,i,k),nua,l2b_1243(:,:,:,j),nuanub,1.0d0,L3B,nua)
                                call zgemm('n','t',nua,nuanub,nob,1.0d0,H2B_oovo_3412(:,:,j,k),nua,l2b_1243(:,:,:,i),nuanub,1.0d0,L3B,nua)
              ...
              subroutine crcc23C_opt(deltaA,deltaB,deltaC,deltaD,&
                        ...
                        do i = 1 , noa
                            do j = 1, nob
                                ...
                                call zgemm('n','n',nuanub,nub,nub,1.0d0,H2B_vvov_1243(:,:,:,i),nuanub,t2c(:,:,j,k),nub,1.0d0,X3C,nuanub)
                                ! Diagram 2: -A(jk) I2B_vooo(a,m,i,j)*t2c(b,c,m,k)
                                call zgemm('n','t',nua,nub2,nob,-0.5d0,I2B_vooo(:,:,i,j),nua,t2c(:,:,:,k),nub2,1.0d0,X3C,nua)
                                call zgemm('n','t',nua,nub2,nob,0.5d0,I2B_vooo(:,:,i,k),nua,t2c(:,:,:,j),nub2,1.0d0,X3C,nua)
                                ! Diagram 3: A(jk) H2C_vvov(c,b,k,e)*t2b(a,e,i,j)
                                call zgemm('n','n',nua,nub2,nub,0.5d0,t2b(:,:,i,j),nua,H2C_vvov_4213(:,:,:,k),nub,1.0d0,X3C,nua)
                                call zgemm('n','n',nua,nub2,nub,-0.5d0,t2b(:,:,i,k),nua,H2C_vvov_4213(:,:,:,j),nub,1.0d0,X3C,nua)
                                ! Diagram 4: -A(bc) I2C_vooo(c,m,k,j)*t2b(a,b,i,m)
                                call zgemm('n','n',nuanub,nub,nob,-1.0d0,t2b_1243(:,:,:,i),nuanub,I2C_vooo_2134(:,:,k,j),nob,1.0d0,X3C,nuanub)
                                ! Diagram 5: A(jk)A(bc) H2B_vvvo(a,b,e,j)*t2b(e,c,i,k)
                                call zgemm('n','n',nuanub,nub,nua,1.0d0,H2B_vvvo(:,:,:,j),nuanub,t2b(:,:,i,k),nua,1.0d0,X3C,nuanub)
                                call zgemm('n','n',nuanub,nub,nua,-1.0d0,H2B_vvvo(:,:,:,k),nuanub,t2b(:,:,i,j),nua,1.0d0,X3C,nuanub)
                                ! Diagram 6: -A(jk)A(bc) I2B_ovoo(m,b,i,j)*t2b(a,c,m,k) -> -A(jk)A(bc) I2B_ovoo(m,c,i,k)*t2b(a,b,m,j)
                                call zgemm('n','n',nuanub,nub,noa,-1.0d0,t2b(:,:,:,j),nuanub,I2B_ovoo(:,:,i,k),noa,1.0d0,X3C,nuanub)
                                call zgemm('n','n',nuanub,nub,noa,1.0d0,t2b(:,:,:,k),nuanub,I2B_ovoo(:,:,i,j),noa,1.0d0,X3C,nuanub)

                                !!!!! L3C !!!!!
                                ! Diagram 1: A(bc) H2B_ovvv(i,e,a,b)*l2c(e,c,j,k)
                                call zgemm('n','n',nuanub,nub,nub,1.0d0,H2B_ovvv_3421(:,:,:,i),nuanub,l2c(:,:,j,k),nub,1.0d0,L3C,nuanub)
                                ! Diagram 2: A(jk) H2C_vovv(e,k,b,c)*l2b(a,e,i,j)
                                call zgemm('n','n',nua,nub2,nub,0.5d0,l2b(:,:,i,j),nua,H2C_vovv_1342(:,:,:,k),nub,1.0d0,L3C,nua)
                                call zgemm('n','n',nua,nub2,nub,-0.5d0,l2b(:,:,i,k),nua,H2C_vovv_1342(:,:,:,j),nub,1.0d0,L3C,nua)
                                ! Diagram 3: A(jk)A(bc) H2B_vovv(e,j,a,b)*l2b(e,c,i,k)
                                call zgemm('n','n',nuanub,nub,nua,1.0d0,H2B_vovv_3412(:,:,:,j),nuanub,l2b(:,:,i,k),nua,1.0d0,L3C,nuanub)
                                call zgemm('n','n',nuanub,nub,nua,-1.0d0,H2B_vovv_3412(:,:,:,k),nuanub,l2b(:,:,i,j),nua,1.0d0,L3C,nuanub)
                                ! Diagram 4: -A(jk) H2B_oovo(i,j,a,m)*l2c(b,c,m,k)
                                call zgemm('n','t',nua,nub2,nob,-0.5d0,H2B_oovo_3412(:,:,i,j),nua,l2c(:,:,:,k),nub2,1.0d0,L3C,nua)
                                call zgemm('n','t',nua,nub2,nob,0.5d0,H2B_oovo_3412(:,:,i,k),nua,l2c(:,:,:,j),nub2,1.0d0,L3C,nua)
                                ! Diagram 5: -A(bc) H2C_ooov(j,k,m,c)*l2b(a,b,i,m)
                                call zgemm('n','n',nuanub,nub,nob,-1.0d0,l2b_1243(:,:,:,i),nuanub,H2C_ooov_3412(:,:,j,k),nob,1.0d0,L3C,nuanub)
                                ! Diagram 6: -A(jk)A(bc) H2B_ooov(i,j,m,b)*l2b(a,c,m,k) -> -A(jk)A(bc) H2B_ooov(i,k,m,c)*l2b(a,b,m,j)
                                call zgemm('n','n',nuanub,nub,noa,-1.0d0,l2b(:,:,:,j),nuanub,H2B_ooov_3412(:,:,i,k),noa,1.0d0,L3C,nuanub)
                                call zgemm('n','n',nuanub,nub,noa,1.0d0,l2b(:,:,:,k),nuanub,H2B_ooov_3412(:,:,i,j),noa,1.0d0,L3C,nuanub)
                        ...
                        do i = 1 , nob
                            do j = i+1, nob
                                do k = j+1, nob
                                   ...
                                   call zgemm('n','t',nub,nub2,nob,-0.5d0,H2C_vooo(:,:,i,j),nub,t2c(:,:,:,k),nub2,1.0d0,X3D,nub)
                                   call zgemm('n','t',nub,nub2,nob,0.5d0,H2C_vooo(:,:,k,j),nub,t2c(:,:,:,i),nub2,1.0d0,X3D,nub)
                                   call zgemm('n','t',nub,nub2,nob,0.5d0,H2C_vooo(:,:,i,k),nub,t2c(:,:,:,j),nub2,1.0d0,X3D,nub)
                                   ! Diagram 2: A(i/jk)A(c/ab) I2C_vvov(a,b,i,e)*t2c(e,c,j,k)
                                   call zgemm('n','n',nub2,nub,nub,0.5d0,I2C_vvov_1243(:,:,:,i),nub2,t2c(:,:,j,k),nub,1.0d0,X3D,nub2)
                                   call zgemm('n','n',nub2,nub,nub,-0.5d0,I2C_vvov_1243(:,:,:,j),nub2,t2c(:,:,i,k),nub,1.0d0,X3D,nub2)
                                   call zgemm('n','n',nub2,nub,nub,-0.5d0,I2C_vvov_1243(:,:,:,k),nub2,t2c(:,:,j,i),nub,1.0d0,X3D,nub2)
                                   !!!!! L3A !!!!!
                                   ! Diagram 1: A(i/jk)A(c/ab) H2C_vovv(e,i,b,a)*l2c(e,c,j,k)
                                   call zgemm('n','n',nub2,nub,nub,0.5d0,H2C_vovv_4312(:,:,:,i),nub2,l2c(:,:,j,k),nub,1.0d0,L3D,nub2)                         
                                   call zgemm('n','n',nub2,nub,nub,-0.5d0,H2C_vovv_4312(:,:,:,j),nub2,l2c(:,:,i,k),nub,1.0d0,L3D,nub2)                         
                                   call zgemm('n','n',nub2,nub,nub,-0.5d0,H2C_vovv_4312(:,:,:,k),nub2,l2c(:,:,j,i),nub,1.0d0,L3D,nub2)
                                   ! Diagram 2: -A(k/ij)A(a/bc) H2A_ooov(j,i,m,a)*l2a(b,c,m,k)-> a,m,j,i * (b,c,m,k)'
                                   call zgemm('n','t',nub,nub2,nob,-0.5d0,H2C_ooov_4312(:,:,j,i),nub,l2c(:,:,:,k),nub2,1.0d0,L3D,nub)
                                   call zgemm('n','t',nub,nub2,nob,0.5d0,H2C_ooov_4312(:,:,k,i),nub,l2c(:,:,:,j),nub2,1.0d0,L3D,nub)
                                   call zgemm('n','t',nub,nub2,nob,0.5d0,H2C_ooov_4312(:,:,j,k),nub,l2c(:,:,:,i),nub2,1.0d0,L3D,nub)
                        ...
                        do i = 1 , noa
                            do j = i+1, noa
                                do k = j+1, noa
                                   ...
                                   call zgemm('n','n',nua2,nua,nua,0.5d0,chi2A_vvvo(:,:,:,j),nua2,t2a(:,:,i,k),nua,1.0d0,Y3A,nua2)
                                   call zgemm('n','n',nua2,nua,nua,-0.5d0,chi2A_vvvo(:,:,:,k),nua2,t2a(:,:,i,j),nua,1.0d0,Y3A,nua2)
                                   call zgemm('n','n',nua2,nua,nua,-0.5d0,chi2A_vvvo(:,:,:,i),nua2,t2a(:,:,j,k),nua,1.0d0,Y3A,nua2)
                                   ! Diagram 1b: A(j/ik)A(c/ab) H2A_vvov(b,a,j,e)*r2a(e,c,i,k)
                                   call zgemm('n','n',nua2,nua,nua,0.5d0,H2A_vvov_2143(:,:,:,j),nua2,r2a(:,:,i,k),nua,1.0d0,Y3A,nua2)
                                   call zgemm('n','n',nua2,nua,nua,-0.5d0,H2A_vvov_2143(:,:,:,k),nua2,r2a(:,:,i,j),nua,1.0d0,Y3A,nua2)
                                   call zgemm('n','n',nua2,nua,nua,-0.5d0,H2A_vvov_2143(:,:,:,i),nua2,r2a(:,:,j,k),nua,1.0d0,Y3A,nua2)
                                   ! Diagram 2a: -A(k/ij)A(b/ac) chi2A_ovoo(m,b,i,j)*t2a(a,c,m,k)
                                   call zgemm('n','n',nua2,nua,noa,-0.5d0,t2a(:,:,:,j),nua2,chi2A_ovoo(:,:,i,k),noa,1.0d0,Y3A,nua2)
                                   call zgemm('n','n',nua2,nua,noa,0.5d0,t2a(:,:,:,i),nua2,chi2A_ovoo(:,:,j,k),noa,1.0d0,Y3A,nua2)
                                   call zgemm('n','n',nua2,nua,noa,0.5d0,t2a(:,:,:,k),nua2,chi2A_ovoo(:,:,i,j),noa,1.0d0,Y3A,nua2)
                                   ! Diagram 2b: -A(k/ij)A(b/ac) H2A_vooo(b,m,j,i)*r2a(a,c,m,k)
                                   call zgemm('n','n',nua2,nua,noa,-0.5d0,r2a(:,:,:,j),nua2,H2A_vooo_2143(:,:,i,k),noa,1.0d0,Y3A,nua2)
                                   call zgemm('n','n',nua2,nua,noa,0.5d0,r2a(:,:,:,i),nua2,H2A_vooo_2143(:,:,j,k),noa,1.0d0,Y3A,nua2)
                                   call zgemm('n','n',nua2,nua,noa,0.5d0,r2a(:,:,:,k),nua2,H2A_vooo_2143(:,:,i,j),noa,1.0d0,Y3A,nua2)
                                   !!!!! MM(2,3)A !!!!!
                                   ! Diagram 1: -A(k/ij)A(a/bc) I2A_vooo(a,m,i,j)*t2a(b,c,m,k)
                                   call zgemm('n','t',nua,nua2,noa,-0.5d0,H2A_vooo(:,:,i,j),nua,t2a(:,:,:,k),nua2,1.0d0,X3A,nua)
                                   call zgemm('n','t',nua,nua2,noa,0.5d0,H2A_vooo(:,:,k,j),nua,t2a(:,:,:,i),nua2,1.0d0,X3A,nua)
              ...
              subroutine creomcc23B_opt(deltaA,deltaB,deltaC,deltaD,&
                        ...
                        do i = 1, noa
                            do j = i+1, noa
                                ...
                                call zgemm('n','t',nua,nuanub,nua,1.0d0,t2a(:,:,i,j),nua,chi2B_vvvo(:,:,:,k),nuanub,1.0d0,Y3B,nua)
                                ! Diagram 1b: A(ab) H2B_vvvo(b,c,e,k)*r2a(a,e,i,j)
                                call zgemm('n','t',nua,nuanub,nua,1.0d0,r2a(:,:,i,j),nua,H2B_vvvo(:,:,:,k),nuanub,1.0d0,Y3B,nua)
                                ! Diagram 2a: -A(ij) chi2B_ovoo(n,c,j,k)*t2a(a,b,i,n)
                                call zgemm('n','n',nua2,nub,noa,0.5d0,t2a(:,:,:,i),nua2,chi2B_ovoo(:,:,j,k),noa,1.0d0,Y3B,nua2)
                                call zgemm('n','n',nua2,nub,noa,-0.5d0,t2a(:,:,:,j),nua2,chi2B_ovoo(:,:,i,k),noa,1.0d0,Y3B,nua2)
                                ! Diagram 2b: -A(ij) H2B_ovoo(n,c,j,k)*r2a(a,b,i,n)
                                call zgemm('n','n',nua2,nub,noa,0.5d0,r2a(:,:,:,i),nua2,H2B_ovoo(:,:,j,k),noa,1.0d0,Y3B,nua2)
                                call zgemm('n','n',nua2,nub,noa,-0.5d0,r2a(:,:,:,j),nua2,H2B_ovoo(:,:,i,k),noa,1.0d0,Y3B,nua2)
                                ! Diagram 3a: A(ij) chi2A_vvvo(a,b,e,j)*t2b(e,c,i,k)
                                call zgemm('n','n',nua2,nub,nua,0.5d0,chi2A_vvvo(:,:,:,j),nua2,t2b(:,:,i,k),nua,1.0d0,Y3B,nua2)
                                call zgemm('n','n',nua2,nub,nua,-0.5d0,chi2A_vvvo(:,:,:,i),nua2,t2b(:,:,j,k),nua,1.0d0,Y3B,nua2)
                                ! Diagram 3b: A(ij) H2A_vvov(b,a,j,e)*r2b(e,c,i,k)
                                call zgemm('n','n',nua2,nub,nua,0.5d0,H2A_vvov_2143(:,:,:,j),nua2,r2b(:,:,i,k),nua,1.0d0,Y3B,nua2)
                                call zgemm('n','n',nua2,nub,nua,-0.5d0,H2A_vvov_2143(:,:,:,i),nua2,r2b(:,:,j,k),nua,1.0d0,Y3B,nua2)
                                ! Diagram 4a: -A(ab) chi2A_vooo(b,n,j,i)*t2b(a,c,n,k) -> -A(ab) chi2A_vooo(a,n,i,j)*t2b(b,c,n,k)
                                call zgemm('n','t',nua,nuanub,noa,-1.0d0,chi2A_vooo(:,:,i,j),nua,t2b(:,:,:,k),nuanub,1.0d0,Y3B,nua)
                                ! Diagram 4b: -A(ab) H2A_vooo(b,n,j,i)*r2b(a,c,n,k)
                                call zgemm('n','t',nua,nuanub,noa,-1.0d0,H2A_vooo(:,:,i,j),nua,r2b(:,:,:,k),nuanub,1.0d0,Y3B,nua)
                                ! Diagram 5a: A(ij)A(ab) chi2B_vvov(b,c,j,e)*t2b(a,e,i,k)
                                call zgemm('n','n',nua,nuanub,nub,1.0d0,t2b(:,:,i,k),nua,chi2B_vvov_4123(:,:,:,j),nub,1.0d0,Y3B,nua)
                                call zgemm('n','n',nua,nuanub,nub,-1.0d0,t2b(:,:,j,k),nua,chi2B_vvov_4123(:,:,:,i),nub,1.0d0,Y3B,nua)
                                ! Diagram 5b: A(ij)A(ab) H2B_vvov(b,c,j,e)*r2b(a,e,i,k)
                                call zgemm('n','n',nua,nuanub,nub,1.0d0,r2b(:,:,i,k),nua,H2B_vvov_4123(:,:,:,j),nub,1.0d0,Y3B,nua)
                                call zgemm('n','n',nua,nuanub,nub,-1.0d0,r2b(:,:,j,k),nua,H2B_vvov_4123(:,:,:,i),nub,1.0d0,Y3B,nua)
                                ! Diagram 6a: -A(ij)A(ab) chi2B_vooo(b,n,j,k)*t2b(a,c,i,n) -> -A(ij)A(ab) chi2B_vooo(a,n,i,k)*t2b(b,c,j,n)
                                call zgemm('n','t',nua,nuanub,nob,-1.0d0,chi2B_vooo(:,:,i,k),nua,t2b_1243(:,:,:,j),nuanub,1.0d0,Y3B,nua)
                                call zgemm('n','t',nua,nuanub,nob,1.0d0,chi2B_vooo(:,:,j,k),nua,t2b_1243(:,:,:,i),nuanub,1.0d0,Y3B,nua)
                                ! Diagram 6b: -A(ij)A(ab) H2B_vooo(b,n,j,k)*r2b(a,c,i,n)
                                call zgemm('n','t',nua,nuanub,nob,-1.0d0,H2B_vooo(:,:,i,k),nua,r2b_1243(:,:,:,j),nuanub,1.0d0,Y3B,nua)
                                call zgemm('n','t',nua,nuanub,nob,1.0d0,H2B_vooo(:,:,j,k),nua,r2b_1243(:,:,:,i),nuanub,1.0d0,Y3B,nua)
              ...
              subroutine creomcc23C_opt(deltaA,deltaB,deltaC,deltaD,&
                        ...
                        do i = 1 , noa
                            do j = 1, nob
                                ...
                                call zgemm('n','n',nuanub,nub,nub,1.0d0,chi2B_vvov_1243(:,:,:,i),nuanub,t2c(:,:,j,k),nub,1.0d0,Y3C,nuanub)
                                ! Diagram 1b: A(bc) H2B_vvov(a,b,i,e)*r2c(e,c,j,k)  
                                call zgemm('n','n',nuanub,nub,nub,1.0d0,H2B_vvov_1243(:,:,:,i),nuanub,r2c(:,:,j,k),nub,1.0d0,Y3C,nuanub)
                                ! Diagram 2a: -A(jk) chi2B_vooo(a,n,i,j)*t2c(b,c,n,k)
                                call zgemm('n','t',nua,nub2,nob,-0.5d0,chi2B_vooo(:,:,i,j),nua,t2c(:,:,:,k),nub2,1.0d0,Y3C,nua) 
                                call zgemm('n','t',nua,nub2,nob,0.5d0,chi2B_vooo(:,:,i,k),nua,t2c(:,:,:,j),nub2,1.0d0,Y3C,nua) 
                                ! Diagram 2b: -A(jk) H2B_vooo(a,n,i,j)*r2c(b,c,n,k) 
                                call zgemm('n','t',nua,nub2,nob,-0.5d0,H2B_vooo(:,:,i,j),nua,r2c(:,:,:,k),nub2,1.0d0,Y3C,nua) 
                                call zgemm('n','t',nua,nub2,nob,0.5d0,H2B_vooo(:,:,i,k),nua,r2c(:,:,:,j),nub2,1.0d0,Y3C,nua) 
                                ! Diagram 3a: A(jk) chi2C_vvvo(c,b,e,j)*t2b(a,e,i,k)
                                call zgemm('n','n',nua,nub2,nub,0.5d0,t2b(:,:,i,k),nua,chi2C_vvvo_3214(:,:,:,j),nub,1.0d0,Y3C,nua)
                                call zgemm('n','n',nua,nub2,nub,-0.5d0,t2b(:,:,i,j),nua,chi2C_vvvo_3214(:,:,:,k),nub,1.0d0,Y3C,nua)
                                ! Diagram 3b: A(jk) H2C_vvov(b,c,j,e)*r2b(a,e,i,k) 
                                call zgemm('n','n',nua,nub2,nub,0.5d0,r2b(:,:,i,k),nua,H2C_vvov_4123(:,:,:,j),nub,1.0d0,Y3C,nua)
                                call zgemm('n','n',nua,nub2,nub,-0.5d0,r2b(:,:,i,j),nua,H2C_vvov_4123(:,:,:,k),nub,1.0d0,Y3C,nua)
                                ! Diagram 4a: -A(bc) chi2C_vooo(b,n,j,k)*t2b(a,c,i,n) -> -A(bc) t2b(a,b,i,n)*chi2C_vooo(c,n,k,j)
                                call zgemm('n','n',nuanub,nub,nob,-1.0d0,t2b_1243(:,:,:,i),nuanub,chi2C_vooo_2134(:,:,k,j),nob,1.0d0,Y3C,nuanub)
                                ! Diagram 4b: -A(bc) H2C_vooo(b,n,j,k)*r2b(a,c,i,n) -> -A(bc) r2b(a,b,i,n)*H2C_vooo(c,n,k,j)
                                call zgemm('n','n',nuanub,nub,nob,-1.0d0,r2b_1243(:,:,:,i),nuanub,H2C_vooo_2134(:,:,k,j),nob,1.0d0,Y3C,nuanub)
                                ! Diagram 5a: A(jk)A(bc) chi2B_vvvo(a,b,e,j)*t2b(e,c,i,k)
                                call zgemm('n','n',nuanub,nub,nua,1.0d0,chi2B_vvvo(:,:,:,j),nuanub,t2b(:,:,i,k),nua,1.0d0,Y3C,nuanub) 
                                call zgemm('n','n',nuanub,nub,nua,-1.0d0,chi2B_vvvo(:,:,:,k),nuanub,t2b(:,:,i,j),nua,1.0d0,Y3C,nuanub) 
                                ! Diagram 5b: A(jk)A(bc) H2B_vvvo(a,b,e,j)*r2b(e,c,i,k) 
                                call zgemm('n','n',nuanub,nub,nua,1.0d0,H2B_vvvo(:,:,:,j),nuanub,r2b(:,:,i,k),nua,1.0d0,Y3C,nuanub) 
                                call zgemm('n','n',nuanub,nub,nua,-1.0d0,H2B_vvvo(:,:,:,k),nuanub,r2b(:,:,i,j),nua,1.0d0,Y3C,nuanub) 
                                ! Diagram 6a: -A(jk)A(bc) chi2B_ovoo(n,b,i,j)*t2b(a,c,n,k) -> -A(jk)A(bc) t2b(a,b,n,j)*chi2B_ovoo(n,c,i,k)
                                call zgemm('n','n',nuanub,nub,noa,-1.0d0,t2b(:,:,:,j),nuanub,chi2B_ovoo(:,:,i,k),noa,1.0d0,Y3C,nuanub)
                                call zgemm('n','n',nuanub,nub,noa,1.0d0,t2b(:,:,:,k),nuanub,chi2B_ovoo(:,:,i,j),noa,1.0d0,Y3C,nuanub)
                                ! Diagram 6b: -A(jk)A(bc) H2B_ovoo(n,b,i,j)*r2b(a,c,n,k) -> -A(jk)A(bc) r2b(a,b,n,j)*H2B_ovoo(n,c,i,k)
                                call zgemm('n','n',nuanub,nub,noa,-1.0d0,r2b(:,:,:,j),nuanub,H2B_ovoo(:,:,i,k),noa,1.0d0,Y3C,nuanub)
                                call zgemm('n','n',nuanub,nub,noa,1.0d0,r2b(:,:,:,k),nuanub,H2B_ovoo(:,:,i,j),noa,1.0d0,Y3C,nuanub)
                        ...
                        do i = 1 , nob
                            do j = i+1, nob
                                do k = j+1, nob
                                   ...
                                   call zgemm('n','n',nub2,nub,nub,0.5d0,chi2C_vvvo(:,:,:,j),nub2,t2c(:,:,i,k),nub,1.0d0,Y3D,nub2)
                                   call zgemm('n','n',nub2,nub,nub,-0.5d0,chi2C_vvvo(:,:,:,k),nub2,t2c(:,:,i,j),nub,1.0d0,Y3D,nub2)
                                   call zgemm('n','n',nub2,nub,nub,-0.5d0,chi2C_vvvo(:,:,:,i),nub2,t2c(:,:,j,k),nub,1.0d0,Y3D,nub2)
                                   ! Diagram 1b: A(j/ik)A(c/ab) H2C_vvov(b,a,j,e)*r2c(e,c,i,k)
                                   call zgemm('n','n',nub2,nub,nub,0.5d0,H2C_vvov_2143(:,:,:,j),nub2,r2c(:,:,i,k),nub,1.0d0,Y3D,nub2)
                                   call zgemm('n','n',nub2,nub,nub,-0.5d0,H2C_vvov_2143(:,:,:,k),nub2,r2c(:,:,i,j),nub,1.0d0,Y3D,nub2)
                                   call zgemm('n','n',nub2,nub,nub,-0.5d0,H2C_vvov_2143(:,:,:,i),nub2,r2c(:,:,j,k),nub,1.0d0,Y3D,nub2)
                                   ! Diagram 2a: -A(k/ij)A(b/ac) chi2C_ovoo(m,b,i,j)*t2c(a,c,m,k)
                                   call zgemm('n','n',nub2,nub,nob,-0.5d0,t2c(:,:,:,j),nub2,chi2C_ovoo(:,:,i,k),nob,1.0d0,Y3D,nub2)
                                   call zgemm('n','n',nub2,nub,nob,0.5d0,t2c(:,:,:,i),nub2,chi2C_ovoo(:,:,j,k),nob,1.0d0,Y3D,nub2)
                                   call zgemm('n','n',nub2,nub,nob,0.5d0,t2c(:,:,:,k),nub2,chi2C_ovoo(:,:,i,j),nob,1.0d0,Y3D,nub2)
                                   ! Diagram 2b: -A(k/ij)A(b/ac) H2C_vooo(b,m,j,i)*r2c(a,c,m,k)
                                   call zgemm('n','n',nub2,nub,nob,-0.5d0,r2c(:,:,:,j),nub2,H2C_vooo_2143(:,:,i,k),nob,1.0d0,Y3D,nub2)
                                   call zgemm('n','n',nub2,nub,nob,0.5d0,r2c(:,:,:,i),nub2,H2C_vooo_2143(:,:,j,k),nob,1.0d0,Y3D,nub2)
                                   call zgemm('n','n',nub2,nub,nob,0.5d0,r2c(:,:,:,k),nub2,H2C_vooo_2143(:,:,i,j),nob,1.0d0,Y3D,nub2)
                                   !!!!! MM(2,3)D !!!!!
                                   ! Diagram 1: -A(k/ij)A(a/bc) H2C_vooo(a,m,i,j)*t2c(b,c,m,k)
                                   call zgemm('n','t',nub,nub2,nob,-0.5d0,H2C_vooo(:,:,i,j),nub,t2c(:,:,:,k),nub2,1.0d0,X3D,nub)
                                   call zgemm('n','t',nub,nub2,nob,0.5d0,H2C_vooo(:,:,k,j),nub,t2c(:,:,:,i),nub2,1.0d0,X3D,nub)
