import numpy as np
from ccpy.eomcc.eaeom3_intermediates import get_eaeom3_intermediates
...
def HR(dR, R, T, H, flag_RHF, system):
    # Get intermediates
    X = get_eaeom3_intermediates(H, R)
    # update R1
...
def build_HR_3A(R, T, X, H):
    ...
    X3A = -(2.0 / 12.0) * np.einsum("mj,abcmk->abcjk", H.a.oo, R.aaa, optimize=True)
    X3A += (3.0 / 12.0) * np.einsum("be,aecjk->abcjk", H.a.vv, R.aaa, optimize=True)
    X3A += (3.0 / 24.0) * np.einsum("abef,efcjk->abcjk", H.aa.vvvv, R.aaa, optimize=True)
    X3A += (1.0 / 24.0) * np.einsum("mnjk,abcmn->abcjk", H.aa.oooo, R.aaa, optimize=True)
    X3A += (6.0 / 12.0) * np.einsum("cmke,abejm->abcjk", H.aa.voov, R.aaa, optimize=True)
    X3A += (6.0 / 12.0) * np.einsum("cmke,abejm->abejk", H.ab.voov, R.aab, optimize=True)
    # moment-like terms
    X3A -= (3.0 / 12.0) * np.einsum("cmkj,abm->abcjk", H.aa.vooo, R.aa, optimize=True)
    X3A += (6.0 / 12.0) * np.einsum("cbke,aej->abcjk", H.aa.vvov, R.aa, optimize=True)
    # 3-body Hbar terms factorized using intermediates
    X3A -= (6.0 / 12.0) * np.einsum("bmj,acmk->abcjk", X["aa"]["voo"], T.aa, optimize=True)
    X3A += (3.0 / 12.0) * np.einsum("abe,ecjk->abcjk", X["aa"]["vvv"], T.aa, optimize=True)
...
def build_HR_3B(R, T, X, H):
    ...
    X3B = -(1.0 / 2.0) * np.einsum("mj,abcmk->abcjk", H.a.oo, R.aab, optimize=True)
    X3B -= (1.0 / 2.0) * np.einsum("mk,abcjm->abcjk", H.b.oo, R.aab, optimize=True)
    X3B += (1.0 / 2.0) * np.einsum("ce,abejk->abcjk", H.b.vv, R.aab, optimize=True)
    X3B += np.einsum("be,aecjk->abcjk", H.a.vv, R.aab, optimize=True)
    X3B += (1.0 / 2.0) * np.einsum("mnjk,abcmn->abcjk", H.ab.oooo, R.aab, optimize=True)
    X3B += (1.0 / 2.0) * np.einsum("mcek,abejm->abcjk", H.ab.ovvo, R.aaa, optimize=True)
    X3B += (1.0 / 2.0) * np.einsum("cmke,abejm->abcjk", H.bb.voov, R.aab, optimize=True)
    X3B += np.einsum("bmje,aecmk->abcjk", H.aa.voov, R.aab, optimize=True)
    X3B += np.einsum("bmje,aecmk->abcjk", H.ab.voov, R.abb, optimize=True)
    X3B -= np.einsum("bmek,aecjm->abcjk", H.ab.vovo, R.aab, optimize=True)
    X3B -= (1.0 / 2.0) * np.einsum("mcje,abemk->abcjk", H.ab.ovov, R.aab, optimize=True)
    X3B += (1.0 / 4.0) * np.einsum("abef,efcjk->abcjk", H.aa.vvvv, R.aab, optimize=True)
    X3B += np.einsum("bcef,aefjk->abcjk", H.ab.vvvv, R.aab, optimize=True)
    # moment-like terms
    X3B -= (1.0 / 2.0) * np.einsum("mcjk,abm->abcjk", H.ab.ovoo, R.aa, optimize=True)
    X3B -= np.einsum("bmjk,acm->abcjk", H.ab.vooo, R.ab, optimize=True)
    X3B += np.einsum("bcek,aej->abcjk", H.ab.vvvo, R.aa, optimize=True)
    X3B += np.einsum("bcje,aek->abcjk", H.ab.vvov, R.ab, optimize=True)
    # 3-body Hbar terms factorized using intermediates
    X3B -= (1.0 / 2.0) * np.einsum("mck,abmj->abcjk", X["ab"]["ovo"], T.aa, optimize=True)
    X3B -= np.einsum("bmj,acmk->abcjk", X["aa"]["voo"], T.ab, optimize=True)
    X3B -= np.einsum("amk,bcjm->abcjk", X["ab"]["voo"], T.ab, optimize=True)
    X3B += (1.0 / 2.0) * np.einsum("abe,ecjk->abcjk", X["aa"]["vvv"], T.ab, optimize=True)
    X3B += np.einsum("ace,bejk->abcjk", X["ab"]["vvv"], T.ab, optimize=True)
