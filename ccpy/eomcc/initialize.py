...
def get_eomcc_initial_guess(
    ...
    for sym_target, num_root_sym in enumerate(nroot):
        ...
        print(
            "Calculating initial guess for {} singlet states of {} symmetry ".format(
                num_root_sym, state_irrep
            )
        )
        ...
        (
            ...
            omega_eomccsd,
            Hmat,
        ) = eomcc_initial_guess.eomcc_initial_guess.eomccs_d_matrix(
            idx1A,
            idx1B,
            idx2A,
            idx2B,
            ...
            for i in range(len(omega_eomccsd)):
                ...
                if abs(chk) < 1.0e-01:
                    (
                        ...
                        r2b,
                        r2c,
                    ) = eomcc_initial_guess.eomcc_initial_guess.unflatten_guess_vector(
                        Cvec[:, i],
                        idx1A,
                        idx1B,
                        idx2A,
                ...
                print(
                    "Could not find {} singlet roots of {} symmetry in EOMCCSd guess!".format(
                        num_root_sym, state_irrep
                    )
                )
        else:  # open shell
    ...
    for i in range(len(E0)):
        print(
            "Root - {}  (Sym: {})     E = {:.10f}    ({:.10f})".format(
                i + 1, root_sym[i], E0[i], E0[i] + ints["Escf"]
            )
        )
...
def flatten_R(r1a, r1b, r2a, r2b, r2c, order="C"):
    ...
    return np.concatenate(
        (
            r1a.flatten(order=order),
            r1b.flatten(order=order),
            r2a.flatten(order=order),
            r2b.flatten(order=order),
            r2c.flatten(order=order),
        ),
        axis=0,
    )
        ...
        if idx[n] < n2a:
            ...
            print(
                "      [{}]     {}A  {}A  ->  {}A  {}A    {:.6f}".format(
                    ...
                    i + sys["Nfroz"] + 1,
                    j + sys["Nfroz"] + 1,
                    a + sys["Nocc_a"] + sys["Nfroz"] + 1,
                    b + sys["Nocc_a"] + sys["Nfroz"] + 1,
                    R2[idx[n]],
                )
            )
        elif idx[n] < n2a + n2b:
