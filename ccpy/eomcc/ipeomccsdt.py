...
Method with 1h, 2h-1p, and 3h-2p Excitations on top of CCSDT [IP-EOMCCSDT(3h-2p)]"""
import numpy as np
...
def HR(dR, R, T, H, flag_RHF, system):
    # Get intermediates
    X = get_ipeomccsdt_intermediates(H, R)
    # update R1
...
def build_HR_2A(R, T, X, H):
    ...
    X2A += 0.5 * ccpy_einsum("e,ebij->ibj", X["a"]["v"], T.aa)
    ...
    # X2A += 0.25 * ccpy_einsum("fne,ebfijn->ibj", X["aa"]["vov"], T.aaa)
    # X2A += 0.5 * ccpy_einsum("fne,ebfijn->ibj", X["ab"]["vov"], T.aab)
...
def build_HR_2B(R, T, X, H):
    ...
    X2B += ccpy_einsum("e,ebij->ibj", X["a"]["v"], T.ab)
    # additional terms with R3
    ...
    # terms with T3 -> Don't include; they are part of h(vvov)*R1 and h(vooo)*R1, which are taken care of
    # X2B += 0.5 * ccpy_einsum("fne,efbinj->ibj", X["aa"]["vov"], T.aab)
    # X2B += ccpy_einsum("fne,ebfijn->ibj", X["ab"]["vov"], T.abb)
    return X2B
