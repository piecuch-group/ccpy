...
def left_ccsd(
    ...
    for iroot in range(nroot + 1):
        ...
        while it_micro < maxitval:
            ...
            l1a, l1b, l2a, l2b, l2c = cc_loops.cc_loops.update_l(
                cc_t["l1a"][iroot],
                cc_t["l1b"][iroot],
                cc_t["l2a"][iroot],
                cc_t["l2b"][iroot],
            ...
            print(
                "   {}       {:.10f}        {:.10f}        {:.10f}".format(
                    it_micro, resid, err_energy, E_lcc
                )
            )
        ...
        if flag_conv:
            print(
                "Left-CCSD successfully converged! ({:0.2f}m  {:0.2f}s)".format(
                    minutes, seconds
                )
            )
        else:
        ...
        if iroot != 0:
            LR = 0.0
            LR += np.einsum(
                "em,em->", cc_t["r1a"][iroot - 1], cc_t["l1a"][iroot], optimize=True
            )
            LR += np.einsum(
                "em,em->", cc_t["r1b"][iroot - 1], cc_t["l1b"][iroot], optimize=True
            )
            LR += 0.25 * np.einsum(
                "efmn,efmn->", cc_t["r2a"][iroot - 1], cc_t["l2a"][iroot], optimize=True
            )
            LR += np.einsum(
                "efmn,efmn->", cc_t["r2b"][iroot - 1], cc_t["l2b"][iroot], optimize=True
            )
            LR += 0.25 * np.einsum(
                "efmn,efmn->", cc_t["r2c"][iroot - 1], cc_t["l2c"][iroot], optimize=True
            )
...
def build_LH_1B(cc_t, H1A, H1B, H2A, H2B, H2C, iroot, ints, sys):
    ...
    X1B += (
        np.einsum("ge,eiga->ia", I1, H2C["vovv"], optimize=True)
    ...
    X1B += (
        np.einsum("ge,eiga->ia", I1, H2B["vovv"], optimize=True)
...
def build_LH_2A(cc_t, H1A, H1B, H2A, H2B, H2C, iroot, ints, sys):
    ...
    X2A += np.einsum("ea,ebij->ijab", H1A["vv"], l2a, optimize=True) - np.einsum(
        "eb,eaij->ijab", H1A["vv"], l2a, optimize=True
    )
    X2A += -np.einsum("im,abmj->ijab", H1A["oo"], l2a, optimize=True) + np.einsum(
        "jm,abmi->ijab", H1A["oo"], l2a, optimize=True
    )
    X2A += (
        np.einsum("jb,ai->ijab", H1A["ov"], l1a, optimize=True)
    ...
    X2A += -0.5 * np.einsum(
        "ea,ijeb->ijab", I1, vA["oovv"], optimize=True
    ...
    X2A += -np.einsum("ea,ijeb->ijab", I1, vA["oovv"], optimize=True) + np.einsum(
        "eb,ijea->ijab", I2, vA["oovv"], optimize=True
    )

    I1 = np.einsum("efin,efmn->im", l2a, t2a, optimize=True)
    I2 = np.einsum("efjn,efmn->jm", l2a, t2a, optimize=True)
    X2A += -0.5 * np.einsum(
        "im,mjab->ijab", I1, vA["oovv"], optimize=True
    ...
    X2A += -np.einsum("im,mjab->ijab", I1, vA["oovv"], optimize=True) + np.einsum(
        "jm,miab->ijab", I2, vA["oovv"], optimize=True
    )

    X2A += (
        np.einsum("eima,ebmj->ijab", H2A["voov"], l2a, optimize=True)
    ...
    X2A += (
        +np.einsum("ieam,bejm->ijab", H2B["ovvo"], l2b, optimize=True)
    ...
    X2A += np.einsum("ejab,ei->ijab", H2A["vovv"], l1a, optimize=True) - np.einsum(
        "eiab,ej->ijab", H2A["vovv"], l1a, optimize=True
    )
    X2A += -np.einsum("ijmb,am->ijab", H2A["ooov"], l1a, optimize=True) + np.einsum(
        "ijma,bm->ijab", H2A["ooov"], l1a, optimize=True
    )
