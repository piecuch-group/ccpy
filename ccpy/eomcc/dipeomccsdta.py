...
def HR(dR, R, T, H, flag_RHF, system):
    ...
    ### TYPE: OOOV
    H0["aa"]["ooov"] = H.aa.ooov - ccpy_einsum("mnfe,fi->mnie", H.aa.oovv, T.a)
    if flag_RHF:
        H0["bb"]["ooov"] = H0["aa"]["ooov"]
    else:
        H0["bb"]["ooov"] = H.bb.ooov - ccpy_einsum("mnfe,fi->mnie", H.bb.oovv, T.b)
    H0["ab"]["ooov"] = H.ab.ooov - ccpy_einsum("mnfe,fi->mnie", H.ab.oovv, T.a)
    H0["ab"]["oovo"] = H.ab.oovo - ccpy_einsum("nmef,fi->nmei", H.ab.oovv, T.b)
    ### TYPE: VOVV
    H0["aa"]["vovv"] = H.aa.vovv + ccpy_einsum("mnfe,an->amef", H.aa.oovv, T.a)
    if flag_RHF:
        H0["bb"]["vovv"] = H0["aa"]["vovv"]
    else:
        H0["bb"]["vovv"] = H.bb.vovv + ccpy_einsum("mnfe,an->amef", H.bb.oovv, T.b)
    H0["ab"]["vovv"] = H.ab.vovv + ccpy_einsum("nmef,an->amef", H.ab.oovv, T.a) 
    H0["ab"]["ovvv"] = H.ab.ovvv + ccpy_einsum("mnef,an->maef", H.ab.oovv, T.b)
    # -------------------------------------------------------------------------
