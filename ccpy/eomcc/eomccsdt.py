...
def eomccsdt(
    ...
    Rvec, omega, is_converged = davidson(
        HR_func, update_R_func, B0, E0, maxit, maxit, tol
    )
    ...
    for i in range(len(omega)):
        ...
        print(
            "   Root - {}    E = {}    omega = {:.10f}    r0 = {:.10f}    [{}]".format(
                i + 1, omega[i] + Eccsdt, omega[i], r0, tmp
            )
        )
...
def update_R(r, omega, H1A_oo, H1A_vv, H1B_oo, H1B_vv, sys):
    ...
    r1a, r1b, r2a, r2b, r2c, r3a, r3b, r3c, r3d = cc_loops.cc_loops.update_r_ccsdt(
        r1a,
        r1b,
        r2a,
        r2b,
...
def unflatten_R(R, sys, order="C"):
    ...
    idx_3b = slice(
        n1a + n1b + n2a + n2b + n2c + n3a, n1a + n1b + n2a + n2b + n2c + n3a + n3b
    )
    idx_3c = slice(
        n1a + n1b + n2a + n2b + n2c + n3a + n3b,
        n1a + n1b + n2a + n2b + n2c + n3a + n3b + n3c,
    )
    idx_3d = slice(
        n1a + n1b + n2a + n2b + n2c + n3a + n3b + n3c,
        n1a + n1b + n2a + n2b + n2c + n3a + n3b + n3c + n3d,
    )
    ...
    r2a = np.reshape(
        R[idx_2a],
        (sys["Nunocc_a"], sys["Nunocc_a"], sys["Nocc_a"], sys["Nocc_a"]),
        order=order,
    )
    r2b = np.reshape(
        R[idx_2b],
...
def build_HR_2A(
    ...
    I1 = np.einsum("mnef,fn->me", vA["oovv"], r1a, optimize=True) + np.einsum(
        "mnef,fn->me", vB["oovv"], r1b, optimize=True
    )
    X2A += np.einsum("me,abeijm->abij", I1, t3a, optimize=True)

    I1 = np.einsum("nmfe,fn->me", vB["oovv"], r1a, optimize=True) + np.einsum(
        "mnef,fn->me", vC["oovv"], r1b, optimize=True
    )
...
def build_HR_3A(
    ...
    I3 = -0.5 * np.einsum(
        "mnef,abfimn->baei", vA["oovv"], r3a, optimize=True
    ...
    I3 = 0.5 * np.einsum(
        "mnef,efcjnk->mcjk", vA["oovv"], r3a, optimize=True
    ...
    I1 = (
        -1.0 * np.einsum("me,bm->be", H1A["ov"], r1a, optimize=True)
        + np.einsum("bnef,fn->be", H2A["vovv"], r1a, optimize=True)
        + np.einsum("bnef,fn->be", H2B["vovv"], r1b, optimize=True)
    )
    I2 = -0.5 * np.einsum("mnef,bfmn->be", vA["oovv"], r2a, optimize=True) - np.einsum(
        "mnef,bfmn->be", vB["oovv"], r2b, optimize=True
    )
    X3A += (1.0 / 12.0) * np.einsum(
        "be,aecijk->abcijk", I1 + I2, t3a, optimize=True
    )  # A(b/ac)

    I1 = (
        np.einsum("me,ej->mj", H1A["ov"], r1a, optimize=True)
        + np.einsum("mnjf,fn->mj", H2A["ooov"], r1a, optimize=True)
        + np.einsum("mnjf,fn->mj", H2B["ooov"], r1b, optimize=True)
    )
    I2 = 0.5 * np.einsum("mnef,efjn->mj", vA["oovv"], r2a, optimize=True) + np.einsum(
        "mnef,efjn->mj", vB["oovv"], r2b, optimize=True
    )
    X3A -= (1.0 / 12.0) * np.einsum(
        "mj,abcimk->abcijk", I1 + I2, t3a, optimize=True
    )  # A(j/ik)
    ...
    X3A += (1.0 / 24.0) * np.einsum(
        "mnij,abcmnk->abcijk", I1 + I2, t3a, optimize=True
    )  # A(k/ij)
    ...
    X3A += (1.0 / 24.0) * np.einsum(
        "abef,efcijk->abcijk", I1 + I2, t3a, optimize=True
    )  # A(c/ab)

    I1 = -1.0 * np.einsum("nmje,bn->bmje", H2A["ooov"], r1a, optimize=True) + np.einsum(
        "bmfe,fj->bmje", H2A["vovv"], r1a, optimize=True
    )
    I2 = np.einsum("mnef,fcnk->cmke", vA["oovv"], r2a, optimize=True) + np.einsum(
        "mnef,cfkn->cmke", vB["oovv"], r2b, optimize=True
    )
    X3A += 0.25 * np.einsum(
        "bmje,aecimk->abcijk", I1 + I2, t3a, optimize=True
    )  # A(j/ik)A(b/ac)

    I1 = -1.0 * np.einsum("nmje,bn->bmje", H2B["ooov"], r1a, optimize=True) + np.einsum(
        "bmfe,fj->bmje", H2B["vovv"], r1a, optimize=True
    )
    I2 = np.einsum("nmfe,fcnk->cmke", vB["oovv"], r2a, optimize=True) + np.einsum(
        "mnef,cfkn->cmke", vC["oovv"], r2b, optimize=True
    )
    X3A += 0.25 * np.einsum(
        "bmje,aceikm->abcijk", I1 + I2, t3b, optimize=True
    )  # A(j/ik)A(b/ac)
    ...
    X3A += (1.0 / 24.0) * np.einsum(
        "mnij,abcmnk->abcijk", H2A["oooo"], r3a, optimize=True
    )
    X3A += (1.0 / 24.0) * np.einsum(
        "abef,efcijk->abcijk", H2A["vvvv"], r3a, optimize=True
    )
...
def build_HR_3B(
    ...
    Q1 = np.einsum("mnef,fn->me", vA["oovv"], r1a, optimize=True) + np.einsum(
        "mnef,fn->me", vB["oovv"], r1b, optimize=True
    )
    Q2 = np.einsum("nmfe,fn->me", vB["oovv"], r1a, optimize=True) + np.einsum(
        "nmfe,fn->me", vC["oovv"], r1b, optimize=True
    )
    ...
    I1 = -0.5 * np.einsum(
        "mnef,bfcmnk->bcek", vA["oovv"], r3b, optimize=True
    ...
    I1 = 0.5 * np.einsum(
        "mnef,efcjnk->mcjk", vA["oovv"], r3b, optimize=True
    ...
    Int3 = np.einsum(
        "amje,bm->abej", H2A["voov"], r1a, optimize=True
    )  # (*) flipped sign to use H2A(voov) instead of H2A(vovo)
    Int3 += 0.5 * np.einsum(
        "abfe,ej->abfj", H2A["vvvv"], r1a, optimize=True
    )  # (*) added factor 1/2 to compensate A(ab)
    Int3 += 0.25 * np.einsum(
        "nmje,abmn->abej", H2A["ooov"], r2a, optimize=True
    )  # (*) added factor 1/2 to compensate A(ab)
    ...
    I1 = -0.5 * np.einsum(
        "mnef,abfmjn->abej", vA["oovv"], r3a, optimize=True
    ...
    Int4 = -0.5 * np.einsum(
        "nmij,bm->bnji", H2A["oooo"], r1a, optimize=True
    )  # (*) added factor 1/2 to compenate A(ij)
    Int4 -= np.einsum(
        "bmie,ej->bmji", H2A["voov"], r1a, optimize=True
    )  # (*) flipped sign to use H2A(voov) instead of H2A(vovo)
    Int4 += 0.25 * np.einsum(
        "bmfe,efij->bmji", H2A["vovv"], r2a, optimize=True
    ...
    I1 = 0.5 * np.einsum(
        "mnef,aefijn->amij", vA["oovv"], r3a, optimize=True
    ...
    I1 = -1.0 * np.einsum(
        "nmfe,bfcjnm->bcje", vB["oovv"], r3b, optimize=True
    ...
    I1 = np.einsum(
        "nmfe,bfejnk->bmjk", vB["oovv"], r3b, optimize=True
    ...
    I1 = (
        np.einsum("me,ek->mk", H1B["ov"], r1b, optimize=True)
        + np.einsum("nmfk,fn->mk", H2B["oovo"], r1a, optimize=True)
        + np.einsum("mnkf,fn->mk", H2C["ooov"], r1b, optimize=True)
    )
    I2 = np.einsum("nmfe,fenk->mk", vB["oovv"], r2b, optimize=True) + 0.5 * np.einsum(
        "mnef,efkn->mk", vC["oovv"], r2c, optimize=True
    )
    X3B -= 0.25 * np.einsum("mk,abcijm->abcijk", I1 + I2, t3b, optimize=True)

    I1 = np.einsum("nmje,ek->nmjk", H2B["ooov"], r1b, optimize=True) + np.einsum(
        "nmek,ej->nmjk", H2B["oovo"], r1a, optimize=True
    )
    ...
    I1 = -1.0 * np.einsum("bmfe,cm->bcfe", H2B["vovv"], r1b, optimize=True) - np.einsum(
        "mcfe,bm->bcfe", H2B["ovvv"], r1a, optimize=True
    )
    ...
    I1 = -1.0 * np.einsum("nmfk,cm->ncfk", H2B["oovo"], r1b, optimize=True) + np.einsum(
        "ncfe,ek->ncfk", H2B["ovvv"], r1b, optimize=True
    )
    I2 = np.einsum("mnef,ecmk->ncfk", vA["oovv"], r2b, optimize=True) + np.einsum(
        "nmfe,ecmk->ncfk", vB["oovv"], r2c, optimize=True
    )
    X3B += 0.25 * np.einsum("ncfk,abfijn->abcijk", I1 + I2, t3a, optimize=True)

    I1 = -1.0 * np.einsum("mnkf,cm->cnkf", H2C["ooov"], r1b, optimize=True) + np.einsum(
        "cnef,ek->cnkf", H2C["vovv"], r1b, optimize=True
    )
    I2 = np.einsum("mnef,ecmk->cnkf", vB["oovv"], r2b, optimize=True) + np.einsum(
        "mnef,ecmk->cnkf", vC["oovv"], r2c, optimize=True
    )
    X3B += 0.25 * np.einsum("cnkf,abfijn->abcijk", I1 + I2, t3b, optimize=True)

    I1 = np.einsum("bmfe,ek->bmfk", H2B["vovv"], r1b, optimize=True) - np.einsum(
        "nmfk,bn->bmfk", H2B["oovo"], r1a, optimize=True
    )
    I2 = -1.0 * np.einsum("mnef,bfmk->bnek", vB["oovv"], r2b, optimize=True)
    X3B -= 0.5 * np.einsum("bmfk,afcijm->abcijk", I1 + I2, t3b, optimize=True)

    I1 = -1.0 * np.einsum("nmje,cm->ncje", H2B["ooov"], r1b, optimize=True) + np.einsum(
        "ncfe,fj->ncje", H2B["ovvv"], r1a, optimize=True
    )
    ...
    I2 = np.einsum("mnef,aeim->anif", vA["oovv"], r2a, optimize=True) + np.einsum(
        "nmfe,aeim->anif", vB["oovv"], r2b, optimize=True
    )
    ...
    I2 = np.einsum("mnef,aeim->anif", vB["oovv"], r2a, optimize=True) + np.einsum(
        "mnef,aeim->anif", vC["oovv"], r2b, optimize=True
    )
...
def build_HR_3C(
    ...
    Q1 = np.einsum("mnef,fn->me", vC["oovv"], r1b, optimize=True) + np.einsum(
        "nmfe,fn->me", vB["oovv"], r1a, optimize=True
    )
    Q2 = np.einsum("mnef,fn->me", vB["oovv"], r1b, optimize=True) + np.einsum(
        "nmfe,fn->me", vA["oovv"], r1a, optimize=True
    )
    ...
    I1 = -0.5 * np.einsum(
        "mnef,cfbknm->cbke", vC["oovv"], r3c, optimize=True
    ...
    I1 = 0.5 * np.einsum(
        "mnef,cfeknj->cmkj", vC["oovv"], r3c, optimize=True
    ...
    Int3 = np.einsum(
        "amje,bm->abej", H2C["voov"], r1b, optimize=True
    )  # (*) flipped sign to use H2A(voov) instead of H2A(vovo)
    Int3 += 0.5 * np.einsum(
        "abfe,ej->abfj", H2C["vvvv"], r1b, optimize=True
    )  # (*) added factor 1/2 to compensate A(ab)
    Int3 += 0.25 * np.einsum(
        "nmje,abmn->abej", H2C["ooov"], r2c, optimize=True
    )  # (*) added factor 1/2 to compensate A(ab)
    ...
    Int3 -= np.transpose(Int3, (1, 0, 2, 3))
    I1 = -0.5 * np.einsum(
        "mnef,abfmjn->abej", vC["oovv"], r3d, optimize=True
    ...
    Int4 = -0.5 * np.einsum(
        "nmij,bm->bnji", H2C["oooo"], r1b, optimize=True
    )  # (*) added factor 1/2 to compenate A(ij)
    Int4 -= np.einsum(
        "bmie,ej->bmji", H2C["voov"], r1b, optimize=True
    )  # (*) flipped sign to use H2A(voov) instead of H2A(vovo)
    Int4 += 0.25 * np.einsum(
        "bmfe,efij->bmji", H2C["vovv"], r2c, optimize=True
    )  # (*) added factor 1/2 to compensate A(ij)
    ...
    Int4 -= np.transpose(Int4, (0, 1, 3, 2))
    I1 = 0.5 * np.einsum(
        "mnef,aefijn->amij", vC["oovv"], r3d, optimize=True
    ...
    I1 = -1.0 * np.einsum(
        "mnef,cfbmnj->cbej", vB["oovv"], r3c, optimize=True
    ...
    I1 = np.einsum(
        "mnef,efbknj->mbkj", vB["oovv"], r3c, optimize=True
    ...
    I1 = -1.0 * np.einsum("mbef,cm->cbef", H2B["ovvv"], r1a, optimize=True) - np.einsum(
        "cmef,bm->cbef", H2B["vovv"], r1b, optimize=True
    )
    ...
    I1 = -1.0 * np.einsum("mnkf,cm->cnkf", H2B["ooov"], r1a, optimize=True) + np.einsum(
        "cnef,ek->cnkf", H2B["vovv"], r1a, optimize=True
    )
    I2 = np.einsum("mnef,cekm->cnkf", vC["oovv"], r2b, optimize=True) + np.einsum(
        "mnef,ecmk->cnkf", vB["oovv"], r2a, optimize=True
    )
    X3C += 0.25 * np.einsum("cnkf,abfijn->cbakji", I1 + I2, t3d, optimize=True)

    I1 = -1.0 * np.einsum("mnkf,cm->ncfk", H2A["ooov"], r1a, optimize=True) + np.einsum(
        "cnef,ek->ncfk", H2A["vovv"], r1a, optimize=True
    )
    I2 = np.einsum("nmfe,cekm->ncfk", vB["oovv"], r2b, optimize=True) + np.einsum(
        "mnef,ecmk->ncfk", vA["oovv"], r2a, optimize=True
    )
    X3C += 0.25 * np.einsum("ncfk,fbanji->cbakji", I1 + I2, t3c, optimize=True)

    I1 = np.einsum("mbef,ek->mbkf", H2B["ovvv"], r1a, optimize=True) - np.einsum(
        "mnkf,bn->mbkf", H2B["ooov"], r1b, optimize=True
    )
    I2 = -1.0 * np.einsum("nmfe,fbkm->nbke", vB["oovv"], r2b, optimize=True)
    X3C -= 0.5 * np.einsum("mbkf,cfamji->cbakji", I1 + I2, t3c, optimize=True)

    I1 = -1.0 * np.einsum("mnej,cm->cnej", H2B["oovo"], r1a, optimize=True) + np.einsum(
        "cnef,fj->cnej", H2B["vovv"], r1b, optimize=True
    )
    I2 = -1.0 * np.einsum("nmfe,cenj->cmfj", vB["oovv"], r2b, optimize=True)
    X3C -= 0.5 * np.einsum("cnej,ebakni->cbakji", I1 + I2, t3c, optimize=True)

    I1 = -1.0 * np.einsum("nmje,bn->mbej", H2C["ooov"], r1b, optimize=True) + np.einsum(
        "bmfe,fj->mbej", H2C["vovv"], r1b, optimize=True
    )
    I2 = np.einsum("mnef,aeim->nafi", vC["oovv"], r2c, optimize=True) + np.einsum(
        "mnef,eami->nafi", vB["oovv"], r2b, optimize=True
    )
    X3C += np.einsum("mbej,ceakmi->cbakji", I1 + I2, t3c, optimize=True)

    I1 = -1.0 * np.einsum("mnej,bn->mbej", H2B["oovo"], r1b, optimize=True) + np.einsum(
        "mbef,fj->mbej", H2B["ovvv"], r1b, optimize=True
    )
    I2 = np.einsum("nmfe,aeim->nafi", vB["oovv"], r2c, optimize=True) + np.einsum(
        "mnef,eami->nafi", vA["oovv"], r2b, optimize=True
    )
...
def build_HR_3D(
    ...
    I3 = -0.5 * np.einsum(
        "mnef,abfimn->baei", vC["oovv"], r3d, optimize=True
    ) - np.einsum("nmfe,fbanmi->baei", vB["oovv"], r3c, optimize=True)
    X3D = 0.25 * np.einsum(
        "abej,ecik->abcijk", I1 + I2 + I3, cc_t["t2c"], optimize=True
    )
    ...
    I3 = 0.5 * np.einsum(
        "mnef,efcjnk->mcjk", vC["oovv"], r3d, optimize=True
    ) + np.einsum("nmfe,fcenkj->mcjk", vB["oovv"], r3c, optimize=True)
    X3D -= 0.25 * np.einsum(
        "mbij,acmk->abcijk", I1 + I2 + I3, cc_t["t2c"], optimize=True
    )
    ...
    X3D += (1.0 / 12.0) * np.einsum(
        "be,aecijk->abcijk", I1 + I2, t3d, optimize=True
    )  # A(b/ac)
    ...
    X3D -= (1.0 / 12.0) * np.einsum(
        "mj,abcimk->abcijk", I1 + I2, t3d, optimize=True
    )  # A(j/ik)
    ...
    X3D += (1.0 / 24.0) * np.einsum(
        "mnij,abcmnk->abcijk", I1 + I2, t3d, optimize=True
    )  # A(k/ij)
    ...
    I2 = 0.5 * np.einsum("mnef,abmn->abef", vC["oovv"], r2c, optimize=True)
    X3D += (1.0 / 24.0) * np.einsum(
        "abef,efcijk->abcijk", I1 + I2, t3d, optimize=True
    )  # A(c/ab)

    I1 = -1.0 * np.einsum("nmje,bn->bmje", H2C["ooov"], r1b, optimize=True) + np.einsum(
        "bmfe,fj->bmje", H2C["vovv"], r1b, optimize=True
    )
    I2 = np.einsum("mnef,fcnk->cmke", vC["oovv"], r2c, optimize=True) + np.einsum(
        "nmfe,fcnk->cmke", vB["oovv"], r2b, optimize=True
    )
    X3D += 0.25 * np.einsum(
        "bmje,aecimk->abcijk", I1 + I2, t3d, optimize=True
    )  # A(j/ik)A(b/ac)

    I1 = -1.0 * np.einsum("mnej,bn->bmje", H2B["oovo"], r1b, optimize=True) + np.einsum(
        "mbef,fj->bmje", H2B["ovvv"], r1b, optimize=True
    )
    I2 = np.einsum("mnef,fcnk->cmke", vB["oovv"], r2c, optimize=True) + np.einsum(
        "mnef,fcnk->cmke", vA["oovv"], r2b, optimize=True
    )
    X3D += 0.25 * np.einsum(
        "bmje,ecamki->abcijk", I1 + I2, t3c, optimize=True
    )  # A(j/ik)A(b/ac)
    ...
    X3D += (1.0 / 24.0) * np.einsum(
        "mnij,abcmnk->abcijk", H2C["oooo"], r3d, optimize=True
    )
    X3D += (1.0 / 24.0) * np.einsum(
        "abef,efcijk->abcijk", H2C["vvvv"], r3d, optimize=True
    )
