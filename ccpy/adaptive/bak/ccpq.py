...
def calc_adaptive_ccpq_norelax(
    sys,
    ints,
    tot_triples,
    workdir,
    ...
    p_spaces_ccsd = {
        "A": np.zeros((nua, nua, nua, noa, noa, noa)),
    ...
    cc_t_ccsd, Eccsd = ccsd(
        sys, ints, shift=ccshift, maxit=ccmaxit, tol=tol, diis_size=diis_size
    )
    if flag_pert_triples:
        _, mcA_ccsd, mcB_ccsd, mcC_ccsd, mcD_ccsd = ccp3_pertT(
            cc_t_ccsd, p_spaces_ccsd, ints, sys, flag_RHF=isRHF
        )
    else:
        ...
        _, mcA_ccsd, mcB_ccsd, mcC_ccsd, mcD_ccsd = ccp3(
            cc_t_ccsd, p_spaces_ccsd, H1A, H1B, H2A, H2B, H2C, ints, sys, flag_RHF=isRHF
        )
    ...
    for ncnt in range(num_calcs):
        ...
        p_spaces, num_triples_2 = selection_function(
            sys,
            mcA_ccsd,
            mcB_ccsd,
            mcC_ccsd,
        ...
        # Left CC and HBar
        if flag_pert_triples:
            Eccpq[ncnt], _, _, _, _ = ccp3_pertT(
                cc_t, p_spaces, ints, sys, flag_RHF=isRHF
            )
        else:
            ...
            Eccpq[ncnt], _, _, _, _ = ccp3(
                cc_t, p_spaces, H1A, H1B, H2A, H2B, H2C, ints, sys, flag_RHF=isRHF
            )

        print("SUMMARY AT TRIPLES PERCENTAGE - {}".format(triples_percentages[ncnt]))
        print(
            "NUMBER OF TRIPLES IN P SPACE - {}/{}  ({}%)".format(
                num_triples, num_triples_2, num_triples / tot_triples * 100
            )
        )
...
def calc_adaptive_ccpq(
    ...
    if restart_dir is not None:
        ...
        num_triples = count_triples_in_P(p_spaces)
        print(
            "    NUMBER OF TRIPLES in P SPACE OF THIS DIRECTORY - {}".format(
                num_triples
            )
        )
        ...
        Ecorr += 0.5 * np.einsum(
            "mnef,fn,em->", ints["vA"]["oovv"], t1a, t1a, optimize=True
        )
        Ecorr += 0.5 * np.einsum(
            "mnef,fn,em->", ints["vC"]["oovv"], t1b, t1b, optimize=True
        )
        ...
        if flag_pert_triples:
            Eccpq_rest, mcA, mcB, mcC, mcD = ccp3_pertT(
                cc_t, p_spaces, ints, sys, flag_RHF=isRHF
            )
        else:
            ...
            Eccpq_rest, mcA, mcB, mcC, mcD = ccp3(
                cc_t, p_spaces, H1A, H1B, H2A, H2B, H2C, ints, sys, flag_RHF=isRHF
            )
        ...
        print(
            "NUMBER OF TRIPLES IN P SPACE - {}  ({}%)".format(
                num_triples, num_triples / tot_triples * 100
            )
        )
    ...
    for niter in range(niter0, niter0 + maxit):
        ...
        p_spaces, _ = selection_function(
            sys, mcA, mcB, mcC, mcD, p_spaces, num_add, flag_RHF=isRHF
        )
        ...
        if niter > 0:
            cc_t, Eccp[niter - niter0] = ccsdt_p(
                sys,
                ints,
                p_spaces,
                maxit=ccmaxit,
        ...
        if flag_pert_triples:
            Eccpq[niter - niter0], mcA, mcB, mcC, mcD = ccp3_pertT(
                cc_t, p_spaces, ints, sys, flag_RHF=isRHF
            )
        else:
            # Calculate HBar using 2BA
            ...
            # Calculate CC(P;Q) moment correction using 2BA
            Eccpq[niter - niter0], mcA, mcB, mcC, mcD = ccp3(
                cc_t, p_spaces, H1A, H1B, H2A, H2B, H2C, ints, sys, flag_RHF=isRHF
            )
...
def calc_adaptive_ccpq_depreciated(
    sys,
    ints,
    tot_triples,
    workdir,
    ...
    if restart_dir is not None:
        ...
        num_triples = count_triples_in_P(p_spaces)
        print(
            "NUMBER OF TRIPLES in P SPACE OF THIS DIRECTORY - {}\n\n".format(
                num_triples
            )
        )
        print(
            "NOTE: THIS NUMBER CORRESPONDS TO THE P SPACE FOR THE **NEXT** ITERATION!!! - MACRO ITERATION {}".format(
                niter0 + 1
            )
        )
        niter0 += 1
    ...
    for niter in range(niter0, niter0 + maxit):
        ...
        add_1perc = int(tot_triples * 0.01)
        num_add = int(growth_percentage * add_1perc)
        p_spaces, num_add = selection_function(
            sys, mcA, mcB, mcC, mcD, p_spaces, num_add, flag_RHF=isRHF
        )
    ...
    if not flag_RHF:
        ...
        while ct < num_add:
            ...
            if idx[ct2] < n3a:
                ...
                a, b, c, i, j, k = np.unravel_index(
                    idx[ct2] - n3a - n3b - n3c, mcD.shape
                )
        ...
        while ct < num_add:
            if idx[ct2] < n3a:
                a, b, c, i, j, k = np.unravel_index(idx[ct2], mcA.shape)
                if (
                    p_spaces["A"][a, b, c, i, j, k] == 1
                    or p_spaces["D"][a, b, c, i, j, k] == 1
                ):
                    ct2 += 1
                    continue
                else:
                ...
                a, b, c, i, j, k = np.unravel_index(idx[ct2] - n3a, mcB.shape)
                if (
                    p_spaces["B"][a, b, c, i, j, k] == 1
                    or p_spaces["C"][c, a, b, k, i, j] == 1
                ):
                    ct2 += 1
                    continue
                else:
