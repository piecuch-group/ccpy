...
def ccsdt_p(
    ...
    while it_micro < maxit:
        ...
        cc_t = update_t3a(
            cc_t, ints, p_spaces["A"], H1A, H1B, H2A, H2B, H2C, sys, shift
        )
        cc_t = update_t3b(
            cc_t, ints, p_spaces["B"], H1A, H1B, H2A, H2B, H2C, sys, shift
        )
        if flag_RHF:
            ...
            cc_t = update_t3c(
                cc_t, ints, p_spaces["C"], H1A, H1B, H2A, H2B, H2C, sys, shift
            )
            cc_t = update_t3d(
                cc_t, ints, p_spaces["D"], H1A, H1B, H2A, H2B, H2C, sys, shift
            )
...
def update_t3a(cc_t, ints, p_space, H1A, H1B, H2A, H2B, H2C, sys, shift):
    ...
    M23_D1 += (
        -np.einsum("abcijk->abckji", M23_D1, optimize=True)
    ...
    M23_D2 += (
        -np.einsum("abcijk->abcjik", M23_D2, optimize=True)
    ...
    # A(k/ij)
    D13 = D1 + D3
    D13 += -np.einsum("abcijk->abckji", D13, optimize=True) - np.einsum(
        "abcijk->abcikj", D13, optimize=True
    )

    # A(c/ab)
    D24 = D2 + D4
    D24 += -np.einsum("abcijk->cbaijk", D24, optimize=True) - np.einsum(
        "abcijk->acbijk", D24, optimize=True
    )
...
def update_t3b(cc_t, ints, p_space, H1A, H1B, H2A, H2B, H2C, sys, shift):
    ...
    D_abij += (
        -np.einsum("abcijk->bacijk", D_abij, optimize=True)
    ...
    t3b = cc_loops.cc_loops.update_t3b_p(
        t3b, X3B, p_space, fA["oo"], fA["vv"], fB["oo"], fB["vv"], shift
    )
...
def update_t3c(cc_t, ints, p_space, H1A, H1B, H2A, H2B, H2C, sys, shift):
    ...
    D_bcjk += (
        -np.einsum("abcijk->acbijk", D_bcjk, optimize=True)
    ...
    t3c = cc_loops.cc_loops.update_t3c_p(
        t3c, X3C, p_space, fA["oo"], fA["vv"], fB["oo"], fB["vv"], shift
    )
...
def update_t3d(cc_t, ints, p_space, H1A, H1B, H2A, H2B, H2C, sys, shift):
    ...
    D56 = D5 + D6
    D56 += (
        -np.einsum("abcijk->abcjik", D56, optimize=True)
        - np.einsum("abcijk->abckji", D56, optimize=True)
