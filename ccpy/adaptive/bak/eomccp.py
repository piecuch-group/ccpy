...
def eomccsdt_p(
    ...
    if initial_guess == "cis":
        ...
        for i in range(nroot):
            print(
                "Root - {}     E = {:.10f}    ({:.10f})".format(
                    i + 1, E0[i], E0[i] + ints["Escf"]
                )
            )
        print("")
        n_doubles = (
    ...
    for iroot in range(nroot):
        ...
        for it in range(maxit):
            ...
            (
                ...
                q3c,
                q3d,
            ) = cc_loops.cc_loops.update_r_ccsdt_p(
                q1a,
                q1b,
                q2a,
                q2b,
...
def build_HR_3A(
    ...
    D1 = np.einsum("abie,ecjk->abcijk", I1, t2a, optimize=True) - np.einsum(
        "ncjk,abin->abcijk", I2, t2a, optimize=True
    )
    D_ijk_cab += D1
    ...
    D2 = -1.0 * np.einsum("amij,bcmk->abcijk", I1, t2a, optimize=True) + np.einsum(
        "cbke,aeij->abcijk", I2, t2a, optimize=True
    )
    D_kij_abc += D2

    I1 = np.einsum("mnef,fn->me", H2A["oovv"], r1a, optimize=True) + np.einsum(
        "mnef,fn->me", H2B["oovv"], r1b, optimize=True
    )
    ...
    D3 = np.einsum("abif,fcjk->abcijk", I1, t2a, optimize=True) - np.einsum(
        "cnkj,abin->abcijk", I2, t2a, optimize=True
    )
    D_ijk_cab += D3
    ...
    I2 -= np.transpose(I2, (0, 1, 3, 2))
    D4 = np.einsum("abif,fcjk->abcijk", I1, t2a, optimize=True) - np.einsum(
        "cnkj,abin->abcijk", I2, t2a, optimize=True
    )
    D_ijk_cab += D4
    ...
    I1 = -0.5 * np.einsum(
        "mnef,abfimn->abie", vA["oovv"], r3a, optimize=True
    ...
    D_ijk_cab -= np.transpose(D_ijk_cab, (2, 1, 0, 3, 4, 5)) + np.transpose(
        D_ijk_cab, (0, 2, 1, 3, 4, 5)
    )
    D_ijk_cab -= np.transpose(D_ijk_cab, (0, 1, 2, 5, 4, 3)) + np.transpose(
        D_ijk_cab, (0, 1, 2, 4, 3, 5)
    )

    D_ijk_abc -= np.transpose(D_ijk_abc, (1, 0, 2, 3, 4, 5)) + np.transpose(
        D_ijk_abc, (2, 1, 0, 3, 4, 5)
    )
    D_ijk_abc -= np.transpose(D_ijk_abc, (0, 1, 2, 5, 4, 3)) + np.transpose(
        D_ijk_abc, (0, 1, 2, 4, 3, 5)
    )

    D_kij_abc -= np.transpose(D_kij_abc, (1, 0, 2, 3, 4, 5)) + np.transpose(
        D_kij_abc, (2, 1, 0, 3, 4, 5)
    )
    D_kij_abc -= np.transpose(D_kij_abc, (0, 1, 2, 5, 4, 3)) + np.transpose(
        D_kij_abc, (0, 1, 2, 3, 5, 4)
    )

    D_kij_cab -= np.transpose(D_kij_cab, (0, 2, 1, 3, 4, 5)) + np.transpose(
        D_kij_cab, (2, 1, 0, 3, 4, 5)
    )
    D_kij_cab -= np.transpose(D_kij_cab, (0, 1, 2, 5, 4, 3)) + np.transpose(
        D_kij_cab, (0, 1, 2, 3, 5, 4)
    )

    D_jik_bac -= np.transpose(D_jik_bac, (0, 2, 1, 3, 4, 5)) + np.transpose(
        D_jik_bac, (1, 0, 2, 3, 4, 5)
    )
    D_jik_bac -= np.transpose(D_jik_bac, (0, 1, 2, 4, 3, 5)) + np.transpose(
        D_jik_bac, (0, 1, 2, 3, 5, 4)
    )

    D_cab -= np.transpose(D_cab, (2, 1, 0, 3, 4, 5)) + np.transpose(
        D_cab, (0, 2, 1, 3, 4, 5)
    )
    D_bac -= np.transpose(D_bac, (0, 2, 1, 3, 4, 5)) + np.transpose(
        D_bac, (1, 0, 2, 3, 4, 5)
    )
    D_jik -= np.transpose(D_jik, (0, 1, 2, 4, 3, 5)) + np.transpose(
        D_jik, (0, 1, 2, 3, 5, 4)
    )
    D_kij -= np.transpose(D_kij, (0, 1, 2, 5, 4, 3)) + np.transpose(
        D_kij, (0, 1, 2, 3, 5, 4)
    )
...
def build_HR_3B(
    ...
    I1A = np.einsum("mnef,fn->me", H2A["oovv"], r1a, optimize=True) + np.einsum(
        "mnef,fn->me", H2B["oovv"], r1b, optimize=True
    )
    I1B = np.einsum("nmfe,fn->me", H2C["oovv"], r1b, optimize=True) + np.einsum(
        "nmfe,fn->me", H2B["oovv"], r1a, optimize=True
    )
    ...
    I1 = np.einsum("mnef,ecmk->ncfk", vA["oovv"], r2b, optimize=True) + np.einsum(
        "nmfe,ecmk->ncfk", vB["oovv"], r2c, optimize=True
    )
    D5 = np.einsum("ncfk,abfijn->abcijk", I1, t3a, optimize=True)
    X3B += D5

    I1 = np.einsum("mnef,ecmk->ncfk", vB["oovv"], r2b, optimize=True) + np.einsum(
        "mnef,ecmk->ncfk", vC["oovv"], r2c, optimize=True
    )
    D6 = np.einsum("ncfk,abfijn->abcijk", I1, t3b, optimize=True)
    X3B += D6

    I1 = np.einsum("mnef,aeim->anif", vA["oovv"], r2a, optimize=True) + np.einsum(
        "nmfe,aeim->anif", vB["oovv"], r2b, optimize=True
    )
    D7 = np.einsum("anif,bfcjnk->abcijk", I1, t3b, optimize=True)
    D_ij_ab += D7

    I1 = np.einsum("mnef,aeim->anif", vB["oovv"], r2a, optimize=True) + np.einsum(
    ...
    I1 = np.einsum("nmfe,fenk->mk", vB["oovv"], r2b, optimize=True) + 0.5 * np.einsum(
    ...
    D_ij_ab -= (
        np.transpose(D_ij_ab, (0, 1, 2, 4, 3, 5))
        + np.transpose(D_ij_ab, (1, 0, 2, 3, 4, 5))
        - np.transpose(D_ij_ab, (1, 0, 2, 4, 3, 5))
    )
