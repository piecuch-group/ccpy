module ipeom3_p_loops
      ...
      contains
              ...
              subroutine build_hr_3a(resid,&
                  ...
                  ! Add on T3 parts
                  !
                  ! diagram 7: A(ijk) x2a_ovo(iem) * t3a(ebcmjk)
                  ! copy over t3a arrays
                  allocate(excits_buff(n3aaa_t,6),amps_buff(n3aaa_t))
                  ...
                  call sort4(excits_buff, amps_buff, loc_arr, idx_table4, (/2,3,5,6/), nua, nua, noa, noa, nloc, n3aaa_t)
                  do idet = 1,n3aaa
                     b = r3a_excits(idet,1); c = r3a_excits(idet,2);
                     i = r3a_excits(idet,3); j = r3a_excits(idet,4); k = r3a_excits(idet,5);
                     ! (1)
                     ...
                     do jdet = loc_arr(1,idx),loc_arr(2,idx)
                        ...
                        ! compute < bcijk | x2a(ovo) | dbcljk >
                        hmatel = x2a_ovo(i,d,l)
                        resid(idet) = resid(idet) + hmatel * amps_buff(jdet)
                     end do
                     ! (ij)
                     ...
                     do jdet = loc_arr(1,idx),loc_arr(2,idx)
                        d = excits_buff(jdet,1); l = excits_buff(jdet,4);
                        ! compute < bcijk | x2a(ovo) | dbclik >
                        hmatel = -x2a_ovo(j,d,l)
                        resid(idet) = resid(idet) + hmatel * amps_buff(jdet)
                     end do
                     end if
                     ! (ik), -
                     ...
                     do jdet = loc_arr(1,idx),loc_arr(2,idx)
                        d = excits_buff(jdet,1); l = excits_buff(jdet,4);
                        ! compute < bcijk | x2a(ovo) | dbclij >
                        hmatel = x2a_ovo(k,d,l)
                        resid(idet) = resid(idet) + hmatel * amps_buff(jdet)
                     end do
                     end if
                  end do
                  ...
                  call sort4(excits_buff, amps_buff, loc_arr, idx_table4, (/2,3,4,6/), nua, nua, noa, noa, nloc, n3aaa_t)
                  !!! SB: (2,3,4,5) !!!
                  call get_index_table4(idx_table4, (/2,nua-1/), (/-1,nua/), (/1,noa-2/), (/-1,noa-1/), nua, nua, noa, noa)
                  call sort4(excits_buff, amps_buff, loc_arr, idx_table4, (/2,3,4,5/), nua, nua, noa, noa, nloc, n3aaa_t)
                  !!! SB: (1,3,5,6) !!!
                  call get_index_table4(idx_table4, (/1,nua-2/), (/-2,nua/), (/2,noa-1/), (/-1,noa/), nua, nua, noa, noa)
                  call sort4(excits_buff, amps_buff, loc_arr, idx_table4, (/1,3,5,6/), nua, nua, noa, noa, nloc, n3aaa_t)
                  !!! SB: (1,3,4,6) !!!
                  call get_index_table4(idx_table4, (/1,nua-2/), (/-2,nua/), (/1,noa-2/), (/-2,noa/), nua, nua, noa, noa)
                  call sort4(excits_buff, amps_buff, loc_arr, idx_table4, (/1,3,4,6/), nua, nua, noa, noa, nloc, n3aaa_t)
                  !!! SB: (1,3,4,5) !!!
                  call get_index_table4(idx_table4, (/1,nua-2/), (/-2,nua/), (/1,noa-2/), (/-1,noa-1/), nua, nua, noa, noa)
                  call sort4(excits_buff, amps_buff, loc_arr, idx_table4, (/1,3,4,5/), nua, nua, noa, noa, nloc, n3aaa_t)
                  !!! SB: (1,2,5,6) !!!
                  call get_index_table4(idx_table4, (/1,nua-2/), (/-1,nua-1/), (/2,noa-1/), (/-1,noa/), nua, nua, noa, noa)
                  call sort4(excits_buff, amps_buff, loc_arr, idx_table4, (/1,2,5,6/), nua, nua, noa, noa, nloc, n3aaa_t)
                  !!! SB: (1,2,4,6) !!!
                  call get_index_table4(idx_table4, (/1,nua-2/), (/-1,nua-1/), (/1,noa-2/), (/-2,noa/), nua, nua, noa, noa)
                  call sort4(excits_buff, amps_buff, loc_arr, idx_table4, (/1,2,4,6/), nua, nua, noa, noa, nloc, n3aaa_t)
                  !!! SB: (1,2,4,5) !!!
                  call get_index_table4(idx_table4, (/1,nua-2/), (/-1,nua-1/), (/1,noa-2/), (/-1,noa-1/), nua, nua, noa, noa)
                  call sort4(excits_buff, amps_buff, loc_arr, idx_table4, (/1,2,4,5/), nua, nua, noa, noa, nloc, n3aaa_t)
                  ! deallocate sorting arrays
                  deallocate(loc_arr,idx_table4,excits_buff,amps_buff)
                  ! diagram 8: X3A += (3.0 / 12.0) * np.einsum("iem,bcejkm->ibcjk", X["ab"]["ovo"], T.aab, optimize=True) # [2]
                  ! diagram 9: X3A += (2.0 / 24.0) * np.einsum("bef,fecijk->ibcjk", X["aa"]["vvv"], T.aaa, optimize=True) # [3]
                  ! diagram 10: X3A += (1.0 / 12.0) * np.einsum("e,ebcijk->ibcjk", X["a"]["v"], T.aaa, optimize=True)      # [4]
              ...
              subroutine build_hr_3c(resid,&
                  ...
                  real(kind=8), intent(in) :: t3c_amps(n3abb_t), t3d_excits(n3bbb_t)
                  ! Input H  and X arrays
                  real(kind=8), intent(in) :: h1a_oo(noa,noa)
                  real(kind=8), intent(in) :: h1b_oo(nob,nob)
                  real(kind=8), intent(in) :: h1b_vv(nub,nub)
