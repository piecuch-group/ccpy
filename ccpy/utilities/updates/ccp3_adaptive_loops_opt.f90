module ccp3_adaptive_loops_opt
    ...
    contains
             ...
             subroutine crcc23A_p_with_selection(deltaA,deltaB,deltaC,deltaD,&
                        ...
                        integer, intent(inout) :: add_counter
                        !f2py intent(in,out) :: add_counter

                        integer :: i, j, k, a, b, c, nua2, icount, idx(2*num_add)
                        real(kind=8) :: D, temp1, temp2, temp3, LM, X3A(nua,nua,nua), L3A(nua,nua,nua), halfval
                                    ...
                                    do a = 1, nua
                                        do b = a+1, nua
                                            do c = b+1, nua
                                                ...
                                                ! if the array is not full yet, then simply append
                                                if( add_count < 2*num_add ) then
                                                    add_count = add_count + 1
                                                    triples_list(add_count,:) = (/2*a-1,2*b-1,2*c-1,2*i-1,2*j-1,2*k-1/)
                                                else if( add_count == 2*num_add ) then ! array has been filled for the FIRST TIME; re-sort and store halfval
                                                    add_count = add_count + 1
                                                    call argsort(moments, idx)
                                                    halfval = moments(num_add)
                                                else ! array has been filled at least once; we now make determinations on icount 
...
end module ccp3_adaptive_loops_opt
