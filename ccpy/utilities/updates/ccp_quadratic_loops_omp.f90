module ccp_quadratic_loops_omp

    ! As it turns out, the amplitude-driven approach is not conducive to shared-memory parallelism.
    ! The best-case scenario for OpenMP is to parallelize a loop of the form
    ! do idet = 1, n3
    !    resid(idet) = resid(idet) + h(...) * t_amp
    ! end do,
    ! so that no race condition can occur when updating the quantity resid. In other words, the updated
    ! quantity should be a function of the loop variable being parallelized, not some other loop variable.
    ! So OpenMP favors the projection-driven approach because the projection (residual) is what changes,
    ! so we could parallelize an outer loop over over projections. In the amplitude-driven approach,
    ! we are doing the opposite. The outer loop is over t3, which are unchanging, and within, we update all
    ! projection amplitudes that are affected by that t3. So if you split the loop over t3, you will have race
    ! conditions as multiple threads try to update the same residual amplitude.
    !
    ! To apply OpenMP in the expensive triples update, we have three options:
    ! (1) Apply OpenMP over the idet loop and give each thread its own copy of resid (undesirable for triples, but
    !     feasible for doubles and singles).
    ! (2) Re-write these updates in a projection-driven approach (this will require figuring out how to work with
    !     the permutationally unique t3).
    ! (3) Pull out the expensive terms in the amplitude-driven approach and re-order the loops, so that the loops
    !     over projection variables occur first and parallelize these.
    !
    ! Alternative idea proposed by Emiliano:
    ! --------------------------------------
    ! We can keep the amplitude-driven approach and reduce the storage of x3 by creating a hash table array,
    ! hashTable(I,J), where I = hashFunction(i,j,k) and J = hashFunction(a,b,c). The array hashTable is of
    ! dimension no**3 * nu**3 (size of T3), but it can be used so that we only store a 1D array resid(n3), such that
    ! resid(hashTable(hashFunction(i,j,k), hashFunction(a,b,c))) = x3(a,b,c,i,j,k). Therefore, all updatdes
    ...
    !     do a = 1, nua
    !         do b = a+1, nua
    !           if (pspace(b,a,c,i,j,k)) then
    !              I = hashFunction(b,a,c)
    !              J = hashFunction(i,j,k)
    !              nx = hashTable(I,J)
    !              x3b(nx) = x3b(nx) + H2A_vvvv(b,a,e,f) * t_amp ! (1)
    !           end if
    !         end do
    !     end do
    ! end do
    !
    ! This scheme has the advantage that now the residual array for triples is small again. Therefore, performing
    ! OpenMP over the idet loop is feasible as each thread can store its own copy of x3, and we reduce(+) them all
    ! at the end. Also, the hashTable array can have a reduced size as we only store those tuples of (i,j,k) and (a,b,c)
    ! that occur in the problem (e.g., by performing a one-time dry run before the routine to create hashTable).
    ! - one question that remains is how do we perform antisymmetrization? x3(:) will be of the size of n3 * permutations,
    !   e.g., n3 = n3aaa * 36, n3 = n3aab * 4, n3 = n3abb * 4, n3 = n3bbb * 36.
...
end module ccp_quadratic_loops_omp
