module cct3_loops
      ...
      contains
              ...
              function is_active(occ,unocc,&
                                num_act_holes_alpha, num_act_particles_alpha,&
                                num_act_holes_beta,  num_act_particles_beta,&
                                num_beta_spins, noa, nob, num_active) result(flag_active)

                  integer, intent(in) :: occ(3), unocc(3)
                  integer, intent(in) :: num_act_holes_alpha, num_act_particles_alpha,&
                                         num_act_holes_beta, num_act_particles_beta,&
                                         noa, nob
                  integer, intent(in) :: num_beta_spins, num_active

                  integer :: flag_active

                  integer :: order, num_active_particles, num_active_holes, p
                  ...
                  do p = 1 , order - num_beta_spins
                      if (unocc(p) <= num_act_particles_alpha) then
                          num_active_particles = num_active_particles + 1
                      end if
                      if (occ(p) > noa - num_act_holes_alpha) then
                          num_active_holes = num_active_holes + 1
                      end if
                  end do
                  do p = 0 , num_beta_spins-1
                    if (unocc(order - p) <= num_act_particles_beta) then
                        num_active_particles = num_active_particles + 1
                    end if
                    if (occ(order - p) > nob - num_act_holes_beta) then
                        num_active_holes = num_active_holes + 1
                    end if
                  end do

                  if (num_active_particles >= num_active .AND. num_active_holes >= num_active) then
                      flag_active = 1
                  end if
              ...
              subroutine crcc23A_opt(deltaA,deltaB,deltaC,deltaD,&
                              ...
                              D3A_O,D3A_v,num_active,num_act_holes_alpha,num_act_particles_alpha,&
                              noa,nua)
                        ...
                        integer, intent(in) :: num_act_holes_alpha, num_act_particles_alpha, num_active
                                    ...
                                    do a = 1, nua
                                        do b = a+1, nua
                                            do c = b+1, nua
                                                ...
                                                if (is_active((/i, j, k/), (/a, b, c/),&
                                                             num_act_holes_alpha, num_act_particles_alpha,&
                                                             0, 0,&
                                                             0, noa, 0, num_active) == 1) cycle
              ...
              subroutine crcc23B_opt(deltaA,deltaB,deltaC,deltaD,&
                              ...
                              num_active,num_act_holes_alpha,num_act_particles_alpha,&
                        ...
                        integer, intent(in) :: num_active, num_act_holes_alpha, num_act_particles_alpha,&
                                               num_act_holes_beta, num_act_particles_beta
              ...
              subroutine crcc23D_opt(deltaA,deltaB,deltaC,deltaD,&
                        ...
                        integer, intent(in) :: num_active, num_act_holes_beta, num_act_particles_beta
