...
def ccp4(cc_t,p_space,H1A,H1B,H2A,H2B,H2C,ints,sys,flag_RHF=False):

    print('\n==================================++Entering CC(P;4) Routine++=============================')
                        ...
                        for j in range(i+1,sys['Nocc_a']):
                            for k in range(j+1,sys['Nocc_a']):
                                for l in range(k+1,sys['Nocc_a']):
                                        ...
                                        if p_space[a,b,c,d,i,j,k,l] == 1:
                                            continue
                                        else:
                                            D_MP = fA['oo'][i,i]+fA['oo'][j,j]+fA['oo'][k,k]+fA['oo'][l,l]\
                                                -fA['vv'][a,a]-fA['vv'][b,b]-fA['vv'][c,c]-fA['vv'][d,d]
                                            dA_AAAA += LM/D_MP
    ...
    for a in range(sys['Nunocc_a']):
        for b in range(a+1,sys['Nunocc_a']):
            for c in range(b+1,sys['Nunocc_a']):
                for d in range(1,sys['Nunocc_b']):
                    for i in range(sys['Nocc_a']):
                        for j in range(i+1,sys['Nocc_a']):
                            for k in range(j+1,sys['Nocc_a']):
                                for l in range(1,sys['Nocc_b']):
                                        LM = L4B[a,b,c,d,i,j,k,l] * MM24B[a,b,c,d,i,j,k,l]
                                        if p_space[a,b,c,d,i,j,k,l] == 1:
                                            continue
                                        else:
                                            D_MP = fA['oo'][i,i]+fA['oo'][j,j]+fA['oo'][k,k]+fB['oo'][l,l]\
                                                -fA['vv'][a,a]-fA['vv'][b,b]-fA['vv'][c,c]-fB['vv'][d,d]
                                            dA_AAAB += LM/D_MP
    # MM24C correction
    dA_AABB = 0.0
                            ...
                            for k in range(sys['Nocc_b']):
                                for l in range(k+1,sys['Nocc_b']):
                                        ...
                                        if p_space[a,b,c,d,i,j,k,l] == 1:
                                            ...
                                            D_MP = fA['oo'][i,i]+fA['oo'][j,j]+fB['oo'][k,k]+fB['oo'][l,l]\
                                                -fA['vv'][a,a]-fA['vv'][b,b]-fB['vv'][c,c]-fB['vv'][d,d]
                                            dA_AABB += LM/D_MP
    if flag_RHF:
    ...
    print('CC(P,4)_A = {} Eh     Ecorr_A = {} Eh     Delta_A = {} Eh'.format(E24A,EcorrA,deltaA))
    print('CC(P,4)_B = {} Eh     Ecorr_B = {} Eh     Delta_B = {} Eh'.format(E24B,EcorrB,deltaB))
    print('CC(P,4)_C = {} Eh     Ecorr_C = {} Eh     Delta_C = {} Eh'.format(E24C,EcorrC,deltaC))
    print('CC(P,4)_D = {} Eh     Ecorr_D = {} Eh     Delta_D = {} Eh'.format(E24D,EcorrD,deltaD))
