            ...
            for n in range(sys['Nocc_a']):
                for f in range(sys['Nunocc_a']):
                    ...
                    for m in range(n+1,sys['Nocc_a']):
                        ...
                        sigma_a[i,j] -= vA['ooov'][m,n,i,f]*vA['ovoo'][j,f,m,n]/denom
                    # particle diagram
                    for e in range(f+1,sys['Nunocc_a']):
                        denom = e_nf - fA['vv'][e,e] + omega
                        sigma_a[i,j] += vA['oovv'][j,n,e,f]*vA['vvoo'][e,f,i,n]/denom
            # beta loop
            for n in range(sys['Nocc_b']):
                for f in range(sys['Nunocc_b']):
                    ...
                    for m in range(sys['Nocc_a']):
                        denom = fA['oo'][m,m] + e_nf - omega
                        sigma_a[i,j] -= vB['ooov'][m,n,i,f]*vB['ovoo'][j,f,m,n]/denom
                    # particle diagram
                    for e in range(sys['Nunocc_a']):
                        denom = e_nf - fA['vv'][e,e] + omega
                        sigma_a[i,j] += vB['oovv'][j,n,e,f]*vB['vvoo'][e,f,i,n]/denom
    # oob block
            ...
            for n in range(sys['Nocc_a']):
                for f in range(sys['Nunocc_a']):
                    ...
                    for m in range(sys['Nocc_a']):
                        ...
                        sigma_b[i,j] -= (vB['oovo'][n,m,f,i]*vB['vooo'][f,j,n,m])/denom
                    # particle diagram
                    for e in range(sys['Nunocc_a']):
                        denom = e_nf - fB['vv'][e,e] + omega
                        sigma_b[i,j] += (vB['oovv'][n,j,f,e]*vB['vvoo'][f,e,n,i])/denom
            # beta loop
            for n in range(sys['Nocc_b']):
                for f in range(sys['Nunocc_b']):
                    e_nf = fB['oo'][n,n] - fB['vv'][f,f]
                    ...
                    for m in range(n+1,sys['Nocc_b']):
                        denom = fB['oo'][m,m] + e_nf - omega
                        sigma_b[i,j] -= (vC['ooov'][m,n,i,f]*vC['ovoo'][j,f,m,n])/denom
                    # particle diagram
                    for e in range(f+1,sys['Nunocc_b']):
                        denom = e_nf - fB['vv'][e,e] + omega
                        sigma_b[i,j] += (vC['oovv'][j,n,e,f]*vC['vvoo'][e,f,j,n])/denom
...
def gf2_ip(nroot,ints,sys,maxit=50,tol=1.0e-08):
    ...
    for p in range(sys['Nocc_a']-1,sys['Nocc_a']-nroot-1,-1):
        ...
        for it in range(maxit):
            ...
            sigma_a, _ = mbgf_loops.mbgf_loops.mbgf2_selfenergy(e0,fA['oo'],fA['vv'],fB['oo'],fB['vv'],\
                            vA['oovv'],vA['vvoo'],vA['ooov'],vA['ovoo'],\
                            vB['oovv'],vB['vvoo'],vB['ooov'],vB['ovoo'],vB['oovo'],vB['vooo'],\
    ...
    for i,ip_energy in enumerate(omega):
        ...
        print('     Root - {}      IP ENERGY = {:>8f}      [{}]'.format(i+1,ip_energy,conv_str))
